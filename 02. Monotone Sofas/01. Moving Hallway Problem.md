...
> __Theorem [sofa-iff-hallway-intersection].__ A shape $S$ is a sofa with rotation angle $\omega$ if and only if $S$ is connected and there is a proper rigid transformation $f$ such that $f(S)$ is contained in some hallway intersection with the angle set $\Theta = [0, \omega]$. ^thm-sofa-iff-hallway-intersection