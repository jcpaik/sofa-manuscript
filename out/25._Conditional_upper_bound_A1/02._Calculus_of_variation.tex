We will prove \Cref{thm:main-cap} by optimizing \(\mathcal{A}_1 : \mathcal{K}_\omega \to \mathbb{R}\). Here, we prepare a minimal amount of language needed to execute the calculus of variations to a concave quadratic functional \(f\) on a general convex space \(\mathcal{K}\). Later, we will show that our functional \(f = \mathcal{A}_1\) is concave and quadratic on the cap space \(\mathcal{K} = \mathcal{K}_\omega\), and then deploy the calculus of variations on \(\mathcal{A}_1\).
