...
For reference, the notion of cap \(K\) as a kind of convex bodies is in \Cref{def:cap}. The space of all caps \(\mathcal{K}_\omega\) with rotation angle \(\omega\) is in \Cref{def:cap-space}. The vertices \(A_K^{\pm}(t)\), \(C_K^{\pm}(t)\) of a cap \(K\) is in \Cref{def:cap-vertices}. The upper boundary \(\delta K\) of a cap \(K\) is in \Cref{def:upper-boundary-of-cap}.