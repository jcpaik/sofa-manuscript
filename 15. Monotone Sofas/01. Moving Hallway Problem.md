We observed that a sofa is essentially the intersection of rotating sofas in the introduction. Let us examine this idea more carefully.
...
Note that $P_\omega$ is the horizontal strip $H$ if $\omega=\pi/2$, and a proper parallelogram if $\omega < \pi/2$. Let us push $S$ towards the upper edges $l(\omega, 1)$ and $l(\pi/2, 1)$ inside $P_\omega$. This puts $S$ in _standard position_ defined as the following.
