We first rigorously show that a sofa that rotates by an angle of $\omega$ is, up to translation, an intersection of the parallelogram $P_\omega$ and hallways rotated counterclockwise by an angle of $t \in [0, \omega]$.
...
> __Theorem [sofa-iff-hallway-intersection].__ A shape $S$ is a sofa with rotation angle $\omega \in [0, \pi/2]$ if and only if $S$ is connected and there is a proper rigid transformation $g$ such that $g(S)$ is a hallway intersection with the angle set $\Theta = [0, \omega]$. Moreover, any hallway intersection $S$ with the angle set $\Theta = [0, \omega]$ admits the movement $f_S(t)$ inside $L$. ^thm-sofa-iff-hallway-intersection
