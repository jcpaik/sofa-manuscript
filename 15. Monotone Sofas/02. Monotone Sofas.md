...
> 
> If the angle set $\Theta$ is $[0, \omega]$, then we call $S$ a _monotone sofa_ with cap $K$. Indeed, $S$ is a sofa by [[01. Moving Hallway Problem#^thm-sofa-iff-hallway-intersection]]. Observe that $S$ admits the movement of rotating hallway $L_K(t) = L_S(t)$. ^def-monotone-sofa
