...
$$
\mathcal{N}_\Theta(S) = F_{\omega} \cap \bigcup_{t \in \Theta} Q^-_S(t) 
$$
> Also, define the _monotonization_ of $S$ as the set $\mathcal{M}_\Theta(S) = \mathcal{C}_\Theta(S) \setminus \mathcal{N}_\Theta(S)$. If the angle set $\Theta$ is clear from context, we will omit the subscript $\Theta$ for the cap/cup/monotonization $\mathcal{C}(S)$, $\mathcal{N}(S)$, and $\mathcal{M}(S)$. 
