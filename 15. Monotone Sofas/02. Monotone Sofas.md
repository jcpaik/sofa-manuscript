...
> __Corollary [monotonization-is-hallway-intersection].__ For any hallway intersetion $S$ with angle set $\Theta$, the set $\mathcal{M}_{\Theta}(S)$ is a monotone hallway intersection with cap $K = \mathcal{C}(S)$. ^cor-monotonization-is-hallway-intersection
