...
The tangent line $l_S(0)$ of $S$ is strictly on the right side of $l_{\pi/2}$. To see this, assume otherwise by contradiction. Then as $p_S(0) = p_{\mathcal{M}(S)}(0)$ we have $l_S(0) = l_{\mathcal{M}(S)}(0)$. So the point  $p \in \mathcal{M}_\Theta(S)$ this is contradiction. If $l_S(0)$ passes $p$, then we prove that $l_{\pi/2} = l_{S}(0)$ passes $p$ so we are done. So we are left with the assumption that $l_{S}(0)$ is strictly on the right side of $l_{\pi/2}$. Consequently, some point of $S$ is strictly on the right side of $l_{\pi/2}$.
