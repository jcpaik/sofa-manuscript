...
> __Theorem [monotonization-eq].__ Let $S$ is any hallway intersection in standard position with rotation angle $\omega$ and angle set $\Theta$. Then the monotonization $\mathcal{M}(S)$ of hallway intersection $S$ is equal to $K \setminus \mathcal{N}(K)$ where $K = \mathcal{C}(S)$ is the cap of $S$ ([[02. Monotone Sofas#^def-monotonization-cap]]) and $\mathcal{N}(K)$ is the niche of $K$ ([[02. Monotone Sofas#^def-niche]]). Consequently, any monotone hallway intersection is $K \setminus \mathcal{N}(K)$ for some cap $K$. ^thm-monotonization-eq
