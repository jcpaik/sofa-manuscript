...
(TODO: insert a figure of a generic cap)
...
> __Definition [niche-monotonization-hallway-intersection].__ Let $S$ be any shape $S$ in standard position with rotation angle $\omega \in [0, \pi/2]$. Define the _cap_ $\mathcal{C}(S)$ of $S$ as the following set. ^def-niche-hallway-intersection
$$
\mathcal{C}(S) = P_\omega \cap \bigcap_{t \in \Theta} Q^+_S(t)
$$
> Define the _niche_ of $S$ as the following set.
$$
...
We get the following from [[#^cor-hallway-intersection-quadrant]] and [[#^def-niche-hallway-intersection]]. 
...
> __Theorem [monotonization-of-cap].__  ^thm-monotonization-of-cap
