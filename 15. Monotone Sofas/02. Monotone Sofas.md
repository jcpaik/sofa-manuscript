...
Let $y_L = l_K(0) \cap l(0, \pi/2)$ and $y_R = l_K(\omega + \pi/2) \cap l(0, \omega)$ be the left and right ends of the upper boundary $\delta K$ of $K$. By [[03. Geometric Results#^thm-boundary-parametrization]], the function $\mathbf{x}_{K, y_L, y_R}$ is a parametrization of $\delta K$ as a curve from $y_L$ to $y_R$. So $\delta K$ is connected in particular. We will show the following two facts.

- (a) The point $z_L = d_K(t) \cap l(\omega, 0)$ lies right to the point $y_L$.
- (b) The point $z_R = b_K(t) \cap l(\pi/2, 0)$ lies left to the point $y_R$.

We first show that $z_L$ lies right to the point $y_L$. Say that a point $p_0$ is further than $p_1$ in the direction of nonzero vector $v \in \mathbb{R}^2$ if and only if $p_0 \cdot v \geq p_1 \cdot v$. For a fixed $v$, this is a partial order. 
