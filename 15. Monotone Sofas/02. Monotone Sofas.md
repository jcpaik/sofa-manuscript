...
Define the niche $\mathcal{N}(K)$ of an arbitrary cap $K$. Note that this is essentially the region carved out from the cap $K$ in our monotone sofa $M$ in discussion.
...
> __Definition [monotonization].__ Let $K$ be any cap in standard position with rotation angle $\omega \in [0, \pi/2]$ and angle set $\Theta \subseteq [0, \omega]$. Define the _monotonization_ of $K$ as the following set. ^def-monotonization
$$
\mathcal{M}(K) = P_\omega \cap \bigcap_{t \in \Theta} L_K(t) = K \setminus \mathcal{N}(K)
$$

_Proof._ The equality is justified by the following.


□

$$
\mathcal{M}(K) = P_\omega \cap \bigcap_{t \in \Theta} L_K(t) =
\left( P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t) \right) \setminus \left( F_\omega \cap \bigcup_{0 \leq t \leq \omega} Q^-_S(t) \right) 
$$
