...
> __Definition [cap-hallway-intersection].__ Let $S$ be any hallway intersection in standard position with rotation angle $\omega \in [0, \pi/2]$ and angle set $\Theta \subseteq [0, \omega]$. Define the _cap_ $K = \mathcal{C}(S)$ of $S$ as the following set. ^def-cap-hallway-intersection
$$
