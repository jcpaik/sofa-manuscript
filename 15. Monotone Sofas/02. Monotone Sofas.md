...
$$
> Say that a shape is _monotone_ with respect to rotation angle $\omega$ and angle set $\Theta$ if it is the monotonization of some $S$, that is, $\mathcal{M}(S)$ for some hallway intersection $S$ with angle set $\Theta$. ^def-monotonization
