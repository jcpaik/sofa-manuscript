...
\left( P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t) \right) \setminus \bigcup_{0 \leq t \leq \omega} Q^-_S(t)
$$
The set $K = P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t)$ is a convex set bounded by parallelogram $P_\omega$ and closed quarter-planes $Q^+_S(t)$. It has a particular shape that we define as a _cap_.
...
> __Theorem [cap-hallway-intersection].__ Let $S$ be any hallway intersection in standard position with rotation angle $\omega \in [0, \pi/2]$ and angle set $\Theta \subseteq [0, \omega]$. Define  $\mathcal{C}(S)$ as the following set. ^thm-cap-hallway-intersection
$$
\mathcal{C}(S) = P_\omega \cap \bigcap_{t \in \Theta} Q^+_S(t)
$$
> Then $\mathcal{C}(S)$ is a cap with rotation angle $\omega$ and angle set $\Theta$. We call $\mathcal{C}(S)$ as the _cap_ of $S$.
...
Now as the set $\bigcap_{t \in \Theta} Q^+_S(t)$ is closed in the direction of $-u_\omega$ and $-v_0$, the orthogonal projections of $p_\omega$ to the bottom sides $l(\omega, 0)$ and $l(\pi/2, 0)$ of $P_\omega$ respetively are also contained in $K$. This implies that $p_K(\pi + \omega) = p_K(3\pi/2) = 0$. □
