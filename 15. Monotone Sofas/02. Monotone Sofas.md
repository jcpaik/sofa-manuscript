...
$$
The set $K = P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t)$ is a convex set bounded by parallelogram $P_\omega$ and closed quarter-planes $Q^+_S(t)$. It has a particular shape that we soon define as a _cap_.  The convex cap $K$ is carved out by the inner corners $Q^-_S(t)$. 
