Let $S$ be a hallway intersection with rotation angle $\omega \in [0, \pi/2]$ and angle set $\Theta$. Here we construct the _motononization_ $\mathcal{M}(S)$ of a sofa $S$ which is a sofa that contains $S$ that consists of its cap $\mathcal{C}(S)$ minus its niche $\mathcal{N}(S)$.
