...
As the line $l_\theta$ is disjoint from $S$ for any $\theta \in [\omega, \pi/2]$, $S$ is inside the set $X = \mathbb{R}^2 \setminus \bigcup_{\theta \in [\omega, \pi/2]} l_\theta$. Note that the set $X$ has exactly two connected components $X_L$ and $X_R$, one in left and another in right respectively. As there is a point of $S$ strictly on the right side of $l_{\pi/2}$, the point of $S$ lies in $X_R$. As there is also a point of $S$ strictly on the left side of $l_\omega$, the point of $S$ lies in $X_L$. Then we get contradiction as $S$ is connected and a subset of $X$. □
