...
Second, the monotonization consists of a cap subtracted by the niche determined by the cap. The cap $K$ of S is defined as the intersection of Pw and Qs. So it is convex. The niche is the region carved out by the inner corners. The tangent hallways of $S$ and $K$ are the same. We will show that $\mathcal{N}(K)$ is strictly inside $K$ and does not touch the upper boundary of $K$. As one can see, everything is determined by the cap $K$. This helps as we can understand everything in $\mathcal{K}$. 
