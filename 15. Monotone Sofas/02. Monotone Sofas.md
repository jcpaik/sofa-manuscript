...
> __Definition [monotone-sofa].__ A _monotone hallway intersection_ $S$ with cap $K$ and angle set $\Theta$ and rotation angle $\omega$ is the shape $S = \mathcal{M}_\Theta(K)$ that is connected. Observe that such $S$ is a hallway intersection: by condition 2 of [[#^thm-monotonization-connected-iff]], $S$ is in standard position. ^def-monotone-sofa

> __Corollary [monotonization-is-hallway-intersection].__ For any hallway intersetion $S$ with angle set $\Theta$, the set $\mathcal{M}_{\Theta}(S)$ is a monotone hallway intersection with cap $K = \mathcal{C}(K)$. ^cor-monotonization-is-hallway-intersection
