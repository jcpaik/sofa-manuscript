...
_Proof._ Letting $K = \mathcal{C}_\Theta(S)$, we have $M = \mathcal{M}_\Theta(S) = \mathcal{M}_\Theta(K)$ by [[#^cor-monotonization-of-cap]]. So $M$ is a monotone hallway intersection with cap $K$ by [[#^thm-monotonization-is-hallway-intersection]] and [[#^def-monotone-sofa]]. By the remark in [[#^def-monotone-sofa]] we have $K = \mathcal{C}(M)$. □
