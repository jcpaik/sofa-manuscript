...
> __Corollary [maximum-area-monotone-sofa].__ Let $S$ be any sofa with rotation angle $\omega \in [0, \pi/2]$. Then the monotonization $\mathcal{M}_{[0, \omega]}(S)$  with cap $K = \mathcal{C}(S)$ after some proper rigid motion. ^cor-maximum-area-monotone-sofa
