...
> __Definition [upper-boundary-of-cap].__ Define the _upper boundary_ $\delta K$ of $K$ as the set $\delta K = \bigcup_{t \in \Theta \cup \Theta+\pi/2} e_K(t)$. This is the path of the boundary from vertex $v_K^-(0)$ to $v^+_K(0)$. Moreover, the $x$-coordinate of the parametrization is monotonically decreasing. (TODO: add reference)  ^def-upper-boundary-of-cap
