...
> __Definition [cap-vertices].__ Let $K$ be a cap with angle set $\Theta$ and rotation angle $\omega$. For any $t \in [0, \omega]$, define $A^+_K(t) = v^+_K(t)$, $A^-_K(t) = v^-_K(t)$,  $C^+_K(t) = v^+_K(t + \pi/2)$, and $C^-_K(t) = v^+_K(t + \pi/2)$. ^def-cap-vertices
...
By [[03. Geometric Results#^thm-boundary-parametrization]], the function $\mathbf{x}_{K, y_L, y_R}$ is a parametrization of $\delta K$ as a curve from $y_L$ to $y_R$. 
