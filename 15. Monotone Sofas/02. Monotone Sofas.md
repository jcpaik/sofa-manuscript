...
Define the niche of an arbitrary cap $K$.
...
> __Definition [monotone-intersection].__ A _monotone intersection_ $S$ with cap $K$, angle set $\Theta$, and rotation angle $\omega$ is the shape $M = \mathcal{M}(K) = K \setminus \mathcal{N}(K)$ that is connected. If the angle set $\Theta$ is $[0, \omega]$, then we call $S$ a _monotone sofa_ with cap $K$. ^def-monotone-intersection
