...
We make small observations on the upper boundary of $K$.
...
> __Lemma [upper-boundary-connected].__ The upper boundary $\delta K$ of any cap $K$ is connected. ^lem-upper-boundary-connected

_Proof._ If $K$ has empty interior, then by [[02. Monotone Sofas#^lem-cap-empty-interior]] $\delta K = K$ is connected. If $K$ has nonempty interior, by [[02. Geometric Results#^thm-boundary-parametrization]], the function $\mathbf{x}_{K, A^-_K(0), C^+_K(\omega)}$ is a Jordan arc parametrization of $\delta K$ from $A_K^-(0)$ to $C^+_K(\omega)$. In particular, $\delta K$ is connected. □
