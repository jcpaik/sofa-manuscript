...
> __Definition [monotonization].__ Let $S$ is any hallway intersection in standard position with rotation angle $\omega$ and angle set $\Theta$. Define the _monotonization_ of $S$ as the following set. Say that a shape is _monotone_ with respect to rotation angle $\omega$ and angle set $\Theta$ if it is equal to $\mathcal{M}(S)$ for some hallway intersection $S$ with angle set $\Theta$. ^def-monotonization
$$
...
We show that a wedge $T_K(t)$ is contained in $K$ if and only if its tip $\mathbf{x}_K(t)$ is either in $\mathbb{R}^2 \setminus F_\omega^\circ$ or $K$.
