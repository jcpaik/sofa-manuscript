Let $S \subseteq P_\omega$ be a sofa in standard position with rotation angle $\omega \in [0, \pi/2]$. We have observed in [[01. Moving Hallway Problem#^thm-in-tangent-hallway]] that for any $t \in [0, \omega]$, $S$ is contained in the tangent hallway $L_S(t)$. The idea of monotone sofa is that the intersection $P_\omega \cap \bigcap_{0 \leq t \leq \omega} L_S(t)$ of all the strips and all pushed hallways is a sofa that contains $S$ with better properties.

This intersection can be separated into two parts.

$$
P_\omega \cap \bigcap_{0 \leq t \leq \omega} L_S(t) =
...
Then we define what is the cap of a hallway intersection, and verify that it is indeed a cap. 
