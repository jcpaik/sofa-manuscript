...
Our main observations on the monotone are the followings. First, the monotonization of any hallway intersection $S$ is connected and so is a hallway intersection. Thus, it is safe to assume that a hallway intersection with angle set $\Theta$ of the maximum area is monotone.

The second observation is that $\mathcal{M}(S) = K \setminus \mathcal{N}(K)$ consists of a convex body $K$ called _cap_ bounded by the outer walls of $L_S(t)$, subtracted by the _niche_ $\mathcal{N}(K)$ which is the region carved out by the inner corner of $L_S(t)$ (see [[01. Introduction#^fig-monotone-sofa]]). 

To describe this exactly, we first define a _fan_.
...
\end{align}
$$
We define the convex body $K = P_\omega \cap \bigcap_{t \in \Theta} Q^+_S(t)$ as the _cap_ of $S$. The outer walls $a_S(t), c_S(t)$ of the tangent hallways forms the upper tangent lines of $K$. The cap $K$ is then carved out by the wedges $F_\omega \cap Q_S^-(t)$ with the inner corners $\mathbf{x}_S(t)$ as tip. We call the union of such wedges as the _niche_ of $S$. It turns out that $Q^-_S(t)$ is equal to $Q_K^-(t)$, so the niche $\mathcal{N}(K) = \bigcup_{t \in \Theta} \left( F_\omega \cap Q^-_K(t) \right)$ is determined by the cap $K$.

As a maximum area hallway intersection $S$ (e.g. a maximum sofa) can be assumed to be monotone, we can assume that $S = K \setminus \mathcal{N}(K)$. Moreover, we show that the niche $\mathcal{N}(K)$ is strictly inside the cap $K$ and does not touch the upper part of the boundary of $K$. This reduces the complexity of the shape of $S$ to that of the union $\mathcal{N}(K)$ of wedges. As $S$ is completely determined by the cap $K$, in later chapters we will understand $S$ mainly in terms of $K$. This gives an advantage as the space $\mathcal{K}_\Theta$ of all caps with angle set $\Theta$ forms a convex space, making the optimization much easier.
...
> __Corollary [monotonization-eq].__ Let $S$ is any hallway intersection in standard position with rotation angle $\omega$ and angle set $\Theta$. Then the monotonization of hallway intersection $S$ ([[02. Monotone Sofas#^def-monotonization]]) is equal to the monotonization of the cap $K = \mathcal{C}(S)$ ([[02. Monotone Sofas#^def-monotonization-cap]]). ^cor-monotonization-eq
