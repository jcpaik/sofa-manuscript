...
Our second main theorem describes the geometric structure of monotonization $\mathcal{M}(S)$ (see [[01. Introduction#^fig-monotone-sofa]]). We will provide the exact definitions later as they are technical.
...
A maximum area hallway intersection $S$ (e.g. a maximum sofa) can be assumed to be monotone by [[02. Monotone Sofas#^thm-monotonization-is-hallway-intersection]], so we can assume that $S = K \setminus \mathcal{N}(K)$ using [[02. Monotone Sofas#^thm-monotonization-eq]]. Moreover, we show that the niche $\mathcal{N}(K)$ is strictly inside the cap $K$ and does not touch the upper part of the boundary of $K$. With this, the maximum area is $|S| = |K| - |\mathcal{N}(K)|$, and we only need to understand the area of niche $\mathcal{N}(K)$. This is essential to proceed a new upper bound in the next chapter.
...
For the proof of [[02. Monotone Sofas#^thm-niche-in-cap]], we identify the exact condition when $K \setminus \mathcal{N}(K)$ is connected and thus a hallway intersection ([[02. Monotone Sofas#^thm-monotonization-connected-iff]]) that will be also useful later.

As $S = K \setminus \mathcal{N}(K)$ is completely determined by the cap $K$, in later chapters we will understand $S$ in terms of $K$. This gives an advantage as the space $\mathcal{K}_\Theta$ of all caps with angle set $\Theta$ forms a convex space, making the optimization much easier. Note however that not all cap $K \in \mathcal{K}_\Theta$ gives rise to a hallway intersection $K \setminus \mathcal{N}(K)$, as $K \setminus \mathcal{N}(K)$ might be disconnected if $K$ is too wide.  In this way, the set of monotone hallway intersections can be considered as a closed subset of the cap space $\mathcal{K}_\Theta$.
