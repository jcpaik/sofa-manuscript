...
> __Definition [fan].__ For any angle $\omega \in [0, \pi/2]$, define the _fan_ $F_\omega = H(0, \pi+\omega) \cap H(0, 3\pi/2)$. $F_\omega$ is the convex cone spanned by $u_0$ and $v_\omega$. Denote the edges of $F_\omega$ as $l_{\pi/2} = l(0, \pi/2)$ and $l_\omega = l(0, \omega)$. Denote the interior of $F_\omega$ as $F_\omega^\circ = H^\circ(0, \pi+\omega) \cap H^\circ(0, 3\pi/2)$. ^def-fan

Note that the fan $F_\omega$ contains the parallelogram $P_\omega$.

Writing each $L_S(t)$ as $Q^+_S(t)\setminus Q^-_S(t)$, the intersection $\mathcal{M}(S)$ can be separated into two parts.
$$
...
> __Definition [niche-hallway-intersection].__ Let $S$ be any shape $S$ in standard position with rotation angle $\omega \in [0, \pi/2]$ and angle set $\Theta \subseteq [0, \omega]$. Define the set $T_S(t) = F_\omega \cap Q_S^-(t)$ for any $t \in [0, \omega]$. Call such set a _triangular set_. Define the _niche_ of $S$ as the union of all triangular sets. ^def-niche-hallway-intersection
$$
\mathcal{N}(S) = F_{\omega} \cap \bigcup_{t \in \Theta} Q^-_S(t) = \bigcup_{t \in \Theta} T_S(t) 
$$
...
> __Definition [monotone-sofa].__ A _monotone hallway intersection_ $S$ with cap $K$, angle set $\Theta$, and rotation angle $\omega$ is the shape $\mathcal{M}(K) = K \setminus \mathcal{N}(K)$ that is connected. 
