...
Let $S \subseteq P_\omega$ be a sofa in standard position with rotation angle $\omega \in [0, \pi/2]$. We pushed the moving hallway towards $S$ as much as possible in [[01. Moving Hallway Problem#^thm-in-tangent-hallway]], so that for any angle $t \in [0, \omega]$ the tangent hallway $L_S(t)$ contains $S$ and touches $S$ in the outer walls $a_S(t)$ and $c_S(t)$. The idea of monotone sofa is to take the intersection $M = P_\omega \cap \bigcap_{0 \leq t \leq \omega} L_S(t)$ of all the strips and tangent hallways. We will call such $M$ as a _monotone sofa_, and show that the monotone sofa $M$ contains $S$ and has good geometric properties.

Let us examine $M$ in more details. For any angle $\omega \in [0, \pi/2]$, define the fan $F_\omega$ as the following.
...
Writing each $L_S(t)$ as $Q^+_S(t)\setminus Q^-_S(t)$, the intersection $M$ can be separated into two parts.
$$
...
Define the niche of arbitrary cap $K$.

> __Definition [niche-hallway-intersection].__ Let $K$ be any cap in standard position with rotation angle $\omega \in [0, \pi/2]$ and angle set $\Theta \subseteq [0, \omega]$. Define the _triangular set_ $T_K(t) = F_\omega \cap Q_K^-(t)$ of $K$ for any $t \in \Theta$. Define the _niche_ of $K$ as the union of all triangular sets. ^def-niche-hallway-intersection
$$
\mathcal{N}(K) = F_{\omega} \cap \bigcup_{t \in \Theta} Q^-_K(t) = \bigcup_{t \in \Theta} T_K(t) 
$$

> __Definition [monotone-intersection].__ A _monotone intersection_ $S$ with cap $K$, angle set $\Theta$, and rotation angle $\omega$ is the shape $S = \mathcal{M}(K) = K \setminus \mathcal{N}(K)$ that is connected. If the angle set $\Theta$ is $[0, \omega]$, then we call $S$ a _monotone sofa_ with cap $K$. ^def-monotone-intersection
