Let $S \subseteq P_\omega$ be a sofa in standard position with rotation angle $\omega \in [0, \pi/2]$. We pushed the moving hallway towards $S$ as much as possible in [[01. Moving Hallway Problem#^thm-in-tangent-hallway]], so that for any angle $t \in [0, \omega]$ the tangent hallway $L_S(t)$ contains $S$ and touches $S$ in the outer walls $a_S(t)$ and $c_S(t)$. The idea of monotone sofa is to take the intersection $\mathcal{M}(S) = P_\omega \cap \bigcap_{0 \leq t \leq \omega} L_S(t)$ of all the strips and tangent hallways. The resulting shape $\mathcal{M}(S)$ is then a sofa that contains $S$ with better geometric properties. We will examine the shape of $\mathcal{M}(S)$ in details.

For any angle $\omega \in [0, \pi/2]$, define the fan $F_\omega$ as the following.

> __Definition [fan].__ For any angle $\omega \in [0, \pi/2]$, define the _fan_ $F_\omega = H(0, \pi+\omega) \cap H(0, 3\pi/2)$. $F_\omega$ is the convex cone spanned by $u_0$ and $v_\omega$. ^def-fan

Note that the fan $F_\omega$ contains the parallelogram $P_\omega$. Writing each $L_S(t)$ as $Q^+_S(t)\setminus Q^-_S(t)$, the intersection $\mathcal{M}(S)$ can be separated into two parts.
$$
...
$$
The set $K = P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t)$ is a convex set bounded by parallelogram $P_\omega$ and closed quarter-planes $Q^+_S(t)$. It has a particular shape that we soon define as a _cap_.  The convex cap $K$ is carved out by the inner corners $Q^-_S(t)$. We will define the carved region $F_\omega \cap \bigcup_{0 \leq t \leq \omega} Q^-_S(t)$ as the _niche_ $\mathcal{N}(K)$ of $K$. 

# Cap

We formally define what is a cap.
...
Let us formally show that the set $K = P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t)$ indeed adheres to [[02. Monotone Sofas#^def-cap]]. Note that we prove this in the general case where $S$ is a hallway intersection.
...
> __Lemma [cap-same-support-function].__ Let $S$ be any hallway intersection in standard position with rotation angle $\omega \in [0, \pi/2]$ and angle set $\Theta \subseteq [0, \omega]$. Let $K = \mathcal{C}(S)$ be its cap. Then the support functions $p_S$ and $p_K$ have the same value on the set $\Theta \cup (\Theta + \pi/2)$. Consequently, the tangent hallways $L_S(t)$ and $L_K(t)$ are the same on any $t \in \Theta$, and so are the parts $a_S(t) = a_K(t)$, $b$, $c$, $d$, $\mathbf{x}$, $\mathbf{y}$, $Q^+$, $Q^-$ of $L$. ^lem-cap-same-support-function
...
The lines $a_K(t)$ and $c_K(t)$ are the upper edges of the cap $K$. We also define the vertices of a cap.
...
> __Corollary [cap-edges].__  ^cor-cap-edges
...
$$
> Define the set $T_S(t) = F_\omega \cap Q_S^-(t)$ for any $t \in [0, \omega]$. Call such set a _triangular set_. Define the _niche_ of $S$ as the following set, which is the union of all triangular sets.
$$
...
> __Definition [monotone-sofa].__ A _monotone hallway intersection_ $S$ with cap $K$ and angle set $\Theta$ and rotation angle $\omega$ is the shape $S = \mathcal{M}_\Theta(K)$ that is connected. Observe that such $S$ is a hallway intersection: by condition 2 of [[#^thm-monotonization-connected-iff]], $S$ is in standard position. Also, by condition 2 of [[#^thm-monotonization-connected-iff]], $S$ contains the upper boundary of $K$ and so we have $\mathcal{C}(S) = K$. 
> 
> If the angle set $\Theta$ is $[0, \omega]$, then we call $S$ a _monotone sofa_ with cap $K$. Indeed, $S$ is a sofa by [[01. Moving Hallway Problem#^thm-sofa-iff-hallway-intersection]]. Observe that $S$ admits the movement of rotating hallway $L_S(t) = L_K(t)$. ^def-monotone-sofa

> __Corollary [monotonization-is-hallway-intersection].__ For any hallway intersetion $S$ with angle set $\Theta$, the set $M = \mathcal{M}_{\Theta}(S)$ is a monotone hallway intersection with cap $K = \mathcal{C}(S) = \mathcal{C}(M)$. ^cor-monotonization-is-hallway-intersection

_Proof._ Letting $K = \mathcal{C}_\Theta(S)$, we have $M = \mathcal{M}_\Theta(S) = \mathcal{M}_\Theta(K)$ by [[#^cor-monotonization-of-cap]]. So $M$ is a monotone hallway intersection with cap $K$ by [[#^thm-monotonization-is-hallway-intersection]] and [[#^def-monotone-sofa]]. By the remark in [[#^def-monotone-sofa]] we have $\mathcal{C}(M) = K$ as well. □
