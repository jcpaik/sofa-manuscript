...
Our first main observation is that the monotonization of any hallway intersection $S$ is connected and so is a hallway intersection. Thus, it is safe to assume that a hallway intersection with angle set $\Theta$ of the maximum area is monotone.

> __Theorem [monotonization-is-hallway-intersection].__ Any shape that is monotone with respect to angle set $\Theta$ is connected and thus itself a hallway intersection with angle set $\Theta$. ^thm-monotonization-is-hallway-intersection

Our second main observation is that a monotone shape $\mathcal{M}(S) = K \setminus \mathcal{N}(K)$ consists of a convex body $K$ called _cap_ bounded by the outer walls of $L_S(t)$, subtracted by the _niche_ $\mathcal{N}(K)$ which is the region carved out by the inner corner of $L_S(t)$ ([[02. Monotone Sofas#^cor-monotonization-eq]]; see [[01. Introduction#^fig-monotone-sofa]]). To describe this exactly, we first define a _fan_.
...
Now define the niche $\mathcal{N}(K)$ of an arbitrary cap $K$.
...
> 
> Consequently, the endpoints $C^+_K(\omega)$ and $A_K^-(0)$ of $\delta K$ are not in the niche $\mathcal{N}(K)$ of $K$.
...
> __Lemma [wedge-in-cap].__ Let $K$ be any cap with angle set $\Theta$ and rotation angle $\omega$. The followings are equivalent. ^lem-wedge-in-cap
> 
> 1. $T_K(t) \subseteq K$
...
_Proof._ 
(3 $\Rightarrow$ 2) To show this, it suffices to show that the wedge $T_K(t) = F_\omega \cap Q^-_K(t)$ is contained in $K$ for any $t \in \Theta$. Then, as $T_K(t)$ is an open set with respect to subset topology of $F_\omega$, and $K \setminus \delta K$ is the interior of $K$ inside the subset topology of $F_\omega$ ([[02. Monotone Sofas#^lem-cap-fan-topology]]), we prove that $T_K(t)$ is disjoint from $\delta K$. If $\mathbf{x}(t) \not\in F_\omega^\circ$ then $T_K(t)$ is an empty set and we're done. So we can assume that $\mathbf{x}(t)$ is in $F_\omega ^ \circ$. Note that $T_K(t)$ degenerates to an empty set when $t$ is 0 or $\omega$, so we assume $0 < t < \omega$.
 
We then show the following two facts.

- (a) The point $C^+_K(\omega)$ is strictly further than the point $z_L = d_K(t) \cap l(\omega)$ in the direction $v_\omega$.
- (b) The point $A_K^-(0)$ is strictly further than the point $z_R = b_K(t) \cap l(\pi/2)$ in the direction $u_0$.

We show (a). The point $c_K(t) \cap l(\omega, 1)$ is strictly further than $z_L = d_K(t) \cap l(\omega, 0)$ in the direction of $v_\omega$ as they are the intersections of boundaries of two unit-width strips $R_\omega(V)$ and $[f_K(t)](H)$. Take any point $q$ in $\delta K \cap c_K(t)$, then $q$ is further than $c_K(t) \cap l(\omega, 1)$ in the direction of $v_\omega$. Finally, the endpoint $C^+_K(\omega)$ is further than $q$ in the direction of $v_\omega$. Thus, $C^+_K(\omega)$ is strictly further than $z_L$ in the direction of $v_\omega$. The proof of (b) can be done using a symmetric argument.

If $(0, 0)$ lies above or on the lines $b_K(t)$ and $d_K(t)$, then it contradicts the assumption that $\mathbf{x}(t)$ is in $F_\omega ^ \circ$. So there are essentially three types of how the wedge $T_K(t)$ can be positioned inside $F_\omega$ as $\mathbf{x}(t) \in K$ and $0 < t < \omega$.
...
Now we finish the proof that the wedge $T_K(t)$ is in $K$. Observe that $K$ contains $(0, 0)$, $C^+_K(\omega)$ and $A_K^-(0)$. In case (i), the wedge is the triangle with vertices $\mathbf{x}(t)$, $z_L$, and the point $l(\omega) \cap b_K(t)$ on the line segment connecting $z_L$ and the origin. By the convexity of $K$ and (a), the three vertices are in $K$ and $T_K(t)$ is contained in $K$. In case (ii), the wedge is the quadrilateral with vertices $z_L, (0, 0), z_R$, and $\mathbf{x}(t)$, and all the vertices are in $K$ by the convexity of $K$ and (a) and (b). Finally, in case (iii) the wedge is the triangle with vertices $\mathbf{x}(t)$, $l(\pi/2) \cap d_K(t)$, and $z_R$, and they are in $K$ by convexity of $K$ and (b). So $T_t$ is contained in $K$, completing the proof.
