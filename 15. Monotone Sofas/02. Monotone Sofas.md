...
> __Corollary [cap-contains-hallway-intersection].__ For any hallway intersection $S$, its cap $\mathcal{C}(S)$ contains $S$. ^cor-cap-contains-hallway-intersection

> __Lemma [cap-same-support-function].__ Let $S$ be any hallway intersection in standard position with rotation angle $\omega \in [0, \pi/2]$ and angle set $\Theta \subseteq [0, \omega]$. Let $K = \mathcal{C}(S)$ be its cap. Then the support functions $p_S$ and $p_K$ have the same value on the set $\Theta \cup (\Theta + \pi/2)$. Consequently, the tangent hallways $L_S(t)$ and $L_K(t)$ are the same on any $t \in \Theta$, and its counter parts $a_S(t) = a_K(t)$, $Q^-_S(t) = Q^-_K(t)$, etc. ^lem-cap-same-support-function
