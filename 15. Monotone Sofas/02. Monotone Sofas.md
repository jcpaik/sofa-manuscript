...
Our first main theorem is that the monotonization of any hallway intersection $S$ is connected and so is a hallway intersection. Thus, it is safe to assume that a hallway intersection of the maximum area with angle set $\Theta$ is monotone.
...
Our second main theorem describes the geometric structure of monotonization $\mathcal{M}(S)$ (see [[01. Introduction#^fig-monotone-sofa]]).
...
The _cap_ $K$ is a convex set with the outer walls of $L_S(t)$ as tangent lines. The _niche_ $\mathcal{N}(K)$ is the region carved out by the inner corner of $L_S(t)$. To describe this structure in more detail, we first define a _fan_.
...
Note that the fan $F_\omega$ contains the parallelogram $P_\omega$. Writing each $L_S(t)$ as $Q^+_S(t)\setminus Q^-_S(t)$, the monotonization $\mathcal{M}(S)$ of a hallway intersection $S$ is essentially the intersection of $Q^+_S(t)$ subtracted by the unions of $Q_S^-(t)$, masked by $P_\omega$.
$$
\begin{align}
\mathcal{M}(S) = P_\omega \cap \bigcap_{t \in \Theta} L_S(t) &  =
