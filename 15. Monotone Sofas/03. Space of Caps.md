...
The space $\mathcal{K}_\omega$ a convex space, making the optimization of $\mathcal{A} : \mathcal{K}_\omega \to \mathbb{R}$ easier. Here, a convex space is a set $X$ equipped with barycentric operator $c_\lambda : X \times X \to X$ for all $\lambda \in [0, 1]$ satisfying the identities of convex-linear combinations.
