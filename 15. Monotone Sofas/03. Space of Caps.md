...
Next, we relate any cap in $K_\Theta$ to its boundary measure. We establish a correspondence between caps and its boundary measures.
...
_Proof._ $\beta_K$ has support in $\Theta \cup (\Theta + \pi/2) \cup \{\pi + \omega, 3\pi/2\}$ by [[02. Monotone Sofas#^def-cap]] and [[02. Geometric Results#^thm-boundary-measure-support]]. We have $\left( A^+_K(\omega) - A^-_K(0) \right) \cdot v_0 = 1$ for any cap $K$, and by applying [[02. Geometric Results#^thm-boundary-measure]] to the formula, we obtain the first equation. The second equation can be obtained by a symmetric argument. □
...
_Proof._ (Sketch) Use [[02. Geometric Results#^thm-supported-gauss-measure]] to construct such a cap $K$ and show the uniqueness. □
