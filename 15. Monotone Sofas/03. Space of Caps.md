...
From now on, we will define and prove the geometric properties of caps in $K_\Theta$.

First observe that the space $K_\omega$ behaves a bit differently when $\omega=\pi/2$ or $\omega < \pi/2$.

> __Theorem [cap-contains-vertex].__ If $\omega < \pi/2$, then any $K \in \mathcal{K}_\omega$ contains the vertex $p_{\omega}$ of $P_{\omega}$. If $\omega = \pi/2$, then any $K \in \mathcal{K}_\omega$ translated horizontally is also contained in $\mathcal{K}_\omega$. That is, $K_{\pi/2}$ is closed under horizontal translation. ^thm-cap-contains-vertex
