...
> __Definition [cap-space].__ For any angle set $\Theta$, let $\mathcal{K}_\Theta$ be the set of caps $K$ with angle set $\Theta$. For any rotation angle $\omega \in [0, \pi/2]$, let $\mathcal{K}_\omega = \mathcal{K}_{[0, \omega]}$. ^def-cap-space
