...
In this way, we maximize the area functional $\mathcal{A} : \mathcal{K}_\omega \to \mathbb{R}$ to find the sofa of maximum area of a sofa that rotates counterclockwise by an angle of $\omega$. This will be a recurring strategy. We will take the space $\mathcal{S}$ of all monotone sofas with area at least that of Gerver's sofa, and 

This gives an advantage as $\mathcal{K}_\omega$ a convex space, making the optimization of $|S| = |K| - |\mathcal{N}(K)|$ ([[02. Monotone Sofas#^thm-niche-in-cap]]) much easier.

> __Theorem [cap-mixin].__ For any caps $K_1$ and $K_2$ with same angle set $\Theta$, and any $\lambda \in [0, 1]$, the convex combination $K = (1-\lambda)K_1 + \lambda K_2$ is also a cap with angle set $\Theta$. That is, the space $\mathcal{K}_\Theta$ is a convex space. ^thm-cap-mixin
...
_Proof._ This is a combination of [[02. Geometric Results#^thm-convex-body-linear]] and the definitions of each object. □



We can identify a cap with its support function.

> __Theorem [cap-support-function].__ The map $K \mapsto p_K|_{\Theta \cup (\Theta + \pi/2)}$ from $\mathcal{K}_\Theta$ to the space of continuous functions $\mathcal{C}^0(\Theta \cup (\Theta + \pi/2))$ on  $\Theta \cup (\Theta + \pi/2)$ is injective and convex-linear. ^thm-cap-support-function

Note however that not all cap $K \in \mathcal{K}_\Theta$ gives rise to a hallway intersection $K \setminus \mathcal{N}(K)$, as $K \setminus \mathcal{N}(K)$ might be disconnected if $K$ is too wide. Indeed, [[02. Monotone Sofas#^thm-monotonization-connected-iff]] specifies exactly when $K$ gives rise to a sofa. So the space of monotone sofas can be understood as a subset of the space $\mathcal{K}_\omega$ of caps. With this, maximizing $\mathcal{A} : \mathcal{K}_\omega \to \mathbb{R}$ technically may not lead to a valid $K$ which gives a sofa $K \setminus \mathcal{N}(K)$.
