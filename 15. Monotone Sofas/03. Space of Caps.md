...
In this way, we maximize the area functional $\mathcal{A} : \mathcal{K}_\omega \to \mathbb{R}$ to find the maximum area of a sofa that rotates counterclockwise by an angle of $\omega$. This gives an advantage as $\mathcal{K}_\omega$ a convex space, making the optimization of $|S| = |K| - |\mathcal{N}(K)|$  much easier. A convex 
