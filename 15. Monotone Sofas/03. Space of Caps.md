...
Note however that not all cap $K \in \mathcal{K}_\Theta$ gives rise to a hallway intersection $K \setminus \mathcal{N}(K)$, as $K \setminus \mathcal{N}(K)$ might be disconnected if $K$ is too wide. Indeed, [[#^thm-monotonization-connected-iff]] specifies exactly when $K$ gives rise to a sofa. So the space of monotone sofas can be understood as a subset of the space $\mathcal{K}_\omega$ of caps.
