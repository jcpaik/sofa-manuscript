...
Moreover, we can identify a cap with its support function.
...
> __Theorem [cap-support-function].__ The map $K \mapsto p_K^0$ from $\mathcal{K}_\Theta$ to the space of continuous functions $\mathcal{C}^0(\Theta \cup (\Theta + \pi/2))$ is injective and convex-linear. ^thm-cap-support-function
