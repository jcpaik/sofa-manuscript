...
In this way, we maximize the area functional $\mathcal{A} : \mathcal{K}_\omega \to \mathbb{R}$ to find the maximum sofa that rotates counterclockwise by an angle of $\omega$. Note however that not all cap $K \in \mathcal{K}_\Theta$ gives rise to a hallway intersection $K \setminus \mathcal{N}(K)$, as $K \setminus \mathcal{N}(K)$ might be disconnected if $K$ is too wide. Indeed, [[02. Monotone Sofas#^thm-monotonization-connected-iff]] specifies exactly when $K$ gives rise to a sofa.

The space $\mathcal{K}_\omega$ a convex space, and this makes the optimization of $\mathcal{A} : \mathcal{K}_\omega \to \mathbb{R}$ easier. Here, a convex space is a set $X$ equipped with barycentric operator $c_\lambda : X \times X \to X$ for all $\lambda \in [0, 1]$ satisfying the identities of convex-linear combinations.
