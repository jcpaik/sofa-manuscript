We have shown that any maximum sofa $S$ rotating by an angle of $\omega \in [0, \pi/2]$ can be assumed to be monotone with cap $K$ and angle set $[0, \omega]$ ([[02. Monotone Sofas#^cor-monotonization-contains]]). That is, $S = K \setminus \mathcal{N}(K)$ ([[02. Monotone Sofas#^thm-monotonization-is-hallway-intersection]]). Note that $S$ is completely determined by the cap $K$. We will understand a maximum sofa $S$ with its cap $K$. This gives an advantage as the space $\mathcal{K}_\Theta$ of all caps with angle set $\Theta$ forms a convex space, making the optimization of $|S| = |K| - |\mathcal{N}(K)|$ ([[02. Monotone Sofas#^thm-niche-in-cap]]) much easier.
...
Note however that not all cap $K \in \mathcal{K}_\Theta$ gives rise to a hallway intersection $K \setminus \mathcal{N}(K)$, as $K \setminus \mathcal{N}(K)$ might be disconnected if $K$ is too wide.  In this way, the set of monotone hallway intersections can be considered as a closed subset of the cap space $\mathcal{K}_\Theta$.
 ...
 That is, the maximum sofa $S$ is the cap $K$ subtracted by the niche $\mathcal{N}(K)$. Thus, the study of maximum sofas can be reduced to the study on caps $K$ and its monotonization. We will now think in terms of the space of caps.
