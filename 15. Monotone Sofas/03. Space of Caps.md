...
_Proof._ By [[02. Monotone Sofas#^thm-niche-in-cap]]. □

In this way, we maximize the area functional $\mathcal{A} : \mathcal{K}_\omega \to \mathbb{R}$ to find the maximum sofa that rotates counterclockwise by an angle of $\omega$. This gives an advantage as $\mathcal{K}_\omega$ a convex space. Here, a convex space is a set $X$ equipped with barycentric operator $c_\lambda : X \times X \to X$ for all $\lambda \in [0, 1]$ satisfying the identities of convex-linear combinations.
