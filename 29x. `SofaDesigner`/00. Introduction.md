...
The difficulty with the niche $\mathcal{N}(K)$ is that the order $\mathbf{i}_K$ of the lines appearing in the niche can change drastically depending on the cap $K$. We need extra restrictions on the coordinates $\mathbf{p}$ of $K$ to ensure a niche with fixed order $\mathbf{i}_K$ of lines. To this end, `SofaDesigner` builds a custom branch-and-bound tree $\mathcal{T}$.

The tree $\mathcal{T}$ is a complete binary tree representing the subset of $\mathcal{K}_\Theta$ with $\mathcal{A} \geq 2.2195$. Each node $N_i$ of the tree $\mathcal{T}$ represents a subset of $\mathcal{K}_\Theta$ constrained by a finite list $I_i$ of linear inequalities of form $f_1 \geq 0, \dots, f_{m_i} \geq 0$ where each $f_j$ is an affine-linear combination of the coordinates $\mathbf{p}$. The root node $N_1$ of the tree $\mathcal{T}$ represents the whole $\mathcal{K}_\Theta$ with the finite list $s_0 \geq 0, s_1 \geq 0, \dots, s_{2n} \geq 0$. Every non-leaf node $N_i$ is split into two child nodes $N_j$ and $N_k$ by an affine-linear combination $g_i$ of the coordinates $\mathbf{p}$ is positive or negative. So, the constraints $I_j$ of node $N_j$ is $I_j = I_i, g_i \geq 0$ (the list $I_i$ with inequality $g_i \geq 0$ appended), and the constraints $I_k$ of node $N_k$ is $I_k = I_i, -g_i \geq 0$ (the list $I_i$ with inequality $-g_i \geq 0$ appended).
