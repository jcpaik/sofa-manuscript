...
_Proof._ We use the [[x1. Measure Theory#^def-function-measure-multiplication|multiplication of a function and a measure]] and that $p_K$ is [[x3. Convex Bodies#^thm-support-function-lipschitz|bounded]] to construct $p_K(t) \cdot dt$. We use that $\partial^+p_K(t)$ is both [[x3. Convex Bodies#^cor-right-differentiation-right-continuous|right-continuous]] and [[x3. Convex Bodies#^cor-right-differentiation-locally-bounded-variation|locally bounded-variation]] to construct the [[x1. Measure Theory#^def-lebesgue-stieltjes-measure|Lebesgue-Stieltjes measure]]. □
...
The first equality ues [[#^def-boundary-measure]]. The second and third equality uses [[x1. Measure Theory#^cor-product-rule-differential]]. The rest uses linearity and definition. □
...
Note that the integral cannot be defined as a Riemann-Stieltjes integral as $v_K^+(t)$ shares the same discontinuity as itself. However, as we are adopting the Lebesgue-Stieltjes integral for the definition, the Lebesgue-Stieltjes measure $d v_K^+(t)$ is well-defined from [[x3. Convex Bodies#^thm-vertex-bounded-variation]]  and [[x3. Convex Bodies#^thm-vertex-right-continuous]].

_Proof._  □

# Support

> __Theorem [boundary-measure-support].__ The support of any convex body $K$ ([[x3. Convex Bodies#^def-convex-body-support]]) is the support of the measure $\beta_K$. ^thm-boundary-measure-support
