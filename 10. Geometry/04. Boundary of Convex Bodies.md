...
> - and $t_{K, t_0} : [0, B_K] \to [t_0, t_0 + 2 \pi]$ as $t_{K, t_0}(s) = \inf \left\{ t \geq t_0 : \beta\left( (t_0, t] \right) \geq s \right\}$.
...
> __Theorem [parametrization-vertex].__ $\mathbf{x}_{K, t_0}(s_{K, t_0}(t)) = v_{K}^+(t)$ for all $t \in [t_0, t_0 + 2\pi]$ and $\mathbf{x}_{K, t_0}(s_{K, t_0}(t-)) = v_{K}^-(t)$ for all $t \in (t_0, t_0 + 2\pi]$. ^thm-parametrization-vertex
...
$$
\int_{(t_0, t_1]} u_t \, \beta(dt) = \int_{t_{K, t_0}^{-1}(t_0, t_1]} u_{t_{K, t_0} (s')} \, ds
$$

$\beta(dt)$ is the pullback of $s_{K, t_0}(t) = \beta_K\left( (t_0, t] \right)$ ? 
Pushforward measure. The $\beta_K$ measure on $t$ is the pushforward of $ds$ w.r.t. $t_{K, t_0}(s) = \inf \left\{ t \in \mathbb{R} : \beta\left( (a, t] \right) \geq s \right\}$.

$\beta_K([t_0, t_1]) = |t_{K, t_0}^{-1}([t_0, t_1])|$ ?

$$
t_{K, t_0}^{-1}([t_0, t_1]) = \left\{ s : \inf \left\{ t \in \mathbb{R} : \beta\left( (t_0, t] \right) \geq s \right\} \in [t_0, t_1] \right\} 
$$
$$
\left\{ s : \inf \left\{ t \in \mathbb{R} : \beta\left( (t_0, t] \right) \geq s \right\} \leq t_1 \right\} 
$$

$$
\left\{ s :  \beta\left( (t_0, t_1] \right) \geq s \right\} 
$$
