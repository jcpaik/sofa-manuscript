...
> __Definition [convex-body-supported].__ A convex body $K$ is _supported by_ a closed set $\Gamma \subseteq S^1$ if $K$ is an intersection of half-planes with normal angles in $\Gamma$. ^def-convex-body-supported
