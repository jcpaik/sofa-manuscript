...
Assuming the [[01. Injectivity Hypothesis#^thm-injectivity|injectivity hypothesis]], $\mathcal{A}_1(K)$ is an upper bound of the area functional $\mathcal{A}(K)$.

> __Theorem [a1-is-upper-bound].__ For any maximum monotone sofa $S$ with cap $K \in \mathcal{K}_{\omega}$ satisfying the conditions of the [[01. Injectivity Hypothesis#^thm-injectivity|injectivity hypothesis]], we have $|S| \leq \mathcal{A}_1(K)$. That is, $\mathcal{A}_1(K)$ is an upper bound of the area of sofa $S$. ^thm-a1-is-upper-bound
...
First observe that the line segments $s_L$ and $s_R$ are contained in the bottom sides $l(\omega) \cap P_\omega$ and $l(\pi/2) \cap P_\omega$ of $P_\omega$ respectively. Each path is injective, and so the joined path $\Gamma$ formed by joining all the three paths is a Jordan curve by [[00. Injectivity Hypothesis#^thm-injectivity]]. So by [[03. Geometric Results#^thm-jordan-curve]], $\Gamma$ divides the plane into an unbounded region and a bounded region. Let $\mathcal{N}_0$ be the bounded open region. 
