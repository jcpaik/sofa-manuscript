In this section, we rigorously define what is a sofa and state the moving sofa problem formally. 
...
We will only consider sofas of rotation angle $\omega \in [\pi/4, \pi/2]$. Indeed, Gerver has shown that a sofa of maximum area have rotation angle in (some interval) and this has been reduced down by Romik-Kallus to (some other interval). Here we provide a short proof that we can assume $\omega \in [\pi/4, \pi/2]$.
