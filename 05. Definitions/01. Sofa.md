...
> __Figure [sofa-in-movement].__ A sofa $S$ in movement. The initial position $f_a(S)$ colored red is in $L_H$, and the final position $f_b(S)$ colored blue is in $L_V$. The sofa in movement $f_t(S)$ is colored purple (with traces in grey lines) and is in the hallway $L$. ^fig-sofa-in-movement
> 
