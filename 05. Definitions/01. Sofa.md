...
We quickly run out of symbols as we use a lot of definitions. In this paper, we circumvent this problem by overriding a single letter by its subscript. For example, the symbols $u$, $u_t$ and $u_{K, t}$ may denote something drastically different. If $u_t$ is a value depending on an mathematical object $u$ (e.g. entries of a matrix), then we explicitly define so before assuming so.
