We define basic geometric notions that will be throughly used in the rest of the document. We use the convention $S^1 \sim \mathbb{R} / 2 \pi \mathbb{Z}$. For any function $f$ on $S^1$ and any $t \in \mathbb{R}$, we define $f(t)$ as the value $f(t + 2 \pi \mathbb{Z})$. That is, a real value is coerced to a value in $S^1$ when used as an argument of a function that takes a value in $S^1$.  
...
> 
> Write $I(p, q)$ for the area functional of the line segment $\mathbf{x}$ connecting point $p$ to $q$. We have $I(\mathbf{x}) = 1/2 \cdot (p \times q)$ where the cross product $(a, b) \times (c, d)$ of two vectors is defined as $ad-b c$. 