...
$$
> Note that $\mathbf{x}(t)$ is rectifiable so is of bounded variation. So the Lebesgue-Stieltjes measure $d \mathbf{x}(t)$ exists and the integral is well-defined. If $\mathbf{x}$ is a line segment from point $p$ to $q$, then we have $I(\mathbf{x}) = 1/2 \cdot (p \times q)$ where the cross product $(a, b) \times (c, d)$ of two vectors is defined as $ad-b c$.