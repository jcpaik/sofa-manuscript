...
The support function $p_K$ is continuous. Furthermoe, it is also Lipschitz.
...
Next, we are going to show that $v_K^+(t)$ is right-continuous with respect to $t$ and that $p_K(t)$ is both left and right-differentiable. This is to  The following lemma is the hard/technical part for computing limits.
...
$$
> As a function of $\theta$, $\alpha_K(t, \theta)$ is continuous and monotonically increasing on the set $(-\pi, \pi) \setminus \left\{ 0 \right\}$. 
