...
> __Corollary [boundary-segment-parametrization].__ Let $K$ be any convex body. For any $t_0, t_1 \in \mathbb{R}$ such that $t_1 \in (t_0, t_0 + 2 \pi]$, the cueve $\mathbf{x}_{K, t_0}(s)$ on the interval $s \in [0, s_{K, t_0}(t_1)]$ is the arc-length parametrization from $v_K^+(t_0)$ to $v_K^+(t_1)$ of the set $\left\{ v_K^+(t_0) \right\} \cup \bigcup_{t \in (t_0, t_1]} e_K(t)$. ^cor-boundary-segment-parametrization
