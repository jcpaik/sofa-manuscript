...
Any Jordan curve is either positively or negatively oriented. Although 

> __Lemma [orientation].__ Let $p$ and $q$ be two different points of $\mathbb{R}^2$. Define the closed half-planes $H_0$ and $H_1$ as the half-planes divided by the line connecting $p$ and $q$, so that for any point $x$ in the interior of $H_0$ (resp. $H_1$) the points $x, p, q$ are in clockwise (resp. counterclockwise) order. If a Jordan curve $J$ consists of two arcs, one from $p$ to $q$ inside $H_0$ and another from $q$ to $p$ inside $H_1$, then $J$ is positively oriented. ^lem-orientation
