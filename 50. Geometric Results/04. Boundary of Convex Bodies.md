...
_Proof._ (sketch) By [[04. Boundary of Convex Bodies#^cor-boundary-segment-parametrization]], the curve $\mathbf{x}_{K, 0, 2 \pi}$ is an arc-length parametrization of $\partial K$. We first show that $\mathbf{x}_{K, 0, 2 \pi}$ is a Jordan curve, and that it is positively oriented. A tedious but rudimetary check using [[04. Boundary of Convex Bodies#^def-st-conversion]], [[04. Boundary of Convex Bodies#^def-parametrization-formal]] and [[04. Boundary of Convex Bodies#^cor-boundary-segment-parametrization]] shows that the curve $\mathbf{x}_{K, 0, 2\pi}$ is the join of two curves $\mathbf{x}_{K, 0, \pi}$ and $\mathbf{x}_{K, \pi, 2\pi}$ connecting $p = v_{K}^+(0)$ and $q = v_K^+(\pi)$ and vice versa. As $K$ has nonempty interior, the $x$-coordinate of $p$ is strictly greater than that of $q$.

We first show that the curve $\mathbf{x}_{K, 0, \pi}$ is a Jordan arc from $p$ to $q$. The curve $\mathbf{x}_{K, 0, \pi}$ is the join of the curve $\mathbf{x}_{K, 0, \pi-}$ and $e_{K}(\pi)$ by [[04. Boundary of Convex Bodies#^cor-boundary-segment-parametrization]]. Also, the derivative $\mathbf{x}_{K, 0, \pi-}'(s')$ for an arbitary $s'$ is always a unit vector $v_{t'}$ where $t' \in (0, \pi)$ so the curve $\mathbf{x}_{K, 0, \pi-}$ is strictly monotonically decrasing in the $x$-coordinates. This with the fact that $e_K(\pi)$ is a vertical line segment shows that the curve $\mathbf{x}_{K, 0, \pi}$ is injective and thus a Jordan arc. A similar argument shows that $\mathbf{x}_{K, \pi, 2\pi}$ is also a Jordan arc. 

Define the closed half-planes $H_0$ and $H_1$ as the half-planes divided by the line connecting $p$ and $q$, so that for any point $x$ in the interior of $H_0$ (resp. $H_1$) the points $x, p, q$ are in clockwise (resp. counterclockwise) order.

We first observe that $\mathbf{x}_{K, 0, \pi}$ is in $H_0$: as the image of $\mathbf{x}_{K, 0, \pi}$ is the point $p$ and the union of edges $e_K(t)$ for some $t \in (0, \pi]$ by [[04. Boundary of Convex Bodies#^cor-boundary-segment-parametrization]], and as $p, q \in K$ and $t \in (0, \pi]$ the edges $e_K(t)$ should lie in $H_0$. A similar argument shows that $\mathbf{x}_{K, \pi, 2\pi}$ is in $H_1$.

Moreover, if $\mathbf{x}_{K, 0, \pi}$ ever contains a point $r$ in the interior of $H_0$, then $\mathbf{x}_{K, 0, \pi}$ connects $p$ and $q$ only using the points in the interior of $H_0$: as $K$ is convex, $K$ contains the triangle $pqr$, and so the edges $e_K(t)$ . So $\mathbf{x}_{K, 0, \pi}$ is either inside the line $pq$ or disjoint from the line $pq$ except for the endpoints $p$ and $q$. The same argument can be carried to $\mathbf{x}_{K, \pi, 2\pi}$. If both curves $\mathbf{x}_{K, 0, \pi}$ and $\mathbf{x}_{K, \pi, 2\pi}$ are in the line $pq$, then it contradicts the fact that $K$ has nonempty interior. So either one of the curves should lie strictly inside the interior of either $H_0$ or $H_1$, and the Jordan arcs $\mathbf{x}_{K, 0, \pi}$ and $\mathbf{x}_{K, \pi, 2\pi}$ overlaps only at the endpoints. Therefore the curve $\mathbf{x}_{K, 0, 2\pi}$ is a Jordan curve. That 

By [[04. Boundary of Convex Bodies#^thm-boundary-is-union-all-edges]], there is some $t \in S^1$ such that $p \in e_K(t)$. By [[04. Boundary of Convex Bodies#^thm-boundary-is-union-all-edges]] and [[04. Boundary of Convex Bodies#^cor-boundary-segment-parametrization]], the curve $\mathbf{x}_{t, t + 2 \pi} : [0, B_K] \to \mathbb{R}$ is an arc-length parametrization of $\partial K$, and $\mathbf{x}_{t, t + 2 \pi}$ restricted to $I = [s_{K, t_0}((t + 2\pi)-), B_K]$ parametrizes the line segement $e_K(t)$ by [[04. Boundary of Convex Bodies#^thm-parametrization-vertex]]. Find a value $t_p \in I$ and 

By modifying, we can construct some variant. 
- Why is it Jordan?
- Why is it positively oriented? 

State some lemma. Say we won't explicitly define what is a winding number, but it can be rigorified. Say that something is a Jordan curve. Point p and q. Line from q to p, and line from p to q. 

Why is it in half-line? Use derivative definition of curve. p and q -> orthogonal



□

State a corollary that $\mathbf{x}_{K, p, q}$ is that v thing modified on both ends.
