This section proves necessary tools for _convex bodies_ in a plane $\mathbb{R}^2$. Recall that a _convex body_ $K$ is a nonempty compact subset of $\mathbb{R}^2$ which is an intersection of half-planes ([[02. Notations, Conventions and Results#^def-half-plane]]). We defined its [[02. Notations, Conventions and Results#^def-support-function|support function]] $p_K$, [[02. Notations, Conventions and Results#^def-tangent-line|tangent lines]] $l_K(t)$, [[02. Notations, Conventions and Results#^def-half-plane|half-planes]] $H_K(t)$, vertices $v_K^\pm (t)$, and edges $e_K(t)$ ([[02. Notations, Conventions and Results#^def-convex-body-edge]]).
...
> __Lemma [limits-converging-to-vertex].__ Let $K$ be a convex body and $t$ be an arbitrary angle. We have the following limits all converging to $v_K^+(t)$. In particular, the vertex $v_K^+(t)$ is a right-continuous function on $t \in S^1$. ^lem-limits-converging-to-vertex
$$
...
The last limit of [[#^lem-intersection-tangent-lines]] and [[#^lem-intersection-tangent-lines]] shows that we have the following.
$$
\begin{align*}
