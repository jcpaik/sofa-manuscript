...
In Section 2, we refine the idea that a maximum sofa $S$ is the intersection of hallways $L_t$ rotated counterclockwise by an angle of $t$. By pushing each rotating hallway $L_t$ towards $S$ as much as possible, we show that a maximum area sofa can be assumed to be a _monotone sofa_ where each $L_t$ always touches the sofa $S$ on the outer walls (see [[#^fig-monotone-sofa]]). A monotone sofa $S$ is made up of a cap $K$ subtracted by the niche $N$. The cap $K$ is a convex set with the outer walls of the hallway as tangent lines. The niche $N$ is a union of the triangular wedges carved out by inner corners.
