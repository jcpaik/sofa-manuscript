...
> __Theorem [convex-linear].__ Support function $p_K$, boundary measure $\beta_K$, arm length $g_K$ and $h_K$, inner and outer corners $\mathbf{x}_K$ and $\mathbf{y}_K$, its derivative $\mathbf{v}_K$ are all convex-linear with respect to $K \in \mathcal{K}_\omega$. ^thm-convex-linear
