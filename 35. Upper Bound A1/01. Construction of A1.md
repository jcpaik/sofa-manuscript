...
> __Definition [convex-linear].__ Let $V$ be a vector space and $f : K_\omega \to V$ be a map. If  ^def-convex-linear

> __Theorem [convex-linear].__ Support function $p_K$, boundary measure $\beta_K$, arm length $g_K$ and $h_K$, inner and outer corners $\mathbf{x}_K$ and $y_K$, its derivative $v_K$ are all convex-linear with respect to $K \in K_\omega$. ^thm-convex-linear
