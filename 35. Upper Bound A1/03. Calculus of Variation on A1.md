...
Now we calculate for which $K_1$ is the value in [[#^thm-variation-a1]] equal to zero. To do this, we solve for $\beta_K(dt) = i_K(t)dt$ on the interior of $J_\omega$ assuming that $i_K$ is differentiable on the interior of $J_\omega$. Note that here we allow the measure $\beta_K$ to have nonzero point mass concentrated on the values $u = \beta, \pi/2$. Computing the derivatives, we get the following using Leibniz integral rule. Here $0 < t < \omega$ is taken arbitrarily.
$$
\begin{align}
