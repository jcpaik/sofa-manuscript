...
> __Definition [maximum-presofa-a1].__ For any rotation angle $\omega \in [0, \pi/2]$, define the cap $K_1$ as the cap with boundary measure $\beta_{K_1}$ with support in $J_{\omega} \cup \left\{ \omega + \pi, 3 \pi/2 \right\}$ satisfying the following conditions. First, $\beta_{K_1}(\{\omega\}) = \beta_{K_1}(\{\pi/2\}) = 1$. Second, $\beta_{{K_1}}(dt) = (\omega -t)dt$ on $t \in [0, \omega)$ and $\beta_{K_1}(dt) = (t - \pi/2)$ on $t \in (\pi/2, \omega + \pi/2]$. Finally, the values of $\beta_{K_1}(\left\{ \omega + \pi \right\})$ and $\beta_{K_1}(\left\{ 3\pi/2 \right\})$ are chosen so that $\int_{S^1} v_t \beta_{K_1}(dt) = 0$. ^def-maximum-presofa-a1
