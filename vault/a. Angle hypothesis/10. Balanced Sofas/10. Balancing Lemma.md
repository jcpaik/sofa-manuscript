...
We now handle the case $p \in S_{\geq 2}$. For every $1 \leq j \leq n$, let $B_j$ be the infinite closed strip of width $\epsilon$ bounded by $l_i = l(t_i, h_i)$ and $l(t_i, h_i + \delta)$. For every $1 \leq i < j \leq n$, define $P_{i, j} = B_i \cap B_j$. Now take any $p \in S_2$, then   So $S_2$ is contained in the union of parallelograms. Now $|S_{\geq 2}| = O(\delta^2)$.
