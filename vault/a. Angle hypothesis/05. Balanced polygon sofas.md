- Polygon area $\mathcal{A}_\Theta$ is continuous on $\mathcal{K}_\Theta'$.
...
If we control $K$ right away, then it could be possible that its adjustment $K'$ may not be a valid cap.

Fix a finite angle set $\Theta$ with rotation angle $\omega$. Let $\mathbf{x} : \Theta \to \mathbb{R}^2$ be an arbitrary assignment of points with values $h_\omega, h_{\pi/2} \in \mathbb{R}$. Impose the constraints $h_\omega + \mathbf{x}(\omega) \cdot u_\omega \leq 1$ and $h_{\pi/2} + \mathbf{x}(0) \cdot v_0 \leq 1$ and nothing else. Define the collection $\mathcal{X}$ of such datum $(\mathbf{x}, h_\omega, h_{\pi/2})$. Define the following sets.
$$
\begin{gather*}
F := H(\omega + \pi/2, h_\omega) \cap H(3\pi/2, h_{\pi/2}) \\
K := F \cap \bigcap_{t \in \Theta} \left( R_t(Q^+) + \mathbf{x}(t) \right) \\
N := F \cap \bigcup_{t \in \Theta \setminus \left\{ 0, \omega \right\}  } \left( R_t(Q^-) + \mathbf{x}(t) \right) 
\end{gather*}
$$
The overall strategy is to take the maximizer $(\mathbf{x}, h_\omega, h_{\pi/2})$ of $|K| - |N|$ and then show that it is the maximizer of polygonal sofas. We do it this way because we don't want many restrictions on controlling $\mathbf{x}$ and $h_\omega, h_{\pi/2}$.  Here, $K$ and $N$ are not necessarily 

- $K$ is a convex polygon with normal angles in $\Pi = \Theta \cup (\Theta + \pi/2) \cup \left\{ \pi + \omega, 3\pi/2 \right\}$.
- $N$ is a region under polygonal chain with normal angles in $\Theta \cup (\Theta + \pi/2)$, intersected with $F$.
- Show that the maximizer exists.
- Show that the maximizer has positive gap length.
- Show that the maximizer has to satisfy height 1 condition.
- Show that the maximizer has to satisfy balancedness condition.
- Show that the maximizer can be translated to a cap so that it is in $\mathcal{K}_\Theta$.
...
The first step is to prove the balancedness condition for polygonal sofas ([[a. Angle hypothesis/05. Balanced polygon sofas#^thm-polygon-balanced-angled]]).
...
$$
in [[a. Angle hypothesis/05. Balanced polygon sofas#^thm-polygon-balanced-angled]], we obtain the following equality for sufficiently small $\epsilon > 0$.
$$
...
$$
in [[a. Angle hypothesis/05. Balanced polygon sofas#^lem-perturbation]], we obtain the following equality.
$$
