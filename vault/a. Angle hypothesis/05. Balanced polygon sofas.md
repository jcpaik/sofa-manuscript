> __Definition [height-space].__ Let $\Theta$ be an arbitrary finite angle set with rotation angle $\omega$ of size $\geq 3$. Define $\mathcal{H}_\Theta$ as the space of all functions $h : \Theta \cup (\Theta + \pi/2) \to \mathbb{R}$. ^def-height-space
...
K_h := F_h \cap \bigcap_{t \in \Theta} \left( R_t(Q^+) + \mathbf{x}_h(t) \right) \\
N_h := F_h \cap \bigcup_{t \in \Theta \setminus \left\{ 0, \omega \right\}  } \left( R_t(Q^-) + \mathbf{x}_h(t) \right)
\end{gather*}
$$

> __Proposition [height-cap].__ For any $h \in \mathcal{H}_\Theta$, the set $K_h$ is ^pro-height-cap
$$
K_h = 
$$
> and is either empty or a convex polygon with normal angles in $\Theta^*$.

> __Proposition [height-niche-polyline].__ For any $h \in \mathcal{H}_\Theta$, the set $F_h \setminus N_h$ is a polyline. ^pro-height-niche-polyline

> __Definition [height-area].__ Define $\mathcal{A}(h) = |K_h| - |N_h|$ for $h \in \mathcal{H}_\Theta$. ^def-height-area
...
Balancedness criteria

> __Definition [height-edges].__ For any $h \in \mathcal{H}_\Theta$, define the following values. ^def-height-edges
> 
> - $a_h(t) = l(t, h(t)))$ $b_h$, $c_h$, $d_h$
> - Contribution of such lines.
> - $w_h$ is the minimum value of ...
> - $z_h$ is the minimum value of ...
