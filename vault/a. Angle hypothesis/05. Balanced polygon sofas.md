...
(Motivate the local adjustment argument)
...
\end{gather*}
$$
The overall strategy is to take the maximizer $(\mathbf{x}, h_\omega, h_{\pi/2})$ of $|K| - |N|$ and then show that it is the maximizer of polygonal sofas. We do it this way because we don't want many restrictions on controlling $\mathbf{x}$ and $h_\omega, h_{\pi/2}$. If we control $K$ right away, then it could be possible that its adjustment $K'$ may not be a valid cap. Here, $K$ and $N$ are not necessarily 
...
- Show that the maximizer can be assumed to 
