In this section, we set up basic notations, definitions and conventions that will be used thoroughout the rest of the document. We also gather the notions that are defined later for easier reference. This section is not meant to be read in one setting. Instead, the reader can start by reading only the definitions related to moving sofas, shapes, and convex bodies and move on. Then they can later refer to parts of this section as needed. Definitions not listed in this section will be referenced before it is used in a proof.
...
For reference, the notion of cap $K$ as a type of convex bodies will be defined in .

The space of all caps $\mathcal{K}_\omega$ with rotation angle $\omega$ will be defined in . $\mathcal{A} : \mathcal{K}_\omega \to \mathbb{R}$ is the _sofa area functional_ that will be defined in . $\mathcal{A}_1 : \mathcal{K}_\omega \to \mathbb{R}$ is a conditional upper bound of $\mathcal{A}$ that is defined in .

A continuous curve $\mathbf{x} : [a, b] \to \mathbb{R}^2$; see 

For reference, The notion of _convex space_ For any quadratic functional 
