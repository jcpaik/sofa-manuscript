In this section, we set up basic notations, definitions and conventions that will be used thoroughout the rest of the document. This section is not meant to be read in one setting; instead, the reader can refer to parts of this section if needed. Definitions not listed in this section will always be referred before it is used for the first time in a proof.
...
With the result of [[@kallusImprovedUpperBounds2018]] that $\omega \in [81.203\dots^\circ, 90^\circ]$ for a maximum-area moving sofa, we will always assume that a moving sofa have rotation angle $\omega \in (0, \pi/2]$. The notion of _standard position_ is defined in [[15. Monotone sofas/07. Tangent Hallway#^def-standard-position]]. The notion of _monotonization_ $\mathcal{M}(S)$, _cap_ $\mathcal{C}(S)$, and _niche_ $\mathcal{N}(K)$ are defined in , , and respectively.
