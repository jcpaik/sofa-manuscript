In this section, we set up basic notations, definitions and conventions that will be used thoroughout the rest of the document. This section is not meant to be read in one setting; instead, the reader can start by reading the definitions related to moving sofas, shapes, and convex bodies, and then later refer to parts of this section as needed. Definitions not listed in this section will always be referred before it is first used in a proof.
...
With the result of [[@kallusImprovedUpperBounds2018]] that $\omega \in [81.203\dots^\circ, 90^\circ]$ for a maximum-area moving sofa, we will always assume that a moving sofa have rotation angle $\omega \in (0, \pi/2]$. For each rotation angle $\omega$, we define the following polygon for future use.
...
For reference, the _cap_ of . The space of all caps $\mathcal{K}_\omega$ with rotation angle $\omega$ will be defined in . $\mathcal{A} : \mathcal{K}_\omega \to \mathbb{R}$ is the _sofa area functional_ that will be defined in . $\mathcal{A}_1 : \mathcal{K}_\omega \to \mathbb{R}$ is a conditional upper bound of $\mathcal{A}$ that is defined in .
