...
Refer to [[@steinRealAnalysisMeasure2005]] for the standard notion of bounded variation and absolute continuity of a single-variable, real-valued function. Remark that a continuous curve $\mathbf{x} : [a, b] \to \mathbb{R}^2$ is rectifiable if and only if its $x$ and $y$ coordinates are both of bounded variation.
