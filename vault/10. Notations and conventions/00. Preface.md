...
For reference, the notion of cap $K$ as a kind of convex bodies will be defined in [[15. Monotone sofas/12. Structure of a monotone sofa#^def-cap]]. The space of all caps $\mathcal{K}_\omega$ with rotation angle $\omega$ will be defined in [[20. Sofa area functional/00. Preface#^def-cap-space]]. The vertices $A_K^{\pm}(t)$, $C_K^{\pm}(t)$ of a cap $K$ is defined in [[15. Monotone sofas/15. Cap contains niche#^def-cap-vertices]]. The upper boundary $\delta K$ of a cap $K$ is defined in [[15. Monotone sofas/15. Cap contains niche#^def-upper-boundary-of-cap]].

# Area functionals

For reference, the function $\mathcal{A}(K)$ is the _sofa area functional_ on cap $K$ defined in [[20. Sofa area functional/00. Preface#^def-sofa-area-functional]]. $\mathcal{A}_1(K)$ is a conditional upper bound of $\mathcal{A}(K)$ that is defined in [[25. Upper bound A1/01. Definition of A1#^def-a1]].

Refer to [@steinRealAnalysisMeasure2005] for the standard notion of bounded variation and absolute continuity of a single-variable, real-valued function. Recall that a continuous curve with parametrization $\mathbf{x} : [a, b] \to \mathbb{R}^2$ is rectifiable if and only if its $x$ and $y$ coordinates are both of bounded variation (Theorem 3.1 of [@steinRealAnalysisMeasure2005]). We now define the _curve area functional_ $\mathcal{I}(\mathbf{x})$ of $\mathbf{x}$.
...
The integral in [[10. Notations and conventions/00. Preface#^def-curve-area-functional]] is the Riemann-Stieltjes integral, for which we again refer to [@steinRealAnalysisMeasure2005]. Note that the value of $\mathcal{I}(\mathbf{x})$ is the same even if we replace $\mathbf{x}$ with a reparametrization of $\Gamma$. In particular, for any parametrization $\mathbf{x}$ of the line segment from point $p$ to $q$, the value $\mathcal{I}(\mathbf{x})$ is equal to 

> __Definition [curve-area-functional-segment].__ Write $\mathcal{I}(p, q)$ for the area functional of the line segment connecting point $p$ to $q$, so that we have $\mathcal{I}(p, q) = 1/2 \cdot (p \times q)$. ^def-curve-area-functional-segment
