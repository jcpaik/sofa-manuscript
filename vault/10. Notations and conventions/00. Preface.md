In this section, we set up basic notations, definitions and conventions that will be used thoroughout the rest of the document. This section is not meant to be read in one setting; instead, the reader can refer to parts of this section if needed. Definitions not listed in this section will always be referred before its first usage in a proof.
...
The notion of _standard position_ is defined in [[15. Monotone sofas/07. Tangent Hallway#^def-standard-position]]. The notion of _monotonization_ $\mathcal{M}(S)$, _cap_ $\mathcal{C}(S)$, and _niche_ $\mathcal{N}(K)$ will be defined in [[15. Monotone sofas/10. Monotonization#^def-monotonization]], [[15. Monotone sofas/10. Monotonization#^def-cap-sofa]], and [[15. Monotone sofas/12. Structure of a monotone sofa#^def-niche]] respectively.
