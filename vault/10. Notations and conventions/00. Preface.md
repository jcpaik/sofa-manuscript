In this section, we set up basic notations, definitions and conventions that will be used thoroughout the rest of the document. The section is not meant to be read in one setting. Instead, the reader can refer to parts of this section if confused. The definitions not in this section will always be mentioned when used for the first time in a proof.
...
> __Definition [line].__ For any angle $t$ in $S^1$, and a value $h \in \mathbb{R}$, define the line $l(t, h)$ with the _normal angle_ $t$ and the distance $h$ from the origin as ^def-line
$$
...
In this paper only, we use the following terminologies of _vertices_ and _edges_ of a planar convex body $K$.
