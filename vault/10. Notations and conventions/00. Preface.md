In this section, we set up basic notations, definitions and conventions that will be used thoroughout the rest of the document. We also gather the notions that are defined later for easier reference.

This section is not meant to be read in one setting. Instead, the reader can start by reading only the definitions related to moving sofas, shapes, and convex bodies and move on. Then they can later refer to parts of this section as needed. Definitions not listed in this section will be referenced before it is used in a proof.
