...
> __Definition [curve-area-functiona-segment].__ Write $\mathcal{I}(p, q)$ for the area functional of the line segment connecting point $p$ to $q$, so that we have $\mathcal{I}(p, q) = 1/2 \cdot (p \times q)$. ^def-curve-area-functiona-segment
