In this section, we set up basic notations, definitions and conventions that will be used thoroughout the rest of the document. We also gather the notions that will be defined later for easier reference.
...
For reference, the notion of cap $K$ as a type of convex bodies will be defined in . The space of all caps $\mathcal{K}_\omega$ with rotation angle $\omega$ will be defined in . $\mathcal{A} : \mathcal{K}_\omega \to \mathbb{R}$ is the _sofa area functional_ that will be defined in . $\mathcal{A}_1 : \mathcal{K}_\omega \to \mathbb{R}$ is a conditional upper bound of $\mathcal{A}$ that is defined in .

A continuous curve $\mathbf{x} : [a, b] \to \mathbb{R}^2$; see 

The notion For any quadratic functional 
