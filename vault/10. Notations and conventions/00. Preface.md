In this section, we set up basic notations, definitions and conventions that will be used thoroughout the rest of the document. This section is not meant to be read in one setting; instead, the reader can refer to parts of this section as needed. Definitions not listed in this section will always be referred before its first usage in a proof.
...
> __Definition [parallelogram].__ For any $\omega \in (0, \pi/2]$, define the _parallelogram_ $P_\omega = H \cap R_\omega(V)$ with _rotation angle_ $\omega$. ^def-parallelogram
...
_Tangent hallway_ $L_S(t)$ will be defined in [[15. Monotone sofas/07. Tangent Hallway#^def-tangent-hallway]]. The parts of the tangent hallway will be defined in [[15. Monotone sofas/07. Tangent Hallway#^def-rotating-hallway-parts]].