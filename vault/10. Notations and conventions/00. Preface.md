We set up basic notations, definitions and conventions that will be used thoroughout the rest of the document. The definitions are not meant to be read in one setting. (TODO:) Instead, one can consult this page if confused. All the other definitions are referred when first used in a proof.
