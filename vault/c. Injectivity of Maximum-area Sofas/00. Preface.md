...
The _moving sofa problem_ asks for the connected shape with the largest area $\alpha_{\text{max}}$ that can move around the right-angled corner of a hallway $L$ with unit width. We show that there exists a moving sofa $S_{\max}$ of the maximum area $\alpha_{\text{max}}$, such that it satisfies the. This is a part of a bigger project. 
