...
It is known that a maximum-area moving sofa $S_{\text{max}}$ exists. [[a. Mamikon's Theorem/00. Preface]] shows that there is a maximum-area sofa which is _monotone_ and _balanced_.
...
This is related to an assumption of Romik, that the values 

> __Theorem [well-behaved].__ A limiting maximum monotone sofa $S$ with the rotation path $\mathbf{x}(t)$ satisfies $\mathbf{x}'(t) \cdot \mu_t > 0$ and $\mathbf{x}'(t) \cdot \nu_t < 0$ for all $t \in (0, \pi/2)$. ^thm-well-behaved
