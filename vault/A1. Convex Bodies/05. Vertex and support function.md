...
We will soon show that the vertex $v_K^{+}(t)$ (resp. $v_K^-(t)$) is right-continuous (resp. left-continuous) respect to $t$ ([[05. Vertex and support function#^cor-vertex-right-continuous]]), and is linear with respect to $K$ ([[05. Vertex and support function#^cor-vertex-linear]]). To do so, we compute the limit of vertices via [[05. Vertex and support function#^thm-limits-converging-to-vertex]].
