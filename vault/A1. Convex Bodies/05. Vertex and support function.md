...
> __Theorem [vertex-formula].__ Support function $p_K$ is both left and right differentiable. Moreover, for any angle $t \in S^1$ the following equalities hold. ^thm-vertex-formula
$$
...
_Proof._ We show the first equality. By letting $\theta = t' - t$ and taking the limit $t' \to t^+$ to [[05. Vertex and support function#^lem-intersection-tangent-lines]], [[05. Vertex and support function#^thm-limits-converging-to-vertex]] shows that
$$
\begin{align*}
v_K^+(t) & = p_K(t) u_{t} + \left(  \lim_{ \theta \to 0^+ } \frac{p_K(t + \theta) - p_K(t) \cos \theta}{\sin \theta} \right) v_t
\end{align*} 
$$
and the limit evaluates to
$$
\begin{align*}
& \lim_{ \theta \to 0^+ } \frac{p_K(t + \theta) - p_K(t) \cos \theta}{\sin \theta} \\
...
\end{align*}
$$
as $(1 - \cos \theta)/\theta \to 0$ and $\theta / \sin \theta \to 1$ as $\theta \to 0$. This proves the first equality. The second equality can be proved similarly. □

Now we have the following corollary of [[05. Vertex and support function#^cor-vertex-right-continuous]] and [[05. Vertex and support function#^thm-vertex-formula]].

> __Corollary [right-differentiation-right-continuous].__ The function $\partial^+ p_K(t)$ of $t \in S^1$ is right-continuous. ^cor-right-differentiation-right-continuous
...
_Proof._ Use that $v_K^+(t) = p_K(t)u_t + \partial^+p_K(t)v_t$ is of [[#^thm-vertex-bounded-variation|bounded variation]] and that $p_K$ is [[#^thm-support-function-lipschitz|Lipschitz]]. □