...
> 
> Likewise, define $\mathbf{q} : (t - \pi, t] \to \mathbb{R}^2$ as $\mathbf{q}(t') = v_K(t', t)$ for all $t' \in (t - \pi, t)$ and $\mathbf{q}(t) = v_K^-(t)$. Then the followings hold analogously. $\mathbf{q}$ is a parametrization of the half-line ending with $v_K^-(t)$ in the direction of $v_t$. As $t$ increases $\mathbf{q}(t)$ moves monotonically in the direction of $v_t$. Finally, $\mathbf{q}$ is absolutely continuous on bounded intervals. 
