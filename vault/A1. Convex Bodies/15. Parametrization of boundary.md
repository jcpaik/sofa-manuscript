...
We haven't yet proved that $\mathbf{b}_K^{t_0}$ does parametrize our boundary $\partial K$ as intended. The three technical lemmas below handle conversion from/to $s$ and $t$.
...
Now we are ready to define the segment $\mathbf{b}_K^{t_0, t_1}$.
...
We prove [[15. Parametrization of boundary#^thm-param-segment]]. That $\mathbf{b}_K^{t_0, t_1}$ is injective will be proved later.
...
_Proof._ Take any $s' \in (0, s_{K, t_0}(t_1)]$. Let $t' = t_{K, t_0}(s')$ and observe that $t' \in (t_0, t_1]$ and $s' \in t_{K, t_0}^{-1}(\left\{ s' \right\})$. Then as $\mathbf{x}_{K, t_0} (s') \in e_K(t')$ by [[#^lem-parametrization-vertex]], we have $\mathbf{x}_{K, t_0} (s') \times v_{t'} = p_K(t')$. So we have the following.
$$
\begin{align*}
