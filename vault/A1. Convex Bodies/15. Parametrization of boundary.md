...
_Proof._ That $f_K^{t_0}(0) = t_0$ is immediate from [[A1. Convex Bodies/15. Parametrization of boundary#^def-conversion-st]]. If $s > 0$, then any $t \geq t_0$ satisfying $\beta_K((t_0, t]) \geq s$ has to satisfy $t > t_0$, so we have $f_K^{t_0}(s) > t_0$. □
