...
We recall the difference betweena Jordan arc and curve (p170 of [[@apostolMathematicalAnalysisModern]]).
...
In order for $\partial K$ to be a Jordan curve, $K$ has to have nonempty interior. For the notion of the orientation of a Jordan curve, we refer to p170 of [[@apostolMathematicalAnalysisModern]].
...
For every $t_0 \in \mathbb{R}$ and $t_1 \in [t_0, t_0 + 2\pi)$ we define $\mathbf{b}_K^{t_0-, t_1}$ as the concatenation of the edge $e_K(t_0)$ from $v_K^-(t_0)$ to $v_K^+(t_1)$ and the curve $\mathbf{b}_K^{t_0, t_1}$. Then $\mathbf{b}_K^{t_0-, t_1}$ is one of: a Jordan arc, a Jordan curve, or a single point, that connects $v_K^-(t_0)$ to $v_K^+(t_1)$ along the boundary $\partial K$. The previously mentioned theorems on $\mathbf{b}_K^{t_0, t_1}$ have corresponding variants on $\mathbf{b}_K^{t_0 -, t_1}$.
...
By [[15. Parametrization of boundary#^def-parametrization-formal]] we have $\left(\mathbf{b}_{K}^{t_0}\right)'(s) = u_{f_K^{t_0}(s)}$, and by [[15. Parametrization of boundary#^pro-conversion-st-nonzero]] and [[15. Parametrization of boundary#^lem-parametrization-set-calculation]] we have $t_0 < f_K^{t_0}(s) < t_1$ for every $0 < s < g_{K}^{t_0}(t_1-)$. Thus we have the following:
