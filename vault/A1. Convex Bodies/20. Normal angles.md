...
> __Corollary [supported-gauss-measure].__ Let $\Pi$ be any closed subset of $S^1$. For any finite Borel measure $\beta$ of $S^1$ supported on the set $\Pi$ such that $\int_{\Pi} v_t\,\beta(dt) = 0$, there is a convex body $K$ with normal angles $\mathbf{n}(K)$ in $\Pi$ satisfying $\beta_K = \beta$, which is unique up to translations of $K$. ^cor-supported-gauss-measure