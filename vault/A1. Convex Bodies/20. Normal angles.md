...
> __Corollary [supported-gauss-measure].__ Let $\Pi$ be any closed subset of $S^1$. For any finite Borel measure $\beta$ on $\Pi$ such that $\int_{\Pi} v_t\,\beta(dt) = 0$, there is a convex body $K$ with normal angles $\mathbf{n}(K)$ in $\Pi$ such that $\beta_K|_{\Pi} = \beta$, which is unique up to translations of $K$. ^cor-supported-gauss-measure