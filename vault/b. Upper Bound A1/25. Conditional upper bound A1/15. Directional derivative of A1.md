In this section, we calculate the directional derivative $D\mathcal{A}_1(K; -)$ of $\mathcal{A}_1$ ([[b. Upper Bound A1/25. Conditional upper bound A1/02. Calculus of variation#^def-convex-space-directional-derivative]]) at any $K \in \mathcal{K}_{\omega}$. As $\mathcal{A}_1(K) = \left| K \right| - \mathcal{I}(\mathbf{x}_K)$, we calculate the directional derivative of $|K|$ and $\mathcal{I}(\mathbf{x}_K)$ separately.
...
_Proof._ For any convex body $K$ we have $|K| = V(K, K)$ where $V$ is the mixed volume of two planar convex bodies. So by applying [[b. Upper Bound A1/25. Conditional upper bound A1/02. Calculus of variation#^lem-derivative-calculation]] to $|K| = V(K, K)$ and using that $V(K, K') = V(K', K)$, we have the following.
$$
\left. \frac{d}{d \lambda} \right|_{\lambda=0} \left| (1-\lambda) K + \lambda K' \right| = 2V(K', K) - 2V(K, K)  
$$
By applying [[b. Upper Bound A1/A1. Convex bodies/10. Surface area measure#^thm-surface-area-measure-area]] we get the result. □
...
_Proof._ Consider the bilinear form $\mathcal{J}(\mathbf{x}_1, \mathbf{x}_2) = \int_a ^b \mathbf{x}_1(t) \times d \mathbf{x}_2(t)$ on rectifiable $\mathbf{x}_1, \mathbf{x}_2 : [a, b] \to \mathbb{R}^2$. Apply [[b. Upper Bound A1/25. Conditional upper bound A1/02. Calculus of variation#^lem-derivative-calculation]] to $2\mathcal{I}(\mathbf{x}) = \mathcal{J}(\mathbf{x}, \mathbf{x})$ to get ^eqn-variation-curve
$$
...
$$
Using the integration by parts ([[b. Upper Bound A1/A1. Convex bodies/07. Lebesgue-Stieltjes measure#^pro-lebesgue-stieltjes-product]]), we have
$$
...
$$
Plug this back in [[b. Upper Bound A1/25. Conditional upper bound A1/15. Directional derivative of A1#^eqn-variation-curve]] and rearrange to get the claimed equality in [[b. Upper Bound A1/25. Conditional upper bound A1/15. Directional derivative of A1#^thm-variation-curve]]. □
...
[[b. Upper Bound A1/25. Conditional upper bound A1/15. Directional derivative of A1#^def-i-cap]] is motivated by the following lemma.
...
_Proof._ By [[b. Upper Bound A1/25. Conditional upper bound A1/01. Definition of A1#^thm-inner-corner-deriv]] and [[b. Upper Bound A1/25. Conditional upper bound A1/01. Definition of A1#^pro-arm-length-unsigned]], the derivative of $\mathbf{x}_{K_2}(t)$ with respect to $t$ exists almost everywhere and is the following.
$$
...
$$
and by [[b. Upper Bound A1/25. Conditional upper bound A1/15. Directional derivative of A1#^def-i-cap]] this integral is equal to $\left< p_{K_1} - 1, \iota_{K_2} \right>_{I \cup (I + \pi/2)}$. □
...
_Proof._ We have $\mathcal{A}_1(K) = |K| - \mathcal{I}(\mathbf{x}_K)$. Apply [[b. Upper Bound A1/25. Conditional upper bound A1/15. Directional derivative of A1#^thm-convex-body-area-variation]] to the term $|K|$ to have the following.
$$
...
Apply [[b. Upper Bound A1/25. Conditional upper bound A1/15. Directional derivative of A1#^thm-variation-curve]] to the term $\mathcal{I}(\mathbf{x}_K)$, and use that the points $O$, $\mathbf{x}_{K}(0)$, and $A^-_K(0)$ (respectively, the points $O$, $\mathbf{x}_K(\omega)$, and $C_K^+(\omega)$) are colinear to get the following.
$$
\begin{align*}
...
\end{align*}
$$
The second equality comes from applying [[b. Upper Bound A1/25. Conditional upper bound A1/15. Directional derivative of A1#^lem-i-cap]] twice. Subtract the derivates of $|K|$ and $\mathcal{I}(\mathbf{x}_K)$ above to conclude the proof. □

We explain the intuitive meaning of [[b. Upper Bound A1/25. Conditional upper bound A1/15. Directional derivative of A1#^thm-variation-a1]] by comparing it to the local optimization argument of Theorem 2 in [@romikDifferentialEquationsExact2018]. Assume for the sake of explanation that $S$ is a monotone sofa of rotation angle $\pi/2$ with cap $K$, such that the niche $\mathcal{N}(K)$ is exactly the region bounded by the curve $\mathbf{x}_K(t)$. Assume that our cap $K$ has vertices $A_K(t) = A_K^{\pm}(t)$ and $C_K(t) = C_K^{\pm}(t)$ continuously differentiable with respect to $t$. Assume also that $\mathbf{x}_K(t)$ is continuously differentiable.
