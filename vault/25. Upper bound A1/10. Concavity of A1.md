We now show that $\mathcal{A}_1$ is concave. The key idea is to use _Mamikon's theorem_ [@mnatsakanianAnnularRingsEqual1997]. The theorem states that for the interval $t \in [a, b]$, a tangent segment of a convex body $K$ of angle $t$ from the $x$-axis and length $f(t)$ sweeps an area of $\frac{1}{2}\int_a^b f(t)\,dt$.

to express $\mathcal{A}_1$ as a negative sum-of-squares. The theorem calculates the area of the region swept by tangent segments of a convex body. For the precise statement of Mamikon's theorem on a general planar convex body $K$, we refer to [[A1. Convex bodies/25. Mamikon's theorem#^thm-mamikon]] and [[A1. Convex bodies/25. Mamikon's theorem#^thm-mamikon-closed]].
