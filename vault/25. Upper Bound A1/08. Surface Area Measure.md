...
This comes from the fact that in dimension $n=2$, the surface area measure is equal to the mixed area measure with $n-1 = 1$ argument which is (e.g. from (5.27), p284 of [[@schneider_2013]]). 

We also can represent the area and mixed volume of convex bodies using. Note that for any measurable function $f$ on a space $X$ and a measure $\beta$ on $X$, we denote the integral of $f$ with respect to $\beta$ as $\left< f, \beta \right>_{X} = \int_{x \in X} f(x)\,\beta(dx)$.

> __Theorem [surface-area-measure-area].__ The mixed volume $V(K_1, K_2)$ of any two convex bodies $K_1$ and $K_2$ can be represented as ^thm-surface-area-measure-area

We have ^eqn-mixed-area
$$
...
> __Theorem [surface-area-measure-vertex].__ For any convex body $K$, the function $v_K^+ : S^1 \to \mathbb{R}^{2}$ is right-continuous so the Lebesgue-Stieltjes measure $d v_K^+(t)$  so  is a uniquely associated _boundary measure_ $\beta_K$ on $S^1$ such that it satisfies the _vertex formulas_: For any $a < b$ such that $b \leq a + 2\pi$, ^surface-area-measure-vertex
$$
