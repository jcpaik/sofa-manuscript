...
is stated as Theorem 5.1.7 in p280 of [[@schneider_2013]].

We have ^eqn-mixed-area
$$
V(K_1, K_2) = \left< p_{K_1}, \beta_{K_2} \right>_{S^1} / 2
$$
where $V(K_1, K_2)$ is the mixed volume of arbitrary convex bodies $K_1$ and $K_2$ , so we have ^eqn-area
$$
...
Here, the function $v_K^+ : S^1 \to \mathbb{R}^{2}$ is right-continuous so the Lebesgue-Stieltjes measure $d v_K^+(t)$ . A rigorous proof of [[08. Surface Area Measure#^surface-area-measure-vertex]] is done in Appendix A.
