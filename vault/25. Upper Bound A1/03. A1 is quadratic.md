Here, we establish that $\mathcal{A}_1 : \mathcal{K}_\omega \to \mathbb{R}$ is quadratic.

# Surface area measure

A key component in our analysis of $\mathcal{A}_1$ is the _surface area measure_ $\beta_K$ of a convex body $K$. $\beta_K$ is a measure on $S^1$ that essentially measures the side lengths of $K$. For example, if $K$ is a convex polygon, then $\beta_K$ is a discrete measure such that the measure $\beta_K\left( \left\{ t \right\} \right)$ at point $t$ is the length of the edge $e_K(t)$. On the other hand, assume that for every $t \in S^1$ the tangent line $l_K(t)$ meets $K$ at a single point $v(t)$, so that $\partial K$ is parametrized smoothly by $v_K(t)$ for $t \in S^1$. Then it turns out that $\beta_K(dt) = R(t) dt$ where $R(t) = \left\lVert v'(t) \right\rVert$ is the radius of curvature of $\partial K$ at $v(t)$.

We gather various properties of $\beta_K$ that will be used thoroughly in the rest of the paper. First, $\beta_K$ is convex-linear with respect to $K$. This comes from the fact that in dimension $n=2$, the surface area measure is equal to _the mixed area measure_ with $n-1 = 1$ argument, and that the mixed area measure is convex-linear with respect to each argument (e.g. from (5.27), p284 of [[@schneider_2013]]).
...
> __Theorem [surface-area-measure-area].__ (Theorem 5.1.7 in p280 of [[@schneider_2013]]) The mixed volume $V(K_1, K_2)$ of any two convex bodies $K_1$ and $K_2$ can be represented as the following. ^thm-surface-area-measure-area
$$
...
So the area $|K|$ of a cap $K \in \mathcal{K}_{\omega}$ is a quadratic functional on $\mathcal{K}_\omega$ in particular. The proof of [[03. A1 is quadratic#^thm-surface-area-measure-area]] can be found in .
