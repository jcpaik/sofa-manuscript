...
_Proof._ Use [[05. Vertex and support function#^pro-support-function-linear]] for $p_K$, [[05. Vertex and support function#^cor-vertex-right-continuous]] for $A^{\pm}_K(t)$ and $C^{\pm}_K(t)$, [[05. Vertex and support function#^def-convex-body-tangent-lines-intersection]] for $\mathbf{y}_K(t), W_K(t)$, and $Z_K(t)$, and [[25. Mamikon's theorem#^cor-tangent-line-length-linear]] for $g_K^+(t)$ and $h_K^+(t)$. Then use the equalities $\mathbf{x}_K(t) = \mathbf{y}_K(t) - u_t - v_t$ and that . □
