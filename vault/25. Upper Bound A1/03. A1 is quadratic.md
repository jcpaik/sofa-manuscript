...
_Proof._ By the second condition of [[12. Structure of a monotone sofa#^def-cap]] and [[20. Normal angles#^thm-convex-set-support]], we have $\mathbf{n}(K) \subseteq J_\omega \cup \left\{ \pi + \omega, 3\pi/2 \right\}$. Now by [[20. Normal angles#^thm-convex-set-support-disjoint]], the point $A_K^-(0)$ is on the line $l_K(3\pi/2)$ which is $y=-1$, and the point $A_K^+(\omega)$ is on the line $l_K(\pi/2)$ which is $y=0$. By [[10. Surface area measure#^cor-boundary-measure-closed]] we have
$$
