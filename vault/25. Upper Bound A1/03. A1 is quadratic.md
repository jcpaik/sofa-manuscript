...
$$
and $\beta(\left\{ \pi + \omega \right\}) = B + v_\omega \cdot p_\omega \geq 0$ and $\beta(\left\{ 3 \pi/2 \right\}) = A + u_0 \cdot p_\omega \geq 0$.

we get the unique extension of $\beta$ to $\Pi$ with $\int_{t \in \Pi} v_t \, \beta(dt) = 0$ defined as the following.
