...
> __Theorem [balanced-ACx].__ $\beta_{K}$ is the same as $\iota_{K}$ on the open interval $(t_1, t_2)$ if and only if there is a constant $a_0$ and $c_0$ such that $\beta_K(t) = (a_0 + t) dt$ and $\beta_K(t + \pi/2) = (c_0 + t) dt$ on the interval $t \in (t_1, t_2)$. Moreover, for such $K$ we have $g^{\pm}(t) = g^{\pm}(t_1) + (t - t_0)$. ^thm-balanced-ACx
...
_Proof._ By [[05. Definition#^thm-a1-is-upper-bound]], [[15. Calculus of Variation#^thm-maximum-presofa-a1]] and [[15. Calculus of Variation#^thm-maximum-presofa-a1-area]]. □
