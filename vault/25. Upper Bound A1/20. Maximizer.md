...
_Proof._ We use [[03. Area Functional on Space of Caps#^thm-cap-from-boundary-measure]] to construct such a cap $K_1$. Calculating $\int_{t \in [0, \omega] } \cos \left( t \right) \, \beta_{K_1} (dt)$ gives 1 like the computation above. Likewise, calculating $\int_{t \in [\pi/2, \omega + \pi/2]} \cos\left( \omega + \pi/2 - t \right)  \, \beta_K(dt)$ also gives 1 by a similar computation. If $\omega = \pi/2$, the construction of [[03. Area Functional on Space of Caps#^thm-cap-from-boundary-measure]] gives a freedom of translating $K_1$ along the $x$-axis. Using the symmetry of $\beta_{K_1}$, position $K_1$ so that it is symmetric along the $y$-axis. □
...
_Proof._ By [[03. Area Functional on Space of Caps#^thm-variation-maximizer]] and [[#^thm-variation-a1]] it suffice to check that $\beta_{K_1}(dt) = i_{K_1}(t)dt$ on $J_\omega$ except for the points $t = \omega, \pi/2$. Take any $t \in (0, \omega)$, then we have the following.
$$
\begin{align*}
