...
We need some preparation to solve for the equation $\beta_K = \iota_K$ on $[0, \pi] \setminus \left\{ \pi/2 \right\}$. From now on, let $K \in \mathcal{K}_{\pi/2}$ be an arbitrary cap. We will use the arm lengths $g_K^{\pm}(t)$ and $h_K^{\pm}(t)$ as our main tool for caculation. For most $t \in (0, \pi/2)$, the values $g_K^+(t)$ and $g_K^-(t)$ (resp. $h_K^+(t)$ and $h_K^-(t)$) are equal and we will denote the common value by dropping their signs.
...
With [[20. Maximizer#^thm-balanced-ACx]], we can now solve for the equation $\beta_K = \iota_K$ on $[0, \pi] \setminus \left\{ \pi/2 \right\}$ with $t_1 = 0$ and $t_2 = \pi/2$. Let $K$ be a solution, then it should be that $g_K(0) = 1$ and $h_K(\pi/2) = 1$ as $\beta_K(\left\{ 0 \right\}) = \beta_K(\left\{ \pi \right\}) = 0$. So by [[20. Maximizer#^thm-balanced-ACx]] we should have $\beta_K(dt) = (\pi/2 - t) dt$ and $\beta_K(dt + \pi/2) = t dt$ on the set $t \in (0, \pi/2)$. We also have $g_K^-(\pi/2) = 1 + \pi/2$. Another calculation by xxx shows $g_K^+(\pi/2) = \int_0^{\pi/2} t \cos t\, dt = \pi / 2 - 1$. So we also have $\beta_K(\left\{ \pi/2 \right\}) = 2$ as well (note that on the other hand we have $\iota_K(\left\{ \pi/2 \right\}) = 0$). So the solution $K$ should be the following cap.
...
To justify the existence of such $K = K_{\max, 1}$ we can use [[08. Surface Area Measure#^thm-cap-from-boundary-measure]]. By [[08. Surface Area Measure#^thm-cap-from-boundary-measure]], we only need to check $\int_0^{\pi/2} \cos(t) (\pi/2 - t) \, dt = 1$ which is true by calculation. By [[20. Maximizer#^thm-balanced-ACx]] we have $\beta_K = \iota_K$ on $[0, \pi] \setminus \left\{ \pi/2 \right\}$ for $K = K_{\max, 1}$ indeed, and so by [[01. Space of Caps#^thm-quadratic-variation]] we get the following.
