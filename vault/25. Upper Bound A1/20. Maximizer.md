Here, we derive the maximizer $K_1$ of our concave upper bound $\mathcal{A}_1(K_1)$. What we want is $D\mathcal{A}_1(K_1; K_2) = \left< p_{K_2} - p_{K_1}, \beta_{K_1} - \iota_{K_1} \right>_{[0, \pi]}$ to be equal to zero for every other $K_2$. That is, we need to solve for $\beta_{K_1} = \iota_{K_1}$ on the set $[0, \pi] \setminus \left\{ \pi/2 \right\}$ (we exclude the point $\pi/2$ because $p_{K_2} - p_{K_1}$ always have the value zero at $t = \pi/2$). 

Note that for every bounded variation function $F : \mathbb{R} \to \mathbb{R}$ we write $dF$ for the Lebesgue-Stieltjes measure of $F$. So for example, $dt$ is the Lebesgue measure on $t \in [0, \pi/2]$. 
...
> __Theorem [balanced-ACx].__ $\beta_{K}$ is the same as $\iota_{K}$ on the open interval $(t_1, t_2)$ and $(t_1 + \pi/2, t_2 + \pi/2)$ if there is a constant $a_0$ and $c_0$ such that $\beta_K(t) = (a_0 + t) dt$ and $\beta_K(t + \pi/2) = (c_0 + t) dt$ on the interval $t \in (t_1, t_2)$. Moreover, for such $K$ we have $g^{\pm}(t) = g^{\pm}(t_1) + (t - t_0)$. ^thm-balanced-ACx
