...
[[03. Surface Area Measure#^surface-area-measure-vertex]]
...
To justify the existence of such $K = K_{\max, 1}$ we can use [[03. Surface Area Measure#^thm-cap-from-boundary-measure]]. By [[03. Surface Area Measure#^thm-cap-from-boundary-measure]], we only need to check $\int_0^{\pi/2} \cos(t) (\pi/2 - t) \, dt = 1$ which is true by calculation. By [[20. Maximizer#^thm-balanced-ACx]] we have $\beta_K = \iota_K$ on $[0, \pi] \setminus \left\{ \pi/2 \right\}$ for $K = K_{\max, 1}$ indeed, and so by [[01. Space of Caps#^thm-quadratic-variation]] we get the following.
