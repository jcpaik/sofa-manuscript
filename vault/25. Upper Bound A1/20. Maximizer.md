...
To solve for the equation $\beta_K = \iota_K$ on $[0, \pi] \setminus \left\{ \pi/2 \right\}$, we need some preparation. We will use the arm lengths $g_K^{\pm}(t)$ and $h_K^{\pm}(t)$ as our main tool. Most of the time, the values $g_K^+(t)$ and $g_K^-(t)$ (resp. $h_K^+(t)$ and $h_K^-(t)$) 
...
The following theorem solves the equation $\beta_K = \iota_K$ on any open interval $(t_1, t_2) \cup (t_1 + \pi/2, t_2 + \pi/2)$. Note that the equaiton $\beta_K = \iota_K$ is a mesure-theoretic version of (ODE3), Theorem 2 of [[@romikDifferentialEquationsExact2018]] generalized to arbitrary caps.
...
_Proof._ Assume that $\beta_K = \iota_K$ on the set $X = (t_1, t_2) \cup (t_1 + \pi/2, t_2 + \pi/2)$. Since the measure of $\beta_K$ is zero for every point of $X$, by [[20. Maximizer#^def-arm-length-unsinged]] we have $g_K(t) = g^\pm_K(t)$ and $h_K(t) = h_K^{\pm}(t)$ for every $t \in (t_1, t_2)$. Also, by xxx functions $g_K(t)$ and $h_K(t)$ are continuous with respect to $t \in (t_1, t_2)$. So by [[20. Maximizer#^thm-arm-length-derivative]]  □
