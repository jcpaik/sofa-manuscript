...
To solve for the equation $\beta_K = \iota_K$ on $[0, \pi] \setminus \left\{ \pi/2 \right\}$, we need some preparation. From now on, let $K \in \mathcal{K}_{\pi/2}$ be an arbitrary cap. We will use the arm lengths $g_K^{\pm}(t)$ and $h_K^{\pm}(t)$ as our main tool for caculation. For most $t \in (0, \pi/2)$, the values $g_K^+(t)$ and $g_K^-(t)$ (resp. $h_K^+(t)$ and $h_K^-(t)$) are equal and we will denote the common value by dropping the signs.
...
> - if there is a constant $a_0$ and $c_0$ such that $\beta_K(dt) = (a_0 + t) dt$ and $\beta_K(dt + \pi/2) = (c_0 + t) dt$ on the interval $t \in (t_1, t_2)$.
> 
> Moreover, for such $K$ we have $g(t) = g(t_1) + (t - t_0)$.
