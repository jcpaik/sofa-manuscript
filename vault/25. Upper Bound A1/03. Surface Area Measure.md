...
We elaborate the meaning of $dv_K^+(t) = v_t \beta_K(dt)$. Here, the function $v_K^+ : S^1 \to \mathbb{R}^{2}$ is right-continuous so the pair of Lebesgue-Stieltjes measures $d v_K^+(t)$ of $v_K^+(t)$ exists for each $x$ and $y$ coordinate, and each would be equal to $\cos(t) \beta_K(dt)$ and $-\sin(t) \beta_K(dt)$. A rigorous proof of [[03. Surface Area Measure#^thm-surface-area-measure-vertex]] is done in Appendix A.
