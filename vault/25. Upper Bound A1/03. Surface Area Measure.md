(Note as of Nov. 4 2023: Here I constantly mention Appendix A which is under construction. I do have the proofs of all claims that I defer to Appendix A, but these are too verbose as of now and I will work on to simplify them)
...
> __Theorem [surface-area-measure-vertex].__ For any convex body $K$, the following _vertex formulas_ hold. For any $a < b$ such that $b \leq a + 2\pi$, the followings are true. ^surface-area-measure-vertex
$$
...
Here, the function $v_K^+ : S^1 \to \mathbb{R}^{2}$ is right-continuous so the Lebesgue-Stieltjes measure $d v_K^+(t)$ of $v_K^+(t)$ exists for each $x$ and $y$ coordinate. A rigorous proof of [[03. Surface Area Measure#^surface-area-measure-vertex]] is done in Appendix A.
