...
%% Maybe talk about how $\mathbf{z}$ will be the full path including core and tails in future. %%
...
> __Definition [cap-tangent-arm-length].__ Let $K \in \mathcal{K}_\omega$ be arbitrary. For any $t \in [0, \omega]$, let $g_K^+(t)$ (resp. $g_K^-(t)$) be the real value such that $\mathbf{y}_K(t) = A^+_K(t) + g_K^+(t) v_t$ (resp. $\mathbf{y}(t) = A^-_K(t) + g_K^-(t) v_t$). Let $h_K^+(t)$ (resp. $h_K^-(t)$) be the value such that $\mathbf{y}(t) = C^+_K(t) + h_K^+(t) u_t$ (resp. $\mathbf{y}(t) = C^-_K(t) + h_K^-(t) u_t$). Call $g_K^{\pm}(t)$ and $h_K^{\pm}(t)$ the _arm lengths_ of $K$. ^def-cap-tangent-arm-length
...
> __Remark [inner-corner-deriv].__  ^rem-inner-corner-deriv