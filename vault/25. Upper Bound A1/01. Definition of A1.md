...
> __Definition [cap-tangent-arm-length].__ Let $K \in \mathcal{K}_\omega$ be arbitrary. For any $t \in [0, \omega]$, let $g_K^+(t)$ (resp. $g_K^-(t)$) be the real value such that $\mathbf{y}_K(t) = A^+_K(t) + g_K^+(t) v_t$ (resp. $\mathbf{y}(t) = A^-_K(t) + g_K^-(t) v_t$). So in particular we have $g_K^{\pm}(t) = \left( \mathbf{y}_K(t) - A_K^{\pm}(t) \right) \cdot v_t$. Similarly, let $h_K^+(t)$ (resp. $h_K^-(t)$) be the value such that $\mathbf{y}_K(t) = C^+_K(t) + h_K^+(t) u_t$ (resp. $\mathbf{y}(t) = C^-_K(t) + h_K^-(t) u_t$). So in particular we have $h_K^{\pm}(t) = \left( \mathbf{y}_K(t) - C_K^{\pm}(t) \right) \cdot u_t$. ^def-cap-tangent-arm-length

Call $g_K^{\pm}(t)$ and $h_K^{\pm}(t)$ the _arm lengths_ of $K$. The derivative of $\mathbf{x}_K$ can be expressed in terms of the arm lengths of $K$.
