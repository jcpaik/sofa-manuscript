...
The width of $K$ along the directions $u_\omega$ and $v_0$ are equal to 1, by applying the equations given in the theorem statement to [[10. Surface area measure#^cor-boundary-measure-width]] with angles $t = \omega, \pi/2$. Now we translate $K$. If $\omega = \pi/2$, we only need $\beta_K(\pi/2) = 1$ for $K$ to be a cap in $\mathcal{K}_{\pi/2}$, so such cap $K$ is unique up to horizontal translation. If $\omega < \pi/2$, we need both $\beta_K(\pi/2) = \beta_K(\omega) = 1$ so such cap $K$ exists uniquely among translations. □