...
\end{align*}
$$
> given by the Minkowski sum. ^pro-cap-space-convex-space

We will prove [[00. Sofa area functional#^thm-main-cap]] by optimizing $\mathcal{A}_1$ with calculus of variation. Here, we prepare a minimal amount of language needed to execute the calculus of variation on a quadratic functional on a general convex space $\mathcal{K}$.
