The conditions of [[00. Injectivity Hypothesis#^con-injectivity]] are chosen to guarantee that the closed curve $\Gamma$ formed by the trajectory of the inner corner $\mathbf{x}_K(t)$ and either the segment connecting $\mathbf{x}_K(\pi/2)$ and $\mathbf{x}_K(0)$ (if $\omega=\pi/2$) or two segments connecting $\mathbf{x}_{K}(\pi/2)$, $(0, 0)$, and $\mathbf{x}_K(0)$ is a Jordan curve. 
...
> __Theorem [a1-is-upper-bound].__ For any maximum monotone sofa $S$ with cap $K \in \mathcal{K}_{\omega}$ satisfying the conditions of the [[01. Injectivity Hypothesis and A1#^con-injectivity|injectivity hypothesis]], we have $|S| = \mathcal{A}(K) \leq \mathcal{A}_1(K)$. ^thm-a1-is-upper-bound
