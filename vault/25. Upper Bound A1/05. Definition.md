...
Gerver derived his sofa by assuming _a priori_ that the niche of a sofa of maximum area consists of such three curves. We circumvent the difficulty by first considering only the region bounded by the path $\mathbf{x}_K(t)$ of the inner corner for all $t \in [0, \pi/2]$. If we assume the injectivity hypothesis that $\mathbf{x}_K(t)$ is injective, the area enclosed by $\mathbf{x}_K(t)$ can be expressed as an integral using Green's theorem. We will use this integral to define $\mathcal{A}_1(K)$ as the area of $K$ subtracted by the area enclosed by $\mathbf{x}_K(t)$. so that $\mathcal{A}_1$ is an upper bound of $\mathcal{A}$ on the space of caps $\mathcal{K}_\omega$ if injectiviy hypothesis is true.

We now rigorously define the upper bound $\mathcal{A}_1$ and prove its properties. First define the _curve area functional_ of an arbitrary rectifiable curve $\mathbf{x}$.
...
For the next upper bound $\mathcal{A}_2$, we will choose a more refined version of $\mathbf{z}$ that matches the boundary of $\mathcal{N}(K)$ more closer by including the small area trimmed at the end of the curve $\mathbf{x}_K$.