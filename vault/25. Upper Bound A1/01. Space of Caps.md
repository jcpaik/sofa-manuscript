...
We will maximize $f = \mathcal{A}_1$ or $\mathcal{A}_2$ by essentially solving the equation $Df(K_{\text{m}}; -) = 0$ for $K_{\text{m}}$. We technically need to solve for the inequality $Df(K_{\text{m}}; -) \geq 0$, but modulo minor details it will turn out that we are essentially solving for the equality. Then the maximum value $\mathcal{A}_1(\mathcal{K}_{\max, 1})$ or $\mathcal{A}_2(\mathcal{K}_{\max, 2})$ will immediately give an upper bound of the sofa area functional $\mathcal{A}$.
