...
To prove that $K$ maximizes a concave quadratic functional $f(K)$ on $\mathcal{K}$, we show that we only need to prove that $Df(K; -)$ is a nonpositive linear functional on $\mathcal{K}$. 
