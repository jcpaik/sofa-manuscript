...
[[01. Space of Caps#^def-convex-space-directional-derivative]] can be seen as a kind of generalization of Gateaux derivative to arbitrary convex spaces. For any quadratic functional $f$ and a fixed $K \in \mathcal{K}$ the value $Df(K; K')$ is always a linear functional of $K'$.
...
We will maximize $f = \mathcal{A}_1$ or $\mathcal{A}_2$ by essentially solving the equation $Df(K_{\text{m}}; -) = 0$ for $K_{\text{m}}$. We technically need to solve for an inequality, but with details it will turn out that we are essentially solving for the equality modulo technical details. Then the maximum value $\mathcal{A}_1(\mathcal{K}_{\max, 1})$ or $\mathcal{A}_2(\mathcal{K}_{\max, 2})$ immediately gives an upper bound of the sofa area functional $\mathcal{A}$.

Moreover, we can use $\mathcal{A}_1$ to prove properties of the maximizer $K$ of $\mathcal{A}$. Assume that we want to show that a linear functional $g(K)$ on $K$ is always negative. We can then define a convex subset $\left\{ g \geq 0 \right\}$ of $\mathcal{K}_{\pi/2}$. By executing the same program, 
