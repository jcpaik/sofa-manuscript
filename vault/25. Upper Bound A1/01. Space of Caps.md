In [[15. Sofa Area Functional#^con-gerver-cap]], we turned the moving sofa problem into optimization of the sofa area functional $\mathcal{A} : \mathcal{K}_\omega \to \mathbb{R}$ on the space of caps $\mathcal{K}_\omega$. Our main observation here is that the space of caps $\mathcal{K}_\omega$ is a _convex space_. We will use this to invoke calculus of variation on the sofa area functional $\mathcal{A} : \mathcal{K}_\omega \to \mathbb{R}$ in later parts.
...
The sofa area functional $\mathcal{A}(K)$ with respect to cap $K \in \mathcal{K}_\omega$ is very likely not a quadratic functional. But we will soon construct upper bounds $\mathcal{A}_1$ and $\mathcal{A}_2$ of $\mathcal{A}$ that are quadratic functionals on cap space $\mathcal{K}_\omega$. We will show that $\mathcal{A}_1$ and $\mathcal{A}_2$ are also concave using the following criterion.
