...
We examine the intuitive meaning of [[15. Directional Derivative#^thm-variation-a1]]. Assume for a moment that $S$ is a monotone sofa of rotation angle $\pi/2$ with cap $K$ such that the niche $\mathcal{N}(K)$ is exactly the region bounded by the curve $\mathbf{x}_K(t)$.

In Theorem 2 (ODE3) of [[@romikDifferentialEquationsExact2018]], Romik essentially computes the following calculus of variation on a sofa. Assume that our cap $K$ has differentiable $A_K(t) = A_K^{\pm}(t)$ and $C_K(t) = C_K^{\pm}(t)$ for every $t \in (0, \pi/2)$. Take an arbitrary angle $t$ and fix a small positive $\delta$. We pertube $S$ as the following. Note that the monotone sofa $S = H \cap \bigcap_{0 \leq t \leq \pi/2} L_K(t)$ is the intersection of rotating hallways $L_K(t)$. For every $t' \in [0, \pi/2]$, we define a new family of rotating hallways $L'(t')$. For every $t' \in [t, t + \delta]$, let $L'(t') = L_S(t') + \epsilon u_t$, and for every other $t'$, let $L'(t') = L_S(t')$. That is, we move $L_S(t')$ in the direction of $\epsilon u_t$ for only $t' \in [t, t + \delta]$. Define $S' = H \cap \bigcap_{0 \leq t \leq \pi/2} L'(t)$, then $S'$ is a slight perturbation of $S$. By [[07. Tangent Hallway#^thm-moving-sofa-iff-hallway-intersection]], $S'$ is also a moving sofa modulo that $S'$ is connected.

Comparing $S$ and $S'$, some area is gained near $A_K(t)$ and some area is lost near $\mathbf{x}_K(t)$. The gain is about $\epsilon \delta \left\lVert A_K'(t) \right\rVert$, and the loss is the projection of $\mathbf{x}'(t)$ which is $\epsilon \delta \mathbf{x}'(t) \cdot v_t$. Observe that this matches with [[15. Directional Derivative#^thm-variation-a1]]. As we pertube $K$ to $K'$, $p_{K'}$ is nearly $p_K$ with $\epsilon 1_{[t, t + \delta]}$ added. As this is gained, the measure $\beta_{K}$ on the interval $[t, t + \delta]$ is approximately $\delta \left\lVert A_K'(t) \right\rVert$. The measure $\iota_{K_1}(t)$ on the interval $[t, t + \delta]$ is essentially defined as $\delta$ times the projection of $\mathbf{x}'(t)$ in the direction of $v_t$. The same argument mathces when we pertube for every $t' \in [t, t + \delta]$, $L'(t') = L_S(t') + \epsilon v_t$. In simpler words, $(p_{K_2} - p_{K_1})(t)$ and $(p_{K_2} - p_{K_1})(t + \pi/2)$ measures the movement of $\mathbf{x}_K(t)$ along the direction $u_t$ and $v_t$ respectively. Then $\beta_{K}(t)$ and $\beta_K(t + \pi/2)$ respectively measures the differential side length $A'(t)$ and $C'(t)$ respectively. Likewise, $\iota_K(t)$ and $\iota_K(t + \pi/2)$ measures the $u_t$ and $v_t$ component of $\mathbf{x}'(t)$ respectively. So multiplying and subtracting, we get the total differential area change.