...
_Proof._ For any convex body $K$ we have $|K| = V(K, K)$ where $V$ is the mixed volume of two planar convex bodies. So by applying [[01. Space of Caps#^lem-derivative-calculation]] we have the following.
$$
\left. \frac{d}{d \lambda} \right|_{\lambda=0} \left| (1-\lambda) K + \lambda K' \right| = V(K', K) - V(K, K)  
$$
...
> __Theorem [curve-area-variation].__ Let $\mathbf{x}, \mathbf{x}_0 : [t_a, t_b] \to \mathbb{R}$ be arbitrary rectifiable curves with the same domain $[t_a, t_b]$. Then we have the following. ^thm-curve-area-variation
$$
\left. \frac{d}{d \lambda} \right|_{\lambda=0} \left| (1-\lambda) \mathbf{x} + \lambda \mathbf{x}_0 \right| = \int_{t_a}^{t_b} (\mathbf{x}_0(t) - \mathbf{x}(t)) \times d\mathbf{x}(t)
$$

_Proof._ 
□

To compute the directional derivative of $\mathcal{I}(\mathbf{x})$ from $\mathbf{x}=\mathbf{x}_K$ to $\mathbf{x}=\mathbf{x}_{K'}$, we need to calculate the differentiation of $\mathbf{x}_K(t)$ for any cap $K \in \mathcal{K}_\omega$.
