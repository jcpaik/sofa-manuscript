...
$$
Let's focus on the second term more. 
...
We examine the intuitive meaning of [[15. Directional Derivative#^thm-variation-a1]]. In [[@romikDifferentialEquationsExact2018]], Romik essentially suggested the following way to derive Gerver's sofa.  Assume for a moment that our cap $K$ has differentiable $A_K(t) = A_K^{\pm}(t)$ for every $t \in (0, \pi/2)$, and likewise for $C$. Then there are two ways to perturb $K$. Take $\delta$ sufficiently small and make the modification from $t_0$ to $t_0 + \delta$. First, is to move the right boundary $A_K(t)$ by $\epsilon u_t$. Then the gain in the area of $K$ 