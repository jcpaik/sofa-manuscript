We now solve for the maximizer $K = K_{\omega, 1}$ of our concave quadratic upper bound $\mathcal{A}_1 : \mathcal{K}_\omega \to \mathbb{R}$. We do this _a priori_ by solving for the cap $K$ that satisfies $\beta_{K} = \iota_{K}$ on the set $J_\omega \setminus \left\{ \omega, \pi/2 \right\}$. Once we find such $K$, then it happens that the directional derivative $D\mathcal{A}_1(K; K') = 0$ for every other cap $K'$ by [[15. Directional derivative of A1#^thm-variation-a1]] because $p_{K'}(\omega) = p_K(\omega) = p_{K'}(\pi/2) = p_K(\pi/2) = 1$. Then by [[02. Calculus of variation#^thm-quadratic-variation]] the cap $K$ attains the maximum value of $\mathcal{A}_1$. As we have seen in the previous subsection, the equation $\beta_{K} = \iota_{K}$ on the set $J_\omega \setminus \left\{ \omega, \pi/2 \right\}$ can be compared to the local optimality equation (ODE3) in Theorem 2 of [[@romikDifferentialEquationsExact2018]]. However, unlike [[@romikDifferentialEquationsExact2018]] which solved for the inner corner $\mathbf{x}_K$, we will solve for the arm lengths $g_K^{\pm}(t)$ and $h_K^{\pm}(t)$ to find $K$.

From now on, let $K \in \mathcal{K}_{\omega}$ be an arbitrary cap with rotation angle $\omega \in (0, \pi/2]$. The following theorem shows that under good conditions, we don't need to differentiate $g_K^+(t)$ and $g_K^+(t)$ (resp. $h_K^+(t)$ and $h_K^-(t)$) and we can also calculate the derivatives of $g_K(t)$ and $h_K(t)$.

%% If $\beta_K = \iota_K$ on the set $J_\omega \setminus \left\{ \omega, \pi/2 \right\}$, then $\beta_K(\left\{ t \right\}) = \iota_K(\left\{ t \right\}) = 0$ for all $t \in J_\omega ^\circ$ so we have $g_K^+(t) = g_K^-(t)$ and $h_K^+(t) = h_K^-(t)$ for all $t \in (0, \omega)$ by [[01. Definition of A1#^pro-arm-length-unsigned]]. That is, we can write the common values simply as $g_K(t)$ and $h_K(t)$ for all $t \in (0, \omega)$ dropping the signs. %%
...
_Proof._ We have $g_K(t) = g_K^{\pm}(t)$ and $h_K(t) = h_K^{\pm}(t)$ for all $t \in U$ by [[01. Definition of A1#^pro-arm-length-unsigned]].

We have the followings for every $t \in U$ by [[20. Maximizer of A1#^lem-arm-length-integral]].
$$
...
The following theorem solves the equation $\beta_K = \iota_K$ on any open set $(t_1, t_2) \cup (t_1 + \pi/2, t_2 + \pi/2)$. Note that this equation is a mesure-theoretic version of (ODE3) in Theorem 2 of [[@romikDifferentialEquationsExact2018]] generalized to arbitrary caps.
