...
\end{gather*}
$$
we should have $\beta_K(\left\{ \omega \right\}) = \beta_K(\left\{ \pi/2 \right\}) = 1$ if $\omega < \pi/2$. Intuitively, this extends naturally to the condition $\beta_K\left( \left\{ \omega \right\} \right) = 2$ if $\omega = \pi/2$. Motivated by all the calculations made here, we define the following candidate $K = K_{\omega, 1}$ 
