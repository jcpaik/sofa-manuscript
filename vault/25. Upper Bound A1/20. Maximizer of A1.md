...
From now on, let $K \in \mathcal{K}_{\omega}$ be an arbitrary cap with rotation angle $\omega \in (0, \pi/2]$. The following theorem shows that under sufficient conditions, we don't need to differentiate $g_K^+(t)$ and $g_K^+(t)$ (resp. $h_K^+(t)$ and $h_K^-(t)$) and also can calculate the derivatives of $g_K$ and $h_K$ in terms of $\beta_K$.
