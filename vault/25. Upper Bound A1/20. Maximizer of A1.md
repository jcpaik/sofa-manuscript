...
_Proof._ Let $K = K_{\omega, 1}$. We will exploit the mirror symmetry of $K_{\omega, 1}$ along the line connecting $O$ to $o_\omega$. The line divides $K_{\omega, 1}$ into two pieces which are mirror images to each other, .  Compute the value of $p_{K}(t)$ for $t \in [0, \omega]$.
$$
\begin{align*}
...
\end{align*}
$$
Define $R := o_\omega \cdot \int_{0}^{\omega} u_t (\omega - t)\, dt$, and we have the following.
$$
\begin{align*}
|K| & = 1 + \omega^3 / 3 + R
\end{align*}
