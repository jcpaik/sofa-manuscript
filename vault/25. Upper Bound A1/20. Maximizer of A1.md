...
> __Definition [maximum-presofa-a1].__ Define the cap $K = K_{\omega, 1} \in \mathcal{K}_{\pi/2}$ as the cap with following boundary measure $\beta_{K}$ and $p_K(0) = 1$. ^def-maximum-presofa-a1
> 
...
_Proof._ Let us compute the value of $p_{K_1}(t)$ for $t \in [0, \omega]$. 
$$
\begin{align*}
p_{K_1}(t) & = A^-_{K_1}(t) \cdot u_t \\
