...
> __Definition [a1].__ For any cap $K \in \mathcal{K}_\omega$, define $\mathcal{A}_1(K) = |K| - I(\mathbf{x}_K)$. Note that $\mathbf{x}_K$ is both left and right [[01. Space of Caps#^def-inner-corner-deriv|differentiable]], so it is absolutely continuous and the value $I(\mathbf{x}_K)$ is well-defined. ^def-a1
