...
3. For every angle $t \in [0, \omega]$, some translation $L_t$ of $R_t(L)$. That is, a copy of $L$ rotated counterclockwise by $t$ and translated. The sofa $S$ is rotated initially by $0$ and finally by $\omega$. By the intermediate value theorem on the rotation of sofa in movement, a copy of $S$ rotated clockwise by $t \in [0, \omega]$ is contained in $L$. Now observe this in perspective of $S$ to conclude $S \subseteq L_t$.

Gerver pushed his idea further and proved the followings for a maximum-area sofa $S$ as well.
1. Translating $S$ without loss of generality,[^translation] we can assume that $S$ is the intersection of $H$, $R_\omega(V)$ and for every $t \in [0, \pi/2]$ some translation $L_t$ of the rotated hallway $R_t(L)$. [^gerver-claim]
2. We can also assume that the rotating hallways $L_t$ containing $S$ are the _tangent hallways_ of $S$ making contact with $S$ in the outer walls.
...
[^gerver-claim]: Gerver's proof in [[@gerverMovingSofaCorner1992]] has a logical gap in implicitly assuming that the intersection of $H$ and the rotating hallways $L_t$ should be connected. In [[@kallusImprovedUpperBounds2018]], Kallus and Romik mitigated this issue by taking the largest connected component of the intersection. In this paper, we fill the gap by showing that for the _tangent hallways_ $L_t$ that Gerver took, the intersection is always connected ([[15. Monotone sofas/10. Monotonization#^thm-monotonization-is-connected]]).

[^translation]: Technically, translating the sofa $S$ may invalidate the initial position $S \subseteq L_H$. To overcome this, we relax our definition of a moving sofa $S$ in [[01. Introduction/05. Notations and Conventions#^def-sofa]], so that a _translation_ of $S$ can move from $L_H$ to $L_V$ inside $L$.