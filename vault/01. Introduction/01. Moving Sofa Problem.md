...
In Section 2, we define monotone sofas ([[10. Monotone Sofa#^def-monotone-sofa]]) and establish [[01. Moving sofa problem#^thm-monotone-sofa]] by filling out a logical gap in Gerver's proof in [[@gerverMovingSofaCorner1992]]. Moreover, we show ([[12. Structure of monotone sofa#^thm-monotonization-structure]]) that any monotone sofa $S$ is equal to $K \setminus \mathcal{N}(K)$, where $K = \mathcal{C}(S)$ is some convex set called the _cap of_ $S$, and $\mathcal{N}(K)$ is a subset of $K$ called the _niche_ determined by the cap $K$ (see [[01. Moving sofa problem#^fig-monotone-sofa]]). Finally, 
