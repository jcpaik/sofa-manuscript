...
1. Horizontal strip $H = \mathbb{R} \times [0, 1]$, because $S$ is contained in the horizontal side $L_H$ initially.
2. A translation of $R_\omega(V)$, the vertical strip $V = [0, 1] \times \mathbb{R}$ rotated counterclockwise by $\omega$. This is because at the end of the movement, $S$ is contained in the vertical side $L_V$ rotated clockwise by $\omega$.
3. For every angle $t \in [0, \omega]$, some translation $L_t$ of the hallway $R_t(L)$ rotated counterclockwise by $t$. The sofa $S$ is rotated initially by $0$ and finally by $\omega$. By the intermediate value theorem on the rotation, some transl.

Gerver pushed the idea further and proved the followings as well.
- If $S$ attains the maximum area, we can always assume that $S$ is the intersection of $H$ and the rotating hallways $L_t$ for all $t \in [0, \pi/2]$.[^gerver-claim]
