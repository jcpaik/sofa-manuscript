...
$$
We define such a moving sofa $S$ with rotating hallways $L_t$ satisfying [[#^eqn-monotone]] a _monotone sofa_ (see [[15. Monotone sofas/10. Monotonization#^def-monotone-sofa]] for the full definition). Gerver's main observation in [[@gerverMovingSofaCorner1992]] summarizes to the following theorem.
...
Another key idea of the proof of [[01. Introduction/00. Preface#^thm-main]] is to identify a monotone sofa $S$ by its cap (convex hul) $K$. In particular, the overestimated area $\mathcal{A}_1$ as a function of the convex body $K$ turns out to be a _concave_ and _quadratic_ functional with respect to $K$. We will construct 
