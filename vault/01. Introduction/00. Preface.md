...
The moving sofa problem can be seen as an instance of the _inverse Kakeya problem_. The original Kakeya problem asks for the minimum-area chamber in which a needle of length 1 can be rotated the full $360\degree$, allowing the needle to translate in its movement. In an inverse Kakeya problem, the shape of the chamber is fixed and we are asked to find the shape $S$ (the 'needle') of largest area that can rotate freely inside the chamber, also allowing translations in movement. If the chamber is the strip $H = \mathbb{R} \times [0, 1]$ of unit width and the shape $S$ has to rotate the full $360\degree$, the problem becomes the isodiametric problem asking for the maximum-area shape of a fixed diameter 1. For any chamber which is convex, it is a consequence of the Brunn-Minkowski theorem that the shape $S$ of the largest area that can rotate the full $360\degree$ is 

As an inverse Kakeya problem, the chamber is the hallway $L$ and the 'needle' is the moving sofa $S$. 
