...
$$
We define such a moving sofa $S$ with rotating hallways $L_t$ satisfying [[#^eqn-monotone]] a _monotone sofa_ (see [[15. Monotone sofas/10. Monotonization#^def-monotone-sofa]] for the full definition). We summarize Gerver's main observation in [[@gerverMovingSofaCorner1992]] summarizes to the following theorem.

> __Theorem [monotone-sofa].__ There exists a monotone sofa $S_{\max}$ with rotation angle $\omega \in [\pi/3, \pi/2]$ attaining the maximum area $\mu_{max}$. ^thm-monotone-sofa
