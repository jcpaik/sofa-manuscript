...
The upper bound $1 + \omega^2/2$ of [[01. Introduction/00. Preface#^thm-main]] maximizes at $\omega = \pi/2$ with the value $1 + \pi^2/8 = 2.2337\dots$. This is much closer to the lower bound $2.2195\dots$ of Gerver than the currently best upper bound of $2.37$ of Kallus and Romik ([[01. Introduction/00. Preface#^eqn-best-bounds]]). Note that Gerver's sofa also satisfies the condition of [[01. Introduction/00. Preface#^thm-main]] (see [[01. Introduction/00. Preface#^fig-gerver]]):
- The rotation path $\mathbf{x}_{S_G}$ of Gerver's sofa $S_G$ is injective.
- The rotation path $\mathbf{x}_{S_G}$ is always on or above the bottom line $y = 0$ of $H$.
So the upper bound of [[01. Introduction/00. Preface#^thm-main]] is effective on a domain containing the conjectured maximum $S_G$.

We conjecture that a monotone sofa of maximum area satisfies the premise of [[01. Introduction/00. Preface#^thm-main]].
...
Our main idea for proving [[01. Introduction/00. Preface#^thm-main]] is to overestimate the area of a monotone sofa $S$. Observe that the lower portion of the boundary of Gerver's sofa $S_G$ consists of two 'tails' and one 'core' (see [[01. Introduction/00. Preface#^fig-gerver]]). The core, which is the trajectory of the inner corner $\mathbf{x}_{S_G}(t)$ for $t \in [\varphi, \pi/2 - \varphi]$ with $\varphi = 0.0392\dots$, forms the majority of the lower boundary [[@romikDifferentialEquationsExact2018]]. . Inspired by this, define the overestimation $\mathcal{A}_1$ of the area $\mathcal{A}$ of a monotone sofa $S$ as the area of the convex hull $K$ of $S$ subtracted by the region bounded by 
