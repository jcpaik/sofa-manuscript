...
Both bounds in [[01. Introduction/00. Preface#^eqn-best-bounds]] are based on an important observation of Gerver in [[@gerverMovingSofaCorner1992]]. Take any moving sofa $S$ with rotation angle $\omega \in (0, \pi/2]$. He looked at the hallway $L$ in the perspective of the moving sofa $S$, so that $S$ is fixed and the hallway $L$ moves around the sofa (see [[01. Introduction/00. Preface#^fig-monotone-sofa]]). In this perspective, the sofa $S$ is now a common subset of the rotating hallways and the two unit-width strips described as the following.

1. For every angle $t \in [0, \omega]$, a hallway $L_t$ rotated counterclockwise by $t$ and translated.
...
Without loss of generality, we can translate the moving sofa $S$ horizontally so that $S$ is contained in $V_\omega$, not its translation.[^translation] If $S$ attains the maximum area, we can further assume that $S$ is simply the intersection of $H$, $V_\omega$, and $L_t$ for all $t \in [0, \omega]$.[^connectedness] In this paper, we define such a maximum-area moving sofa $S$ as a _monotone sofa_ ([[15. Monotone sofas/10. Monotonization#^def-monotone-sofa]]). Gerver's main observation in [[@gerverMovingSofaCorner1992]] then summarizes to:
