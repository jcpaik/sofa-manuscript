...
Without loss of generality, we can translate the moving sofa $S$ horizontally to further assume that $S$ is contained in $V_\omega$, not its arbitrary translation.[^translation] If $S$ attains the maximum area, we can further assume that $S$ is simply the intersection of the sets $H$, $V_\omega$, and $L_t$ for all $t \in [0, \omega]$ containing $S$, so that the following holds.[^connectedness] ^eqn-monotone
$$
S = H \cap V_\omega \cap \bigcap_{0 \leq t \leq \omega} L_t
$$
We define such a moving sofa $S$ satisfying [[#^eqn-monotone]] a _monotone sofa_ (see [[15. Monotone sofas/10. Monotonization#^def-monotone-sofa]] for a full definition). Gerver's main observation in [[@gerverMovingSofaCorner1992]] summarizes to the following theorem.
