...
Without loss of generality, we can translate the moving sofa $S$ horizontally to assume that $S$ is contained in $V_\omega$, not its translation.[^translation] If $S$ attains the maximum area, we can further assume that $S$ is simply the intersection of $H$, $V_\omega$, and $L_t$ for all $t \in [0, \omega]$.[^connectedness] In this paper, we define such a maximum-area moving sofa $S$ as a _monotone sofa_ ([[15. Monotone sofas/10. Monotonization#^def-monotone-sofa]]; see [[01. Introduction/00. Preface#^fig-monotone-sofa]]). Gerver's main observation in [[@gerverMovingSofaCorner1992]] summarizes to:
