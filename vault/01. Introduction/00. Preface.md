...
Both bounds in [[01. Introduction/00. Preface#^eqn-best-bounds]] are based on an important observation of Gerver in [[@gerverMovingSofaCorner1992]]. Take any moving sofa $S$ with rotation angle $\omega \in (0, \pi/2]$. He looked at the hallway $L$ in the perspective of the moving sofa $S$, so that $S$ is fixed and the hallway $L$ moves around the sofa (see [[01. Introduction/00. Preface#^fig-monotone-sofa]]). In this perspective, the sofa $S$ is now a common subset of the rotated hallways and the two unit-width strips described as the following.

1. For every angle $t \in [0, \omega]$, some hallway $L_t$ rotated counterclockwise by $t$ and translated.
	- The sofa $S$ rotates initially by an angle of $0$ and finally by $\omega$. By the intermediate value theorem, a copy of $S$ is rotated clockwise by $t \in [0, \omega]$ in its movement inside $L$. Now see this in the perspective of $S$ to conclude $S \subseteq L_t$.
...
3. A translation of $V_\omega$, where $V_\omega$ is the vertical strip $V = [0, 1] \times \mathbb{R}$ rotated counterclockwise by $\omega$ along the origin.
	- At the end of the movement, $S$ is contained in the vertical way $L_V$ rotated clockwise by $\omega$; observe this in perspective of $S$.

Without loss of generality, we can translate the moving sofa $S$ horizontally to assume that $S$ is contained in $V_\omega$, not its translation.[^translation] If $S$ attains the maximum area, we can further assume that $S$ is simply the intersection of $H$, $V_\omega$, and $L_t$ for all $t \in [0, \omega]$.[^connectedness] In this paper, we define such a maximum-area moving sofa $S$ as a _monotone sofa_ ([[15. Monotone sofas/10. Monotonization#^def-monotone-sofa]]; see [[01. Introduction/00. Preface#^fig-monotone-sofa]]). Gerver's main observation in [[@gerverMovingSofaCorner1992]] then summarizes to:
...
Let $S$ be a monotone sofa with rotation angle $\omega \in (0, \pi/2]$, so that $S$ is the intersection of $H$, $R_\omega(V)$, and the hallways $L_t$ rotating counterclockwise by an angle of $t \in [0, \omega]$. For every $t \in [0, \omega]$, the coordinate of the inner corner $\mathbf{x}_S(t)$ of $L_t$ determines the location of $L_t$ completely. So the trajectory $\mathbf{x}_S : [0, \omega] \to \mathbb{R}^2$ of the inner corner, defined as the _rotation path_ of $S$ by [[@romikDifferentialEquationsExact2018]], determines the monotone sofa $S$. In particular, Romik derived Gerver's sofa $S_G$ by solving optimality conditions as a set of ordinary differential equations on $\mathbf{x}_S$ [[@romikDifferentialEquationsExact2018]].
...
[^connectedness]: A subtle assumption of  should be connected. In this paper, we fill this gap in [[15. Monotone sofas/10. Monotonization#^thm-monotonization-is-connected]].
