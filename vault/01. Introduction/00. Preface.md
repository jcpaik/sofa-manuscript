...
In this paper, we prove that any moving sofa satisfying a condition named as the _injectivity condition_ has area at most $1 + \pi^2/8 = 2.2337\dots$, matching the lower bound of Gerver much closer than the upper bound of Kallus and Romik. Since Gerver's sofa $S_G$ satisfies the injectivity condition, the conditional upper bound is effective on the domain containing the conjectured optimum $S_G$. The proof does not rely on any computer assistance. We also propose the _injectivity hypothesis_ that there is a maximum-area moving sofa satisfying the injectivity hypothesis. We show that any maximum-area sofa sufficiently close to Gerver's sofa in the Hausdorff distance should satisfy the injectivity condition, which supports the injectivity hypothesis. Showing the injectivity hypothesis will prove the unconditional upper bound $\mu_{\max} \leq 1 + \pi^2/8 = 2.2337\dots$.

# Moving Hallway Problem

The moving sofa problem can be seen as an instance of the _inverse Kakeya problem_. The original Kakeya problem asks for the minimum-area chamber $R$ in which a needle $S$ of length 1 can be rotated the full $360\degree$, allowing the needle to translate in its movement. In an inverse Kakeya problem, the shape of the chamber $R$ is fixed and we are asked to find the shape $S$ (the 'needle') of largest area that can rotate freely inside the chamber, also allowing translations in movement. If the chamber is the strip $R = \mathbb{R} \times [0, 1]$ of unit width, and the shape $S$ has to rotate the full $360\degree$, the problem becomes the well-studied _isodiametric problem_ asking for the maximum-area shape with diameter at most 1. If the chamber $R$ is a planar convex set, it is a consequence of the Brunn-Minkowski theorem that the largest-area shape $S$ that can rotate the full $360\degree$ inside $R$ is the largest inscribed ball. As an inverse Kakeya problem, the moving sofa problem have the hallway $L$ as the chamber and the moving sofa $S$ as the 'needle'.

The _rotation angle_ $\omega$ of a moving sofa $S$ is the clockwise angle that $S$ rotates as it moves from $L_H$ to $L_V$ inside $L$. As an inverse Kakeya problem, the rotation angle dictates how much the moving sofa $S$ should rotate inside the hallway $L$. Gerver's sofa has the rotation angle $\omega = \pi/2$. On the other hand, the unit square $[0, 1]^2$ can move inside $L$ with only translation, so it has the rotation angle $\omega = 0$. Let $\omega$ is the rotation angle of a moving sofa of maximum area. Gerver showed in [[@gerverMovingSofaCorner1992]] that we can assume $\pi/ 3 \leq \omega \leq \pi/2$. Kallus and Romik improved the lower bound of $\omega$ by showing that $\omega \geq \arcsin(84/85) = 81.203\dots^\circ$ [[@kallusImprovedUpperBounds2018]]. With this, we will only consider moving sofas with rotation angle $\omega \in (0, \pi/2]$, and it seems reasonable to conjecture that a maximum-area moving sofa has $\omega = \pi/2$.
