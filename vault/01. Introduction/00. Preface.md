...
In this paper, we prove that any moving sofa satisfying a condition named as the _injectivity condition_ has area at most $1 + \pi^2/8 = 2.2337\dots$. Since Gerver's sofa $S_G$ satisfies the injectivity condition, the conditional upper bound is effective on the domain containing the conjectured optimum $S_G$. The proof does not rely on any computer assistance, and . We also propose the _injectivity hypothesis_ that there is a maximum-area moving sofa satisfying the injectivity hypothesis. 

# Moving Hallway Problem

The moving sofa problem can be seen as a 
