...
The moving sofa problem can be seen as an instance of the _inverse Kakeya problem_. The original Kakeya problem asks for the minimum-area chamber in which a needle of length 1 can be rotated the full $360\degree$, allowing the needle to translate in its movement. In an inverse Kakeya problem, the shape of the chamber is fixed and we are asked to find the shape (the 'needle') of largest area that can rotate freely inside the chamber, again allowing translations in movement. If the chamber is the strip $H = \mathbb{R} \times [0, 1]$ of unit width and the needle has to rotate the full $360\degree$, the problem becomes the isodiametric problem asking for the maximum-area shape of a fixed diameter 1. For any chamber which, it is a consequence of the Brunn-Minkowski theorem that the shape of the largest area 

In the moving sofa problem as an inverse Kakeya problem, the chamber is the hallway $L$ and the 'needle' is the moving sofa $S$. 

The rotation angle $\omega$ of a moving sofa $S$ is the clockwise angle that $S$ rotates as it moves from $L_H$ to $L_V$ inside $L$. Gerver's sofa has the rotation angle $\omega = \pi/2$. On the other hand, the unit square $[0, 1]^2$ can move inside $L$ with only translation, so it has the rotation angle $\omega = 0$. Let $\omega$ is the rotation angle of a moving sofa of maximum area. Gerver showed in [[@gerverMovingSofaCorner1992]] that we can assume $\pi/ 3 \leq \omega \leq \pi/2$. Kallus and Romik improved the lower bound of $\omega$ by showing that $\omega \geq \arcsin(84/85) = 81.203\dots^\circ$ [[@kallusImprovedUpperBounds2018]]. With this, we will only consider moving sofas with rotation angle $\omega \in (0, \pi/2]$, and it seems reasonable to conjecture that a maximum-area moving sofa has $\omega = \pi/2$.
