...
Our main idea for proving [[01. Introduction/00. Preface#^thm-main]] is to overestimate the area of a monotone sofa $S$. For explanation, consider the case $S = S_G$ and $\omega = \pi/2$. Observe that the lower portion of the boundary of Gerver's sofa $S_G$ consists of two 'tails' and one 'core' (see [[01. Introduction/00. Preface#^fig-gerver]]). The core, which is the trajectory of the inner corner $\mathbf{x}_{S_G}(t)$ for $t \in [\varphi, \pi/2 - \varphi]$ with $\varphi = 0.0392\dots$, forms the majority of the lower boundary [[@romikDifferentialEquationsExact2018]]. The area below the two tails, carved out by the inner left and walls of , is and constitue only of $\mu_{\max} = 2.2195\dots$. Inspired by this, we define the overestimation $\mathcal{A}_1$ of the area $\mathcal{A}$ of a monotone sofa $S$ as the area of the convex hull $K$ of $S$, subtracted by the region enclosed by $\mathbf{x}_S$ and the line $y=0$ (and an extra line $x \cos \omega + y \sin \omega = 0$ if $\omega < \pi/2$).
