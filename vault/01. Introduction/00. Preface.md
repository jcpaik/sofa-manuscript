...
[^translation]: Technically, translating the moving sofa $S$ may invalidate the initial condition $S \subseteq L_H$. To overcome this, we relax our [[10. Notations and conventions/00. Preface#^def-sofa]] of a moving sofa $S$ in so that only a _translation_ of $S$ is required to be movable from $L_H$ to $L_V$ inside $L$.
