...
1. For every angle $t \in [0, \omega]$, the hallway $L_t$ rotated counterclockwise by $t$ so that the outer walls of $L_t$ make contact with $S$.
...
The sofa $S$ rotates initially by an angle of $0$ and finally by $\omega$. By the intermediate value theorem, a copy of $S$ is rotated clockwise by $t \in [0, \omega]$ in its movement inside $L$. Now see this in the perspective of $S$ to conclude $S \subseteq L_t$. At the end of the movement, $S$ is contained in the vertical way $L_V$ rotated clockwise by $\omega$; observe this in perspective of $S$.

Without loss of generality, we can translate the moving sofa $S$ horizontally to further assume that $S$ is contained in $V_\omega$, not its translation.[^translation] If $S$ attains the maximum area, we can further assume that $S$ is simply the intersection of $H$, $V_\omega$, and $L_t$ for all $t \in [0, \omega]$.[^connectedness] In this paper, we define such a moving sofa $S$ as a _monotone sofa_ ([[15. Monotone sofas/10. Monotonization#^def-monotone-sofa]]; see [[01. Introduction/00. Preface#^fig-monotone-sofa]]). Gerver's main observation in [[@gerverMovingSofaCorner1992]] summarizes to the following theorem.
