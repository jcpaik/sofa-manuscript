...
Once we fix the rotation angle $\omega \in (0, \pi/2]$ of a moving sofa $S$, we can change the moving sofa problem as the moving _hallway_ problem. In [[@gerverMovingSofaCorner1992]], Gerver looked at the hallway $L$ in the perspective of the moving sofa $S$, so that $S$ is fixed and the hallway $L$ moves around the sofa [[@gerverMovingSofaCorner1992]] (see [[01. Introduction/00. Preface#^fig-monotone-sofa]]). In this perspective, the sofa $S$ is now a common subset of the following sets.
