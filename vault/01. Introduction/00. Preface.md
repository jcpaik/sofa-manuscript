...
The inverse Kakeya problem has been solved If the chamber $R$ is a planar convex set; it is a consequence of the Brunn-Minkowski theorem that the largest-area shape $S$ that can rotate the full $360\degree$ inside a convex chamber $R$ is the largest ball inscribed in $R$. The moving sofa problem, as an inverse Kakeya problem, has the non-convex hallway $L$ as the chamber and the moving sofa $S$ as the 'needle' inside the hallway $L$.  
