...
Both bounds in [[01. Introduction/00. Preface#^eqn-best-bounds]] are based on an important observation of Gerver in [[@gerverMovingSofaCorner1992]]. Take any moving sofa $S$ with rotation angle $\omega \in (0, \pi/2]$. He looked at the hallway $L$ from the perspective of the moving sofa $S$ in movement, so that the sofa $S$ is fixed and the hallway $L$ moves around the sofa (see [[01. Introduction/00. Preface#^fig-monotone-sofa]]). From this observation, we can see that $S$ is always a subset of the following strips and rotating hallways. Let $R_t$ be the rotation map of $\mathbb{R}^2$ along the origin by a counterclockwise angle of $t$.
...
[^translation]: Technically, translating the sofa $S$ may invalidate the initial position $S \subseteq L_H$. To overcome this, we relax our definition of a moving sofa $S$ in [[10. Notations and conventions/00. Preface#^def-sofa]], so that we only require a _translation_ of $S$ to be movable from $L_H$ to $L_V$ inside $L$.
