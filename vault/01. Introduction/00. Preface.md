...
For each angle $t \in [0, \omega]$, let $R_t$ be the rotation map of $\mathbb{R}^2$ along the origin  $\mathbf{x}_S(t)$ be the coordinate of the inner corner of $L_t$. determines the location of $L_t$. So the trajectory $\mathbf{x}_S : [0, \omega] \to \mathbb{R}^2$ of the inner corner, defined as the _rotation path_ of $S$ in [[@romikDifferentialEquationsExact2018]], determines the monotone sofa $S$ completely. In particular, Romik derived Gerver's sofa $S_G$ by solving for the local optimality as a set of ordinary differential equations on $\mathbf{x}_S$ [[@romikDifferentialEquationsExact2018]].[^gerver-derivation]
