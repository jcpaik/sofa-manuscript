...
Without loss of generality, we can translate the moving sofa $S$ horizontally to further assume that $S$ is contained in $V_\omega$, not its translation.[^translation] If $S$ attains the maximum area, we can also assume that $S$ is simply the intersection of $H$, $V_\omega$, and $L_t$ for all $t \in [0, \omega]$.[^connectedness] In this paper, we define such a moving sofa $S$ as a _monotone sofa_ ([[15. Monotone sofas/10. Monotonization#^def-monotone-sofa]]; see [[01. Introduction/00. Preface#^fig-monotone-sofa]]). Gerver's main observation in [[@gerverMovingSofaCorner1992]] summarizes to the following theorem.
...
Let $S$ be a monotone sofa with rotation angle $\omega \in (0, \pi/2]$, so that $S$ is the intersection of $H$, $R_\omega(V)$, and the hallways $L_t$ rotating counterclockwise by an angle of $t \in [0, \omega]$. For every $t \in [0, \omega]$, the coordinate of the inner corner $\mathbf{x}_S(t)$ of $L_t$ determines the location of $L_t$ completely. So the trajectory $\mathbf{x}_S : [0, \omega] \to \mathbb{R}^2$ of the inner corner, defined as the _rotation path_ of $S$ in [[@romikDifferentialEquationsExact2018]], determines the monotone sofa $S$. In particular, Romik derived Gerver's sofa $S_G$ by solving for the area optimality condition as a set of ordinary differential equations on $\mathbf{x}_S$ [[@romikDifferentialEquationsExact2018]].
