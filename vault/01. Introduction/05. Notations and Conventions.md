...
A line on $\mathbb{R}^2$ divids the plane into two half-planes. Following [[01. Introduction/05. Notations and Conventions#^def-line]], we also give a name to one of the half-planes in the direction of $-u_t$.
...
Observe that the width of $S$ along $u_t$ measures the distance between the parallel tangent lines $l_S(t)$ and $l_S(t + \pi)$ of $S$.

We adopt the following definition of convex bodies.

> __Definition [convex-body].__ A _convex body_ $K$ is a nonempty, compact, and convex subset of $\mathbb{R}^2$. ^def-convex-body

We follow p8 of [[@schneider_2013]] for [[01. Introduction/05. Notations and Conventions#^def-convex-body]]. Many authors often also include the condition that $K^\circ$ is nonempty, but in our definition we allow $K^\circ$ to be empty (e.g. line segment or point).

In this paper, we will call 
