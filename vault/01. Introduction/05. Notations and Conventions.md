For clarity of explanation, some proofs of theorems are delegated at the end of each subsection, which a reader can either skip or verify if interested. A _proposition_ is a statement whose truth immediately follows from discussion, so its proof is always omitted. A _corollary_ is a consequence directly followed from a theorem. A _lemma_ is a theorem mainly used to prove another theorem (and likely serve no other purpose).

We set up basic notations, definitions and conventions that will be used thoroughout the rest of the document. The definitions are collected for reference and does not need to be read at once in one setting. For the first read, the reader may read the definitions on a moving sofa, convex body, and basic geometry and move on to Section 1.
