A _proposition_ is a statement whose truth immediately follows from discussion, so its proof is omitted. A _corollary_ is a consequence directly followed by a theorem. A _lemma_ is a theorem mainly used to prove another theorem and likely serve no other purpose.

Each subsection comes with an overview, and it suffices to understand and accept the theorems stated in the overview to be true, in order to move on to the next (sub)sections. The full details of the proofs follow after the overview, which an interested reader can read and verify.

We now set up basic notions and conventions that will be used thoroughout the rest of the document. The definitions are collected here for the purpose of reference and does not need to be read in one setting.
