...
Many authors often also include the condition that $K^\circ$ is nonempty, but in our definition we allow $K^\circ$ to be empty (e.g. line segment or point).

In this paper, we use the following terminologies for a planar convex body $K$.
...
It is possible that the edge $e_K(t)$ can be a single point. In such case, the tangent line $l_K(t)$ touches $K$ at the single point $v_K^+(t) = v_K^-(t)$. In fact, this holds for every $t \in S^1$ except for a countable number of values. In connection with existing literature, the vertices $v_K^{\pm}(t)$ are the _extreme points_ of $K$ , and the edges $e_K(t)$ are the _exposed faces_ of $K$.
