Each subsection mostly starts with a general exposition on what it tries to achieve. For clarity, some proofs are delegated at the end of the subsection. A _proposition_ is a statement whose truth immediately follows from discussion, so its proof is omitted. A _corollary_ is a consequence directly followed by a theorem. A _lemma_ is a theorem mainly used to prove another theorem (and likely serve no other purpose).

We now set up basic notations, definitions and conventions that will be used thoroughout the rest of the document. The definitions are collected here for reference and does not need to be read at once in one setting. For first read, the reader may read the definitions on a moving sofa, convex body, and basic geometry and move on.
