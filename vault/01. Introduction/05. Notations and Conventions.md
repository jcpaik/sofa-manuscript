Each subsection starts with a summary, which is sufficient for a reader to read in order to move on to next parts. For clarity of explanation, some technical definitions and proofs are delegated at the end of the subsection, which an interested reader can check if interested. A _proposition_ is a statement whose truth immediately follows from discussion, so its proof is omitted. A _corollary_ is a consequence directly followed by a theorem. A _lemma_ is a theorem mainly used to prove another theorem (and likely serve no other purpose).

We now set up basic notations, definitions and conventions that will be used thoroughout the rest of the document. The definitions are collected here for reference and does not need to be read at once in one setting. For the first read, the reader may read the definitions on a moving sofa, convex body, and basic geometry and move on to Section 1.
