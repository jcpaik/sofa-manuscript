Each subsection starts with a summary which is sufficient for a reader to read in order to move on to next parts. For clarity of explanation, some technical definitions and proofs are delegated at the end of the subsection, which an interested reader can verify if needed. A _proposition_ is a statement whose truth immediately follows from discussion, so its proof is always omitted. A _corollary_ is a consequence directly followed by a theorem. A _lemma_ is a theorem mainly used to prove another theorem (and likely serve no other purpose).
