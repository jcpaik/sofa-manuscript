...
$$
is maximized when $c_t = \tau_K(t)$ for all $t \leq s$ and $c_t = 0$ otherwise. For such $c_t$ we have
$$
\sum_{t \in \Theta^{\diamond}} c_t v_t = A_K^+(s) - A_K^-(0)
$$
as we follow the upper boundary $\delta K$ from $A_K^-(0)$ to $A_K^+(s)$. So we have
$$
