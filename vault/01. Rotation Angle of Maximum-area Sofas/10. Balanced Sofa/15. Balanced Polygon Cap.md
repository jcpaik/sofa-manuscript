...
_Proof._ Any translate $K'$ of an arbitrary $K \in \mathcal{K}_\Theta$ satisfies (1) and (2) immediately. On the other hand, for any $K'$ satisfying conditions (1) and (2), find a translation $K$ of $K'$ so that $h_K(\omega) = h_K(\pi/2) = 1$ and $K \in \mathcal{K}_\Theta$. □
...
> __Proposition [niche-extension-compatible].__ For any polygon cap $K \in \mathcal{K}_\Theta$ with $h := h_K \in \mathcal{H}_\Theta$ we have $\mathcal{N}_\Theta(h) = \mathcal{N}_\Theta(K)$. Consequently, we have $\mathcal{A}_\Theta(h) = \mathcal{A}_\Theta(K)$. ^pro-niche-extension-compatible
