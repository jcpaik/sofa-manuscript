...
> __Theorem [balanced-polygon-sofa-ineq].__ For any maximum polygon cap $K$ with angle set $\Theta$ of rotation angle $\omega < \pi/2$, we have $w_K^{\circ} \leq v_K(\pi/2)$ and $z_K^{\circ} \leq v_K(\omega)$. ^thm-balanced-polygon-sofa-ineq
...
$$
(p - A_K^-(0)) \cdot u_{t_0} = \sum_{t \in \Theta^{\diamond}} c_t (v_t \cdot u_{t_0})
$$
...
$$
(p - A_K^-(0)) \cdot u_{t_0} \leq (A_K^+(t_0) - A_K^-(0)) \cdot u_{t_0}
$$
which implies $p \cdot u_{t_0} \leq A_K^+(t_0) \cdot u_{t_0}$, that in turn implies $p \in g_K(t_0)$ as we desired. □
