...
> 
> 1. For any $t \in \Theta$, we have $\tau_K(t) = \mathcal{H}^1 \left( \partial \mathcal{N}_\Theta(K) \cap b_K(t) \right)$ and $\tau_K(t + \pi/2) = \lambda^1 \left( \partial \mathcal{N}_\Theta(K) \cap d_K(t) \right)$.
> 2. For any $t \in \left\{ \omega, \pi/2 \right\}$ we have $\tau_K(t) + \mathcal{H}^1\left( \partial \mathcal{N}_\Theta(K) \cap l(t, 0) \right) = \sigma_K(t + \pi)$.
...
We will show that the boundary $\partial \mathcal{N}_\Theta(K)$ is a disjoint union of $M, N, P$. Since $F_\omega^\circ$ is open, we have $N = \partial \mathcal{N}_\Theta(K) \cap F_\omega^\circ$. 

Define $D := K \cap \partial F_\omega$, so that $D = e_K(\omega + \pi) \cup e_K(3\pi/2)$.

We will show $\mathbf{p}_K = N \cup (D \setminus M)$. 
...
$$
holds because $M, P, D \subseteq \partial F_\omega$ are not relevant.
...
$$
by applying [[01. Rotation Angle of Maximum-area Sofas/10. Balanced Sofa/01. Nef Polygon#^thm-simple-nef-polygon]] to the simple Nef polygon $\mathcal{C}_\Theta(h)$ ([[01. Rotation Angle of Maximum-area Sofas/10. Balanced Sofa/15. Balanced Polygon Cap#^pro-cap-niche-nef-polygons]]). We also have 
$$
...
Now consider the case $t \in \left\{ \omega, \pi/2 \right\}$. We have
$$
|\mathcal{C}_\Theta(h^+)| = |\mathcal{C}_\Theta(h)| + (\sigma_K(t) - \sigma_K(t + \pi) ) \epsilon + O(\epsilon^2)
$$
by applying [[01. Rotation Angle of Maximum-area Sofas/10. Balanced Sofa/01. Nef Polygon#^thm-simple-nef-polygon]] twice to $\mathcal{C}_\Theta(h)$, first pushing the boundary $l(t, h(t))$ towards $l(t, h^+(t))$ by $\epsilon$ while keeping the line $l(t, h(t) - 1)$ intact, and then pusing the line $l(t, 1 - h(t))$ towards $l(t, 1 - h^+(t))$ by $\epsilon$ while keeping the line $l(t, h^+(t))$ intact. Since the two lines are parallel and of distance 1, the two applications of [[01. Rotation Angle of Maximum-area Sofas/10. Balanced Sofa/01. Nef Polygon#^thm-simple-nef-polygon]] do not intefere with each other. We also have
$$
...
%% TODO: hygiene bad here %%

Choosing the right angle $t \in \Theta^\diamond$ in [[01. Rotation Angle of Maximum-area Sofas/10. Balanced Sofa/15. Balanced Polygon Cap#^lem-not-balanced-positive]] guarantees that the new function $h^+ \in \mathcal{H}_\Theta$ with $h^+(t) := h(t) + \epsilon$ corresponds to a cap translate $K^+$.
...
> 
> Then the intersection $K^+ := \mathcal{C}_\Theta(h^+)$ is a polygon cap translate in $\mathcal{K}_\Theta^\mathrm{t}$. Also, $h^+$ is the supporting function $h_{K^+}$ of $K^+$.
