...
> __Definition [uniform-angle-set].__ Define the _uniform angle set_ $\Theta_{n}^{\omega}$ of $n$ intervals with rotation angle $\omega \in (0, \pi/2]$ as $\Theta_{n}^{\omega} := \left\{ i / n\omega : 1 \leq i < n \right\}$. ^def-uniform-angle-set
...
> 2. For each $n = n_i$ with $i \geq 1$, there exists a maximum polygon cap $K_n$ with uniform angle set $\Theta_i := \Theta_n^\omega$.
