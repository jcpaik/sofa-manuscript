...
For the space of polygon sofas, the root node is the space of cap. Coordinate can be chosen as $p$. 
...
> __Definition [frontline].__ The _polyline_ $\mathbf{p}$ is a finite sequence $\mathbf{p} = i_0, \dots, i_m$ of indices with integers $-n < i_j < n$ such that the followings hold. ^def-polyline
> 
> 1. $i_0 = i_m = 0$.
