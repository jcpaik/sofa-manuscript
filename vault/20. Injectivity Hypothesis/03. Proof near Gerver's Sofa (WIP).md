Here, we show that for any cap $K$ with rotation angle $\pi/2$ that is sufficiently close to the cap $K_G$ of Gerver's sofa, then either $K$ is well-behaved  holds for caps sufficiently close to the cap $K_G$ of . This can be seen as the proof [[01. Statement#^con-injectivity]]. Using this, we suggest a theoretical possibility of proving Gerver's conjecture. More precisely, we show that if Gerver's conjecture is true _a priori_, then by running a variant of the branch-and-bound algorithm for sufficiently large yet finite time, the algorithm can produce a proof of Gerver's conjecture.
