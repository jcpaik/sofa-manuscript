...
> __Lemma [niche-segment-disjoint].__ $T_K(t)$ and the line segment connecting $W_K(t)$, $A^-_K(0)$ are disjoint. So line segment connecting $W_K^\circ$ and $A^-_K(0)$ is disjoint from $\mathcal{N}(K)$. IF $\mathcal{N}(K)$ is nonempty, then the segments A and B are disjoint. IF $\mathcal{N}(K)$ is empty, then $\mathcal{A}(K)$ can't be maximum. ^lem-niche-segment-disjoint
