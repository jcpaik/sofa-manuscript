...
Define $U$ as the following value. Note that the second equality holds by [[old/xb. Upper Bound A1/25. Upper Bound A1/03. Boundary measure#^thm-surface-area-measure-vertex]].
$$
U = (C^-_K(t_{i+1}) - A^-_K(t_{i})) \cdot v_{i+1} = \int_{[t_i, t_{i+1}+\pi/2]} \cos(u - t_{i+1}) \, \beta_K(du)
$$
Observe that $L \leq U \cdot (1/\cos(t_{i+1} - t_i))$ because $f(u) \leq \cos(u - t_{i+1}) / \cos(t_{i+1} - t_i)$ for all $u \in [t_i, t_{i+1}  + \pi/2]$. Then use [[old/xd. Injectivity Hypothesis/xx. Bounding Arm Lengths (crude, old)#^lem-vertex-approximation]] to bound $U$ further from above by $(C_i - A_i) \cdot v_{t_{i+1}}$. □
...
$$
The equality holds by [[old/xb. Upper Bound A1/25. Upper Bound A1/03. Boundary measure#^thm-surface-area-measure-vertex]]. Now use [[old/xd. Injectivity Hypothesis/xx. Bounding Arm Lengths (crude, old)#^lem-vertex-approximation]] to bound it from below. □
