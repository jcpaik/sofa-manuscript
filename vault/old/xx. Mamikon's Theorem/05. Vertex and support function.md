Here, we prove properties of the support function $p_K$ ([[old/xb. Upper Bound A1/10. Notations and conventions/00. Preface#^def-support-function]]) and the vertices $v_K^{\pm}(t)$ ([[old/xb. Upper Bound A1/10. Notations and conventions/00. Preface#^def-convex-body-vertex]]) of a convex body $K$.
...
Note that the sum used in [[old/xx. Mamikon's Theorem/05. Vertex and support function#^def-convex-body-linear]] is the Minkowski sum of convex bodies. That is, $aK = \left\{ ap : p \in K \right\}$ and $K_1 + K_2 = \left\{ p_1 + p_2 : p_1 \in K_1, p_2 : K_2 \right\}$.
...
_Proof._ First condition of [[old/xx. Mamikon's Theorem/05. Vertex and support function#^def-convex-body-linear]] on $p_K$ follows from a direct argument.
$$
...
\end{align*}
$$
so the second condition of [[old/xx. Mamikon's Theorem/05. Vertex and support function#^def-convex-body-linear]] is true. □

We will soon show that the vertex $v_K^{+}(t)$ (resp. $v_K^-(t)$) is right-continuous (resp. left-continuous) respect to $t$ and is linear with respect to $K$. To do so, we compute the limit of vertices via [[old/xx. Mamikon's Theorem/05. Vertex and support function#^thm-limits-converging-to-vertex]].
...
_Proof._ The point $p = v_K(t_1, t_2)$ is the unique point such that $p \cdot u_{t_1} = p_K(t_1)$ and $p \cdot u_{t_2} = p_K(t_2)$ holds. By solving the linear equations, observe that the coordinates of $p$ are linear combinations of $p_K(t_1)$ and $p_K(t_2)$. By [[old/xx. Mamikon's Theorem/05. Vertex and support function#^pro-support-function-linear]] the result follows. □
...
Let $\epsilon > 0$ be arbitrary. Let $p = v_K^+(t) + \epsilon v_t$. Then by the definition of $v_K^+(t)$ the point $p$ is not in $K$. As $\mathbb{R}^2 \setminus K$ is open, any sufficiently small open neighborhood of $p$ is disjoint from $K$, so we can take some positive $\epsilon' < \epsilon$ such that the closed line segment connecting $p$ and $q = p - \epsilon' u_t$ is disjoint from $K$ as well. Define the closed right-angled triangle $T$ with vertices $v_K^+(t)$, $p$, and $q$. Take the line $l$ that passes through both $q$ and $v_K^+(t)$. Call the two closed half-planes divided by the line $l$ as $H_T$ and $H'$, where $H_T$ is the half-plane containing $T$ and $H'$ is the other one. By definition of $H'$, the half-plane $H'$ contains $v_K^+(t)$ and $q$ on its boundary and does not contain the point $q$. And consequently $H'$ has normal angle $t' \in (t, t + \pi/2)$ ([[old/xb. Upper Bound A1/10. Notations and conventions/00. Preface#^def-half-plane]]) because $p = v_K^+(t) + \epsilon v_t$ and $q = p - \epsilon' u_t$.
...
The vertex $v_K^+(t)$ is right-continuous by [[old/xx. Mamikon's Theorem/05. Vertex and support function#^thm-limits-converging-to-vertex]].
...
From [[old/xx. Mamikon's Theorem/05. Vertex and support function#^lem-tangent-lines-intersection-linear]] and [[old/xx. Mamikon's Theorem/05. Vertex and support function#^thm-limits-converging-to-vertex]], we have the linearlity of vertices $v_K^{\pm}(t)$.
