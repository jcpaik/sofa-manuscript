...
> __Theorem [parametrization].__ Let $K$ be any convex body with nonempty interior. For any point $p \in \partial K$ there is a unique positively-oriented arc-length parametrization $\mathbf{x}_{K, p} : [0, B_K] \to \mathbb{R}^2$ of the boundary $\partial K$ as a Jordan arc such that $\mathbf{x}_{K, p}(0) = p$. Consequently, for any two different points $p, q \in \partial K$ there is a unique Jordan arc $\mathbf{x}_{K, p, q}$ which is a restriction of $\mathbf{x}_{K, p}$. Moreover, we have $\mathbf{x}_{K, v^+_K(t_0)} = \mathbf{x}_{K, t_0}$ and $\mathbf{x}_{K, v^+_K(t_0), v^+_K(t_1)} = \mathbf{x}_{K, t_0, t_1}$. ^thm-parametrization

_Proof._ [[02. Boundary of Convex Bodies#^thm-parametrization-positive-jordan]] shows that $\partial K$ is a Jordan curve with length $B_K$. So for any point $p \in \partial K$ there is a unique positively-oriented arc-length parametrization $\mathbf{x}_{K, p} : [0, B_K] \to \mathbb{R}^2$ of the boundary $\partial K$ as a Jordan arc such that $\mathbf{x}_{K, p}(0) = p$. TODO TODO TODO

$\mathbf{x}_{K, t_0}$ is a positively oriented Jordan arc parametrization of $\partial K$. So all positively oriented Jordan arc parametrization is a cyclic shift of $\mathbf{x}_{K, t_0}$. □
