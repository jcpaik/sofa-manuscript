...
_Proof._ The product $\mathcal{K} \times \mathcal{K} \times \mathcal{K}$ of convex spaces $\mathcal{K}$ is naturally a convex space. Recall that $\mathcal{K}^\mathrm{i}$ is a convex subspace of $\mathcal{K}$. That $B \subseteq K$ (resp. $D \subseteq K$) can be written as linear inequalities $h_B(t) \leq h_K(t)$ (resp. $h_D(t) \leq h_K(t)$) for all $t \in S^1$. So the condition (1) of [[03. Optimality of Gerver's Sofa/15. Upper Bound Q/05. Definition of Q#^def-cap-tail-space]] is linear. □
