In this [[03. Optimality of Gerver's Sofa/12. Gerver's Sofa/00. Preface]], we follow the derivation of Gerver's sofa $G$ by Romik (Section 4 of [@romik]) and extract the properties of $G$ that will be needed in the next [[03. Optimality of Gerver's Sofa/15. Quadratic Upper Bound/00. Preface]].

> __Figure [gerver-curves].__ Gerver's sofa $G$ is determined by five curves $\mathbf{A}, \mathbf{B}, \mathbf{C}, \mathbf{D}$, and $\mathbf{x}$ solved in [@romik]. ^fig-gerver-curves
> 
> ![](images/gerverCurves.svg)

In Romik's derivation, Gerver's sofa $G$ is a monotone sofa with the boundary parametrized by five curves $\mathbf{A}, \mathbf{B}, \mathbf{C}, \mathbf{D}$, and $\mathbf{x}$ (see [[03. Optimality of Gerver's Sofa/12. Gerver's Sofa/00. Preface#^fig-gerver-curves]]). The five curves are determined by a finite number of parameters, that satisfy the system of Equations 27 to 44 in [@romik]. By solving the system of equations with `Mathematica` with respect to , he determines the five curves $\mathbf{A}, \mathbf{B}, \mathbf{C}, \mathbf{D}, \mathbf{x}$. and thus $G$ completely.
...
> __Theorem [gerver-monotone].__ Gerver's sofa $G$ is a monotone sofa. Let $K := \mathcal{C}(G)$ be the cap of $G$. Then the cap $K$ have vertices $A_K(t) = \mathbf{A}(t)$ and $C_K(t) = \mathbf{C}(t)$. The niche $\mathcal{N}(K)$ is the region enclosed by the counterclockwise Jordan curve which is the concatenation of the following curves in order. ^thm-gerver-monotone
> 
> 1. The curve $\mathbf{D} : [0, \theta] \to \mathbb{R}^2$. 
> 2. 

We do not prove [[03. Optimality of Gerver's Sofa/12. Gerver's Sofa/00. Preface#^thm-gerver-monotone]] rigorously here. However, experts agree that this is the right derivation. 
