...
The niche $\mathcal{N}(K)$ is also paritioned into three parts $\mathcal{N}(K) \cap H_K^\mathrm{R}$, $\mathcal{N}(K) \cap H_K^\textrm{L}$ and $\mathcal{N}(K) \setminus H_K^\mathrm{R} \setminus H_K^\mathrm{L}$. We bound the area of each part from below. The right and left part $\mathcal{N}(K) \cap H_K^\mathrm{R}$ and $\mathcal{N}(K) \cap H_K^\textrm{L}$ are disjoint from the following convex bodies below.

> __Definition [left-right-regions].__ Take arbitrary cap $K \in \mathcal{K}^\mathrm{c}$. Define the convex body ^def-left-right-regions
$$
...
$$
> which is from $W_K^\mathrm{R} := v_B$ to  Similarly, define the convex body
$$
D_K := K \cap \bigcap_{t \in [0, \varphi_L]} H_K^\textrm{d}(t)
$$
> and its convex curve
$$
