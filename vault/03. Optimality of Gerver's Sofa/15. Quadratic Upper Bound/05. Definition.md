...
_Proof._ Recall that from XX, the curve $\mathbf{r}_B := ??$ is the segment of $\partial B$ from the vertex $X_B := v_B^+(\varphi^\textrm{R} + \pi)$ of $B$ to the vertex $W_B := v_B^-(3\pi/2)$ of $B$. Also, we have equality $W_K^\mathrm{R} = v_B\left( \pi + \varphi^\mathrm{R}, 3\pi/2 \right)$ from .
