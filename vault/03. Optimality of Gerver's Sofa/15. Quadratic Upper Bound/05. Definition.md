...
> __Definition [cap-left-right].__ For any $K \in \mathcal{K}$, define the line $l_K^\mathrm{R} := \overline{b}_K(\varphi^\mathrm{R})$ and the half-plane $H_K^\mathrm{R} := H_K^\mathrm{b}(\varphi^\mathrm{R})$ bounded by the line $l_K^\mathrm{R}$ with normal angle $\varphi^\mathrm{R} + \pi$. Define the points $W_K^\mathrm{R} := W_K(\varphi^\mathrm{R})$ and $\mathbf{x}_K^\mathrm{R} := \mathbf{x}_K(\varphi^\mathrm{R})$ on the line $l_K^\mathrm{R}$.
> 
> Similarly, define the line $l_K^\mathrm{L} := \overline{d}_K(\varphi^\mathrm{L})$ and the half-plane $H_K^\mathrm{R} := H_K^\mathrm{d}(\varphi^\mathrm{L})$ bounded by the line $l_K^\mathrm{L}$ with normal angle $\varphi^\mathrm{L} + \pi$. Define the points $Z_K^\mathrm{L} := Z_K(\varphi^\mathrm{L})$ and $\mathbf{x}_K^\mathrm{L} := \mathbf{x}_K(\varphi^\mathrm{L})$ on the line $l_K^\mathrm{L}$. ^def-cap-left-right
...
_Proof._ Fix such $K$. Because $K \subseteq H_K(\varphi^\mathrm{R})$, the strip $H_K(\varphi^\textrm{R}) \cap H_K^\mathrm{R}$ of unit width contains $K \cap H_K^\mathrm{R}$. So $K \cap H_K^\mathrm{R}$ is contained in the paralleogram $P^\mathrm{R} := H \cap H_K(\varphi^\mathrm{R}) \cap H_K^\mathrm{R}$ of angle $\varphi^\mathrm{R}$ and unit heights. Likewise, $K \cap H_K^\mathrm{L}$ is contained in the parallelogram $P^\mathrm{L} := H \cap H_K(\pi/2 + \varphi^\mathrm{L}) \cap H_K^\mathrm{L}$. 

Now assume by contradictory that $K \cap H_K^\mathrm{R}$ and $K \cap H_K^\mathrm{L}$ intersect. Then $P^\mathrm{R}$ and $P^\mathrm{L}$ should intersect as well. The length of the side of two parallelograms on the line $l(\pi/2, 1)$ is $s := \sec \varphi$ (we use $\varphi^\mathrm{R} = \varphi$ and $\varphi^\mathrm{L} = \pi/2 - \varphi$ here). Note that $K$ is contained in the trapezoid $R := H \cap H_K(\varphi^\mathrm{R}) \cap H_K(\pi/2 + \varphi^\mathrm{L})$ containing $P^\mathrm{R}$ and $P^\mathrm{L}$. Since $P^\mathrm{R}$ and $P^\mathrm{L}$ overlaps, the side length of $R$ on the line $l(\pi/2, 1)$ is at most $2s$, and the side length of $R$ on the line $l(\pi/2, 0)$ is at most $2s + 2 \tan \varphi$. Now the area of $R$ is at most $2s + \tan \varphi = \dots < 2.2$, so $|K| \leq |R| < 2.2$ and we get contradiction. □
...
> __Definition [right-left-body].__ Take arbitrary cap $K \in \mathcal{K}^\mathrm{c}$. Define the convex body ^def-right-left-body
$$
