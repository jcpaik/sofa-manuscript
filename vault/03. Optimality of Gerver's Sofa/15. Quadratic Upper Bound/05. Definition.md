...
> 3. $v_B(\pi + t) = \mathbf{B}(t)$ on $t \in [0, \theta]$, and $v_B(\pi + t) = \mathbf{B}(\theta)$ on $t \in [\theta, \varphi^\mathrm{R}]$.
> 4. On the interval 

_Proof._ 

- Need a way to say the radius of curvature of $\mathbf{B}$ and $\mathbf{D}$.
- $\breve{r}_B$ and $\breve{s}_D$.
	- Can I use it? No without proof.
- For general $K \in \mathcal{K}^\mathrm{i}$, it may not be the case that curvature of $\mathbf{B}$ and $\mathbf{D}$ will well-behave.
- The curvature _happens_ to be thresholded by theta.
...
- $\mathbf{D} \subseteq \mathcal{N}(K) \subseteq K$.
	...
	- Attempt using $D'$.
		- Now define $D'$ as the intersection of $K \cap H_K^\mathrm{L}$ and the region carved out by $\mathbf{D}$.
			- How to define such a thing?????
			- We can guarantee $\mathbf{u}_{D'}$ is equal to $\mathbf{D}$.
		- Want to show $D' = D_K$. 
		- Injectivity hypothesis lemma [[03. Optimality of Gerver's Sofa/15. Quadratic Upper Bound/05. Definition#^lem-monotonicity-intervals]]
			- The curve $\mathbf{D}$ itself is disjoint from $\mathcal{N}(K)$.
			- So $\mathbf{D}$ and thus $D'$ contained in all $H_K^\mathrm{d}(t)$.
				- So $D' \subseteq D_K$.
	- We show that $\mathbf{l}_D := \mathbf{u}_D^{\varphi^\mathrm{R}, 3\pi/2}$ is equal to $\mathbf{D}$ exactly.
