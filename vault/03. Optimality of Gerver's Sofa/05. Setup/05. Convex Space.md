...
> __Remark [convex-linear-convention].__ Note that [[#^def-convex-linear]], [[03. Optimality of Gerver's Sofa/05. Setup/05. Convex Space#^def-convex-bilinear]], and [[03. Optimality of Gerver's Sofa/05. Setup/05. Convex Space#^def-convex-space-quadratic]] requires _affine_-linearity instead of the usual linearity on vector spaces. That is, the maps $f, g, h$ are allowed to not preserve the origin, even if the convex domain $\mathcal{V}$ is a vector space. ^rem-convex-linear-convention
