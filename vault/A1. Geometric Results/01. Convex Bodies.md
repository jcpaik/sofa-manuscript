This section proves necessary tools for convex bodies in a plane $\mathbb{R}^2$. Recall that a convex body $K$ is a nonempty compact subset of $\mathbb{R}^2$ which is an intersection of half-planes ([[01. Notations and Conventions#^def-half-plane]]). We defined its [[01. Notations and Conventions#^def-support-function|support function]] $p_K$, [[01. Notations and Conventions#^def-tangent-line|tangent lines]] $l_K(t)$, [[01. Notations and Conventions#^def-half-plane|half-planes]] $H_K(t)$, vertices $v_K^\pm (t)$, and edges $e_K(t)$ ([[01. Notations and Conventions#^def-convex-body-edge]]).
...
The first equality ues [[#^def-boundary-measure]]. The second and third equality uses [[01. Notations and Conventions#^cor-product-rule-differential]]. The rest uses linearity and definition. The equality
$$
...
We end by showing that multiple values attached to a convex body $K$ is linear in terms of $K$ ([[02. Geometric Results#^def-convex-body-linear]]).

_Proof._ (of [[02. Geometric Results#^thm-convex-body-linear]]) The support function $p_K$ is linear in $K$. If $t_1, t_2 \in S^1$ are two different angles, the intersection $p = l_K(t_1) \cap l_K(t_2)$ is linear in $K$ because $p$ is the unique point satisfying $p \cdot u_{t_1} = p_K(t_1)$ and $p \cdot u_{t_2} = p_K(t_2)$. The vertices $v_K^+$ and $v_K^-$ are linear in $K$ because they are the limits of intersection points as in [[02. Geometric Results#^def-convex-body-linear]]. The boundary measure $\beta_K$ is linear in $K$ because $v_t \cdot d v_K^+(t) = \beta_K(dt)$. □