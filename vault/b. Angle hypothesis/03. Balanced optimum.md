...
_Proof._ Note that if $K \in \mathcal{K}_\Theta$, then we have $F_K = F_\omega$ so [[b. Angle hypothesis/03. Balanced optimum#^def-cap-translate-niche]] agrees with [[15. Monotone sofas/10. Monotonization#^def-niche]], justifying the same notion $\mathcal{N}(K)$ used in [[b. Angle hypothesis/03. Balanced optimum#^def-cap-translate-niche]]. □
