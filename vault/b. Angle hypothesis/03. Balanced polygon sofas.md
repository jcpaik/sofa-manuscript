...
_Proof._ Note that if $K \in \mathcal{K}_\Theta$, then we have $F_K = F_\omega$ so [[b. Angle hypothesis/03. Balanced polygon sofas#^def-cap-translate-niche]] agrees with [[15. Monotone sofas/10. Monotonization#^def-niche]], justifying the same notion $\mathcal{N}(K)$ used in [[b. Angle hypothesis/03. Balanced polygon sofas#^def-cap-translate-niche]]. □
...
The first step is to prove the balancedness condition for polygonal sofas ([[b. Angle hypothesis/03. Balanced polygon sofas#^thm-polygon-balanced-angled]]).
...
$$
in [[b. Angle hypothesis/03. Balanced polygon sofas#^thm-polygon-balanced-angled]], we obtain the following equality for sufficiently small $\epsilon > 0$.
$$
...
$$
in [[b. Angle hypothesis/03. Balanced polygon sofas#^lem-perturbation]], we obtain the following equality.
$$
