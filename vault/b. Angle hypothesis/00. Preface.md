...
A powerful idea described in [[@gerverMovingSofaCorner1992]] is that a moving sofa is essentially the intersection of rotating _hallways_, if we look at the movement of $S$ inside $L$ in the perspective of $S$. Recall that a moving sofa $S$ of maximum area can be assumed to be a _monotone sofa_ which is the intersection of the following sets.
