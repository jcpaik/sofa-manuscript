...
> __Lemma [lower-bound-sequence].__ For any balanced maximum cap $K \in \mathcal{K}$, we have $f_K(t) \geq f_n(t)$ for all $t \in [0, \pi/2)$ and $h(t) \geq f_n(\pi/2 - t)$ for all $t \in (0, \pi/2]$. ^lem-lower-bound-sequence
