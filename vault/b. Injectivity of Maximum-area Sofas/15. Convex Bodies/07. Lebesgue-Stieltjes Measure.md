...
Here, we will define the _Lebesgue-Stieltjes measure_ $\mathrm{d} f$ of a real-valued function $f$ ([[b. Injectivity of Maximum-area Sofas/15. Convex Bodies/07. Lebesgue-Stieltjes Measure#^def-lebesgue-stieltjes]]), so that we can state informal calculations involving differentials like $d(t^2) = 2t \, dt$ rigorously, like $\textrm{d}(t^2) = 2t \, \textrm{d}t$. Unlike differentials $df$, the measure $\mathrm{d} f$ has a clear interpretation and also works for functions $f$ that may not be continuous. We will use this in particular to prove equalities on general convex bodies.
...
We extend [[b. Injectivity of Maximum-area Sofas/15. Convex Bodies/07. Lebesgue-Stieltjes Measure#^def-bounded-variation]] to half-open intervals.
...
We use [[b. Injectivity of Maximum-area Sofas/15. Convex Bodies/07. Lebesgue-Stieltjes Measure#^def-lebesgue-stieltjes]] to state informal calculations of differentials like $d(t^2) = 2t \, dt$ rigorously as $\textrm{d}(t^2) = 2t \, \textrm{d}t$. Here, the variable $t \in I$ parametrizes the domain $I := (a, b]$. Note the followings.
...
With [[b. Injectivity of Maximum-area Sofas/15. Convex Bodies/07. Lebesgue-Stieltjes Measure#^def-lebesgue-stieltjes-integral]], the Lebesgue-Stieltjes measure $\textrm{d} f$ converts to the differential $df$ when integrated. A measure $\mu$ which is not Lebesgue-Stieltjes, on the other hand, converts to the differential $\mu(dt)$ when integrated.

The map $f \mapsto \textrm{d} f$ is linear like differentials would do.

> __Theorem [lebesgue-stieltjes-sum].__ For any right-continuous $f, g : (a, b] \to \mathbb{R}$ of bounded variation and real values $r, s \in \mathbb{R}$, we have $\textrm{d} (r f + s g) = r \, \textrm{d} f + s \, \textrm{d} g$ as signed measures on $I$. ^thm-lebesgue-stieltjes-sum

The product rule $\textrm{d}(fg) = g \, \textrm{d}f + f \, \textrm{d}g$ is more subtle. We need one of $f$ or $g$ to be continuous.
