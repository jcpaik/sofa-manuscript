...
$$
\textrm{d} f_K^+(t) = g_K^+(t)\, \textrm{d} t + \sigma_K^\textrm{R}
$$
> where $f_K^+(t), g_K^+(t), t$ are .

_Proof._  □

TODO: what is $\sigma_K(dt + \pi/2)$ ???? Do we need to state $\textrm{d} g_K^+(t) = f_K^+(t)\, \textrm{d} t + \beta_K$ ?????