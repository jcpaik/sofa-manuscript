...
Note that in [[b. Injectivity of Maximum-area Sofas/20. Injectivity of Balanced Maximum Sofa/10. Proof of Injectivity#^def-o-notation]], while the value of $g(\delta)$ should depend solely on $\delta$, the expression $f(\delta)$ might depend on other values than $\delta$ like an arbitrary balanced polygon cap $K$. For example, we can write $\delta^2 / (1 + p_K(0)^2) = O_\delta(\delta^2)$ where $K \in \mathcal{K}$ can be arbitrary as the value $\delta^2$ inside $O_\delta$ is a function solely on $\delta$, but we cannot write $\delta = O_\delta(\delta(1+p_K(0)^2))$ as the value inside $O_\delta$ depends on $K \in \mathcal{K}$.
