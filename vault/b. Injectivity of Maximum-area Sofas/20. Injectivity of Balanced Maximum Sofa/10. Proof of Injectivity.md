...
_Proof._ We first show that there is a unique differentiable function $f : [0, \pi/2] \to \mathbb{R}$ that satisfies the time-reversal delayed ODE
$$
\begin{gather*}
f(0) = 1 \\
f'(x) = m(f(\pi/2-x))
\end{gather*}
$$
Let $e(x) = f(\pi/2 - x)$. Then we have a set of ODEs
