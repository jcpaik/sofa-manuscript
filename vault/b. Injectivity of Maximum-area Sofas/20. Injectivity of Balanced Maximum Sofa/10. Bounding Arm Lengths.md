...
> __Lemma [arm-length-discrete-bound].__ Let $K$ be any balanced polygon cap with $n$ steps of step size $\delta := (\pi/2) / n$ and angle set $\Theta := \Theta_n$. For any angle $t \in \Theta$ and open interval $I := (t - \delta, t)$, the followings are true. ^lem-arm-length-discrete-bound
> 
> 1. For any $t' \in I$, we have $f_K^+(t - \delta) \leq f_K^{+}(t') = f_K^-(t') \leq f_K^-(t)$.
> 2. Moreover, the gap $f_K^-(t) - f_K^+(t - \delta)$ between the lower and upper bound is at most $5 \delta$.
...
$$
for all $t' \in I$. Call the common point $C$. Take any $t' \in \overline{I} = [t - \delta, t]$. The lines $a_K(t')$ and $c_K(t')$ meet orthogonally at the point $\mathbf{y}_K(t')$. The line $a_K(t')$ passes through $A$, and the line $c_K(t')$ passes through $C$. So by elementary geometry, the trajectory of the point $\mathbf{y}_K(t')$ over all $t' \in \overline{I}$ forms an arc $\Delta$ of the circle $\Gamma$ with the diameter of length $\leq 5$ connecting $A$ and $C$. The value $f_K^+(t - \delta)$ is the distance from $A$ to $\mathbf{y}_K(t - \delta)$, and $f_K^-(t)$ is the distance from $A$ to $\mathbf{y}_K(t)$. As the lines $a_K(t - \delta)$ and $a_K(t)$ makes an angle of $\delta$ at point $A$, the arc $\Delta$ have central angle $2\delta$. So the distance between $\mathbf{y}_K(t - \delta)$ and $\mathbf{y}_K(t)$ is at most $5 \delta$.  Yatta yatta. □
...
> __Figure [leg-computation].__ The hallway $L_K(t)$ depicted in upright position with sides $a_K(t \pm \delta)$ and $\overline{b}_K(t \pm \delta)$ for the proof of [[old/xd. Injectivity Hypothesis/15. Injectivity Hypothesis/88. Right-Lipschitz h#^lem-leg-computation]]. ^fig-leg-computation
> 
...
> 2. $\mathcal{H}^1(d_K(t) \cap H_K^\textrm{b}(t - \delta)) = \tan \delta \cdot \max(0, 1 - f_K^-(t) + \tan(\delta / 2))$

_Proof._ (1) (See the left side of [[old/xd. Injectivity Hypothesis/15. Injectivity Hypothesis/88. Right-Lipschitz h#^fig-leg-computation]]) Let $p$ be the intersection of $\overline{b}_K(t + \delta)$ and $\overline{d}_K(t)$. Since $p \in \overline{d}_K(t)$ there is a unique real value $\alpha$ such that $p + \alpha u_t = \mathbf{x}_K(t)$. As $d_K(t)$ is the half-line starting at $\mathbf{x}_K(t)$ in the direction of $-u_t$, we now have $\mathcal{H}^1(d_K(t) \cap H_K^\textrm{b}(t + \delta)) = \max(\alpha, 0)$ and it remains to evaluate $\alpha$. Define $q$ as the intersection of $\overline{b}_K(t + \delta)$ and $\overline{b}_K(t)$, and $\beta$ as the unique real value such that $q + \beta v_t = \mathbf{x}_K(t)$. Since the points $p$, $q$, $\mathbf{x}_K(t)$ form a right-angled triangle of angle $\delta$ at $q$, we have $\alpha = \tan \delta \cdot \beta$. It remains to compute $\beta$.
...
$$
\begin{align*}
\beta & = (\mathbf{x}_K(t) - q) \cdot v_t = (\mathbf{x}_K(t) + u_t + \tan(\delta/2) v_t - r) \cdot v_t \\
...
(2) (See the right side of [[old/xd. Injectivity Hypothesis/15. Injectivity Hypothesis/88. Right-Lipschitz h#^fig-leg-computation]]) The proof is analogous to that of (1). Let $p$ be the intersection of $\overline{b}_K(t - \delta)$ and $\overline{d}_K(t)$. Since $p \in \overline{d}_K(t)$ there is a unique real value $\alpha$ such that $p + \alpha u_t = \mathbf{x}_K(t)$. As $d_K(t)$ is the half-line starting at $\mathbf{x}_K(t)$ in the direction of $-u_t$, we now have $\mathcal{H}^1(d_K(t) \cap H_K^\textrm{b}(t - \delta)) = \max(\alpha, 0)$ and it remains to evaluate $\alpha$. Define $q$ as the intersection of $\overline{b}_K(t - \delta)$ and $\overline{b}_K(t)$, and $\beta$ as the unique real value such that $\mathbf{x}_K(t) + \beta v_t = q$. Since the points $p$, $q$, $\mathbf{x}_K(t)$ form a right-angled triangle of angle $\delta$ at $q$, we have $\alpha = \tan \delta \cdot \beta$. It remains to compute $\beta$.
...
> __Lemma [balanced-discrete-ineq].__ Let $K \in \mathcal{K}_\Theta$ be any balanced polygon cap of $n$ steps of step size $\delta = (\pi/2) / n$ and uniform angle set $\Theta := \Theta_n$. Take any $t \in \Theta$. We have the following upper bound of $\sigma_K(t + \pi/2)$. ^lem-balanced-discrete-ineq
$$
\sigma_K(t + \pi/2) \leq k(f_K^-(t)) \delta + O_\delta(\delta^2)
$$

_Proof._ Write $s := \sigma_K(t + \pi/2)$. Recall that the balancedness condition of $K$ states $s = \nu_K(t + \pi/2)$, where $\nu_K(t + \pi/2)$ is the length $\mathcal{H}^1(X)$ of the set $X := d_K(t) \cap \partial \mathcal{N}_\Theta(K)$ which is the side(s) of niche $\mathcal{N}(K)$ contributed by the half-line $d_K(t)$ form $\mathbf{x}_K(t)$. We bound $\mathcal{H}^1(X)$ from above. 

Define the set $U := \left\{ t - \delta, t, t + \delta \right\}$ and  $R := \bigcup_{u \in U} g_K^b (u)$ be the union of three half-planes. Divide $X$ into $X \cap R$ and $X \setminus R$. We bound $\mathcal{H}^1(X \cap R)$ and $\mathcal{H}^1(X \setminus R)$ separately.
...
$$
by [[b. Injectivity of Maximum-area Sofas/20. Injectivity of Balanced Maximum Sofa/10. Bounding Arm Lengths#^lem-arm-length-discrete-bound]].
