...
> 
> 1. For any $t' \in I$, we have $g_K^-(t) \leq g_K^{+}(t') = g_K^-(t') \leq g_K^+(t - \delta)$.
> 2. Moreover, the gap $g_K^+(t - \delta) - g_K^-(t)$ between the lower and the upper bound is at most $5 \delta$.
...
> __Figure [leg-computation].__ The hallway $L_K(t)$ depicted in upright position with sides $c_K(t \pm \delta)$ and $\overline{d}_K(t \pm \delta)$ for the proof of [[old/xd. Injectivity Hypothesis/15. Injectivity Hypothesis/88. Right-Lipschitz h#^lem-leg-computation]]. ^fig-leg-computation
> 
...
> 
> 1. $\mathcal{H}^1(b_K(t) \cap H_K^\textrm{d}(t - \delta)) = \tan \delta \cdot \max(0, 1 + g_K^-(t) - \tan(\delta / 2))$
> 2. $\mathcal{H}^1(b_K(t) \cap H_K^\textrm{d}(t + \delta)) = \tan \delta \cdot \max(0, 1 - f_K^-(t) + \tan(\delta / 2))$

TODO: modify this.

_Proof._ (1) (See the left side of [[old/xd. Injectivity Hypothesis/15. Injectivity Hypothesis/88. Right-Lipschitz h#^fig-leg-computation]]) Let $p$ be the intersection of $\overline{d}_K(t - \delta)$ and $\overline{b}_K(t)$. Since $p \in \overline{b}_K(t)$ there is a unique real value $\alpha$ such that $p + \alpha u_t = \mathbf{x}_K(t)$. As $d_K(t)$ is the half-line starting at $\mathbf{x}_K(t)$ in the direction of $-u_t$, we now have $\mathcal{H}^1(d_K(t) \cap H_K^\textrm{b}(t + \delta)) = \max(\alpha, 0)$ and it remains to evaluate $\alpha$. Define $q$ as the intersection of $\overline{b}_K(t + \delta)$ and $\overline{b}_K(t)$, and $\beta$ as the unique real value such that $q + \beta v_t = \mathbf{x}_K(t)$. Since the points $p$, $q$, $\mathbf{x}_K(t)$ form a right-angled triangle of angle $\delta$ at $q$, we have $\alpha = \tan \delta \cdot \beta$. It remains to compute $\beta$.
...
> __Lemma [balanced-discrete-ineq].__ Let $K \in \mathcal{K}_\Theta$ be any balanced polygon cap of $n$ steps with step size $\delta = (\pi/2) / n$ and uniform angle set $\Theta := \Theta_n$. Take any $t \in \Theta$. We have the following upper bound of side length $\sigma_K(t)$. ^lem-balanced-discrete-ineq
$$
\sigma_K(t) \leq k(g_K^+(t)) \delta + O_\delta(\delta^2)
$$
