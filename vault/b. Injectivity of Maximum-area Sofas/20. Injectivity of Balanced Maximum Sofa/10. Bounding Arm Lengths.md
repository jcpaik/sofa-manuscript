...
A balanced maximum cap $K_\infty$ is the limit of _balanced polygon caps_ $K_1, K_2, \dots$ converging to $K$ in Hausdorff distance $d_\text{H}$. Each balanced polygon cap $K_i$ have angle set $\Theta_{n_i}$ with $n_i$ increasing powers of 2. To refer each $K_i$ more easily, we introduce the following notion.
