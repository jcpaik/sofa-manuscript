...
_Proof._ Control lim sup of gn to g??? □
...
> __Lemma [balanced-ineq-limit].__ Let $K$ be a balanced maximum cap, so that $K$ is the limit of balanced polygon caps $K_n$ of $n$ steps with step size $\delta = (\pi/2) / n$ as $n \to \infty$, where $n = n_1, n_2, \dots$ takes values in a strictly increasing powers of two. Then on the interval $t \in [0, \pi/2)$, we have $\sigma_K \leq k(g_K^+(t)) \, \mathrm{d} t$. ^lem-balanced-ineq-limit

_Proof._ It suffices to show ^eqn-to-show-interval
$$
\sigma_K(I) \leq \int_{t \in I} k(g_K^+(t)) \, d t
$$
for any open interval $I = (a, b) \subseteq [0, \pi/2)$ and half-open interval $I = [0, b) \subseteq [0, \pi/2)$. Then for any set $U$ open in the subspace topology of $[0, \pi/2)$, the set $U$ is a countable union of such intervals so $\sigma_K(U) \leq \int_{t \in U} k(g_K^+(t))\,dt$. Since both $\sigma_K$ and $k(g_K^+(t)) \mathrm{d} t$ are outer regular, we have $\sigma_K \leq k(g_K^+(t)) \, \mathrm{d} t$ on $[0,\pi/2)$.

Let $\Theta := \bigcup_n \Theta_n$ be the union of angle sets $\Theta_n$ for all $n = n_1, n_2, \dots$  that are increasing powers of two, so that $\Theta$ is the set of dyadic angles in $(0, \pi/2)$. By taking limits, it suffices to show [[#^eqn-to-show-interval]] for $I = (a, b)$ and $I = [0, b)$ for $a, b \in \Theta$. Take sufficiently large index $i$ and $n$ so that $n = n_j$ for arbitrary $j \geq i$. We can assume $a, b \in \Theta_n$.

Fix an index $i \geq 1$. For any $t \in \left\{ 0 \right\} \cup \Theta_{n_i}$, we have ^eqn-single-step-integral
$$
