...
> __Lemma [balanced-ineq-limit].__ Let $K_{\infty}$ be a balanced maximum cap, so that it is the limit of balanced polygon caps $K_1, K_2, \dots$ with the number of steps $n_1 < n_2 < \dots$ and step sizes $\delta_1 > \delta_2 > \dots$. Then on the interval $t \in [0, \pi/2)$, we have $\sigma_K \leq k(g_K^+(t)) \, \textrm{d} t$. ^lem-balanced-ineq-limit
