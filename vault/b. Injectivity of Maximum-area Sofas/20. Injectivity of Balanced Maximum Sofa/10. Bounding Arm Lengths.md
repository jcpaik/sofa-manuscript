...
\end{align*}
$$
and the result follows from $\mathcal{H}^1(b_K(t) \cap H_K^\textrm{d}(t - \delta)) = \max(\alpha, 0) = \tan \delta \cdot \max(0, \beta)$.

(2) The proof is analogous to that of (1). See the right side of [[old/xd. Injectivity Hypothesis/15. Injectivity Hypothesis/88. Right-Lipschitz h#^fig-leg-computation]].

TODO TODO.

Let $p$ be the intersection of $\overline{b}_K(t - \delta)$ and $\overline{d}_K(t)$. Since $p \in \overline{d}_K(t)$ there is a unique real value $\alpha$ such that $p + \alpha u_t = \mathbf{x}_K(t)$. As $d_K(t)$ is the half-line starting at $\mathbf{x}_K(t)$ in the direction of $-u_t$, we now have $\mathcal{H}^1(d_K(t) \cap H_K^\textrm{b}(t - \delta)) = \max(\alpha, 0)$ and it remains to evaluate $\alpha$. Define $q$ as the intersection of $\overline{b}_K(t - \delta)$ and $\overline{b}_K(t)$, and $\beta$ as the unique real value such that $\mathbf{x}_K(t) + \beta v_t = q$. Since the points $p$, $q$, $\mathbf{x}_K(t)$ form a right-angled triangle of angle $\delta$ at $q$, we have $\alpha = \tan \delta \cdot \beta$. It remains to compute $\beta$.

Let $r$ be the intersection of $a_K(t - \delta)$ and $a_K(t)$. Since the lines $a_K(t)$ and $b_K(t)$ are parallel of distance one, and so are $a_K(t - \delta)$ and $b_K(t - \delta)$, and the two pairs of lines make an angle of $\delta$, we have $r = q + u_t - \tan(\delta / 2) v_t$. As $K$ have angle set $\Theta$ and the angles $t$ and $t - \delta$ are adjacent, we have $r = A_K^-(t)$. By [[b. Injectivity of Maximum-area Sofas/20. Injectivity of Balanced Maximum Sofa/05. Arm Lengths#^lem-cap-tangent-arm-length]] we have $r + f_K^-(t) v_t = \mathbf{y}_K(t)$. Summing up, we now have
$$
\begin{align*}
\beta & = (q - \mathbf{x}_K(t)) \cdot v_t = (- \mathbf{x}_K(t) - u_t + \tan(\delta/2) v_t + r) \cdot v_t \\
& = (-\mathbf{x}_K(t) - u_t + \tan(\delta / 2) v_t + \mathbf{y}_K(t) - f_K^-(t)v_t) \cdot v_t \\
& = 1 - f_K^-(t) + \tan(\delta/2)
\end{align*}
$$
and the result follows from $\mathcal{H}^1(d_K(t) \cap H_K^\textrm{b}(t - \delta)) = \max(\alpha, 0) = \tan \delta \cdot \max(0, \beta)$. □
...
We now bound each side length of a maximum polygon cap. This is the important part of the analysis.
...
$$
\sigma_K(t) \leq k(g_K^+(t)) \cdot \delta + \sigma_K(t + \pi/2) \cdot \delta + O_\delta(\delta^2)
$$

_Proof._ If $t = 0$, then $\sigma_K(t) = 0$ by the definition of $K$ so the inequality holds trivially. So we can assume $t \in \Theta$.

Write $s := \sigma_K(t)$. Recall that the balancedness condition of $K$ states that $s = \nu_K(t)$, where $\nu_K(t) := \mathcal{H}^1(X)$ is the length of the set $X := b_K(t) \cap \partial \mathcal{N}_\Theta(K)$, the side(s) of the niche $\mathcal{N}(K)$ contributed by the half-line $b_K(t)$ form $\mathbf{x}_K(t)$. We bound $\mathcal{H}^1(X)$ from above. Define the set $U := \left\{ t - \delta, t, t + \delta \right\}$ and $R := \bigcup_{u \in U} H_K^\textrm{d} (u)$ as the union of three half-planes. Divide $X$ into $X \cap R$ and $X \setminus R$. We bound $\mathcal{H}^1(X \cap R)$ and $\mathcal{H}^1(X \setminus R)$ separately.

We first bound $\mathcal{H}^1(X \cap R)$ from above. As $X \subseteq b_K(t)$, we have
$$
\mathcal{H}^1(X \cap R) \leq \mathcal{H}^1(b_K(t) \cap R) = \max(b_K(t) \cap H_K^\textrm{d}(t - \delta), b_K(t) \cap H_K^\textrm{d}(t + \delta))
$$
...
\end{align*}
$$
We have $g_K^+(t) - g_K^-(t) = \sigma_K(t + \pi/2)$ by definition. So ^eqn-bound-x-r
$$
\begin{align*}
\mathcal{H}^1(X \cap R) & \leq \left| g_K^+(t) - 1 \right| \cdot \delta + \sigma_K(t + \pi/2) \cdot \delta + O_\delta(\delta^2).
\end{align*}
$$
by triangle inequality.

Now we bound $\mathcal{H}^1(X \setminus R)$ from above. Define the closed region $S := \bigcap_{u \in U} H_K^\textrm{b} (u)$; our intermediate goal is to show $X \setminus R \subseteq \overline{b}_K(t) \cap S$. Define $H_0 := H_+(\pi/2, 0)$ as the closed half-plane on and above the $x$-axis. Then for any angle $u \in U$, we have $\mathcal{N}_\Theta(K) \supseteq H_0 \cap Q_K^-(u)$ by the definition of $\mathcal{N}_\Theta(K)$. $H_K^\textrm{d}(u)$

Elementary set theory gives
$$
S \setminus R = \bigcap_{u \in U} H_K^\textrm{b} (u) \setminus \bigcup_{u \in U} H_K^\textrm{d} (u) = \bigcap_{u \in U} \left( H_K^\textrm{b}(u) \setminus H_K^\textrm{d} (u) \right) 
$$

 So
$$
H_0 \setminus \mathcal{N}_\Theta(K) \subseteq H_0 \setminus \bigcup_{u \in U} Q_K^-(u)
$$


For every $t \in U$, we have $Q_K^-(t) = \mathbb{R}^2 \setminus H_K^\textrm{b}(t) \setminus H_K^{\textrm{d}}(t)$ by definitions. So we have
$$
H_0 \subseteq R \cup S \cup \mathcal{N}_\Theta(K).
$$

Recall that
$$
\mathcal{N}_\Theta(K) \supseteq H(\pi/2, 0) \cap  \bigcup_{t \in U} Q_K^-(t)
$$
by the definition of $\mathcal{N}_\Theta(K)$. 

$$
X \setminus R = (b_K(t) \cap \partial \mathcal{N}_\Theta(K)) \setminus R \subseteq (\partial \mathcal{N}_{\Theta}(K) \setminus R) \cap b_K(t) \subseteq \partial(\mathcal{N}_\Theta(K) \setminus R) \cap b_K(t)
$$

. $\mathbb{R}^2 = R \cup S \cup \mathcal{N}_\Theta(K) \cup H_-^\circ(\pi/2, 0)$.

We just have shown that $X \setminus R \subseteq \overline{b}_K(t) \cap S$ where $S := \bigcap_{u \in U} H_K^\textrm{b} (u)$ is the intersection of three closed half-planes with normal angles $u \in U$ and boundaries $\overline{b}_K(u)$. Define the intersections $B_+ := \overline{b}_K(t) \cap \overline{b}_K(t + \delta)$ and $B_- := \overline{b}_K(t) \cap \overline{b}_K(t - \delta)$. There is a unique real value $\beta$ such that $B_+ = B_- - \beta v_t$. Elementary geometry shows that the side $\overline{b}_K(t) \cap S$ of $S$ is empty if $\beta < 0$, or has length $\beta$ if $\beta \geq 0$. On the other hand, XX shows that $\beta = 2 \tan (\delta / 2) - \sigma_K(t)$. So ^eqn-bound-x-not-r
$$
\begin{align*}
...
Recall that $s = \nu_K(t) = \mathcal{H}^1(X)$. If $s \leq \delta$, then adding [[#^eqn-bound-x-r]] and [[#^eqn-bound-x-not-r]] gives
$$
s \leq \left| g_K^+(t) - 1 \right| \cdot \delta + \sigma_K(t + \pi/2) \cdot \delta + \delta - s + O_\delta(\delta^2)
$$
so rearranging gives
$$
s \leq (\left| g_K^+(t) - 1 \right| + 1)/2 \cdot \delta + \sigma_K(t + \pi/2) \cdot \delta + O_\delta(\delta^2) \leq k(g_K^+(t)) + \sigma_K(t + \pi/2) \cdot \delta + O_\delta(\delta^2).
$$
On the other hand, if $s > \delta$, then adding [[#^eqn-bound-x-r]] and [[#^eqn-bound-x-not-r]] gives
$$
s \leq \left| g_K^+(t) - 1 \right| \cdot \delta + \sigma_K(t + \pi/2) \cdot \delta + O_\delta(\delta^2) \leq k(g_K^+(t)) + \sigma_K(t + \pi/2) \cdot \delta + O_\delta(\delta^2).
$$
Either way, the claimed inequality holds. □

> __Lemma [leg-convergence].__ Assume that a sequence of polygon caps $K_n$ converge to a cap $K \in \mathcal{K}^\mathrm{c}$ in Hausdorff distance as $n \to \infty$. Then we have the convergence $g_{K_n}^+ \to g_K^+$ in $L^1$ norm. ^lem-leg-convergence
...
> __Lemma [balanced-ineq-limit].__ Let $K_{\infty}$ be a balanced maximum cap, so that it is the limit of balanced polygon caps $K_1, K_2, \dots$ with the number of steps $n_1 < n_2 < \dots$ and step sizes $\delta_1 > \delta_2 > \dots$. Then on the interval $t \in [0, \pi/2)$, we have $\sigma_K \leq k(f_K^-(t)) \, \textrm{d} t$. ^lem-balanced-ineq-limit

_Proof._ Fix an index $i \geq 1$. For any $t \in \left\{ 0 \right\} \cup \Theta_{n_i}$, we have ^single-step-integral
$$
\sigma_{K_i}\left( [t, t + \delta_i) \right) = \sigma_{K_i} (t) \leq k(g_{K_i}^+(t)) \cdot \delta_i + \sigma_K(t + \pi/2) \cdot \delta_i + O_{\delta_i}(\delta_i^2) =  \int_t^{t + \delta_i} k(g_{K_i}^+(u)) \, du + O_{\delta_i}(\delta_i^2)
$$
by [[b. Injectivity of Maximum-area Sofas/20. Injectivity of Balanced Maximum Sofa/10. Bounding Arm Lengths#^lem-balanced-discrete-ineq]] and [[b. Injectivity of Maximum-area Sofas/20. Injectivity of Balanced Maximum Sofa/10. Bounding Arm Lengths#^lem-arm-length-discrete-bound]]. Now for any $a, b \in \left\{ 0 \right\} \cup \Theta_{n_i}$ and the interval $I = [a, b)$, sump up [[#^single-step-integral]] for all $t \in I \cap \Theta_{n_i}$ and get the inequality ^polygon-integral
$$
\begin{align*}
