The overall idea is to set up the two tails and one core separately. We will assume that $\mathbf{x}(t)$ restricted to $t \in [\varphi_R, \varphi_L]$ draws one core in the middle.
