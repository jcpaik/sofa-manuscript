...
For the proof of [[15. Monotone sofas/07. Tangent Hallway#^pro-tangent-hallway-contains]], assume that an arbitrary shape $S$ is contained in a translation $L'$ of $R_t(L)$ for some angle $t$. Then while keeping $S$ inside $L'$, we can push $L'$ towards $S$ in the directions $-u_t$ and $-v_t$ until the outer walls of the final $L' = L_S(t)$ make contact with $S$. The pushed hallway $L_S(t)$ still contains $S$ because the directions $-u_t$ and $-v_t$ of movement only push the inner walls of $L'$ away from $S$, completing the proof of [[15. Monotone sofas/07. Tangent Hallway#^pro-tangent-hallway-contains]].
