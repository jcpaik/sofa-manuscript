...
So any moving sofa $S$ is a common subset of two strips of width 1 and the rotated hallways (condition 2), and we can also assume that the rotated hallways containing $S$ are the tangent hallways $L_S(t)$ of $S$ making contact with $S$ in the outer walls (condition 3). We will now always work with the tangent hallways $L_S(t)$ for all $t \in [0, \omega]$. Name the parts of tangent hallway $L_S(t)$ according to the parts of $L$.
