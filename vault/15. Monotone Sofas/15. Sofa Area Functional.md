...
We will prove [[15. Sofa Area Functional#^thm-niche-in-cap]] later, so for now let's assume it and continue our discussion. [[15. Sofa Area Functional#^thm-niche-in-cap]] is very important as it allows us to understand the area of a monotone sofa $|S| = |K| - |\mathcal{N}(K)|$ by the area of cap and niche separately.  Also observe that the area of $S$ depends solely on the cap $K$. So we can turn the moving sofa problem into maximizing the sofa area functional $\mathcal{A}(K) = |K| - |\mathcal{N}(K)|$ of an arbitrary cap $K$.
