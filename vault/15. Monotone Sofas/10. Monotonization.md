...
To prove [[10. Monotonization#^thm-monotonization-is-sofa]], we need to analyze the geometric structure of $\mathcal{M}(S)$ in depth. Define the following sets. 
...
By breaking down $L_S(t)$ into $Q_S^+(t) \setminus Q_S^-(t)$, we get the following representation of $\mathcal{M}(S)$ as a subtraction of two sets.
$$
\begin{align*}
...
$$
is a convex set circumscribed inside the parallelogram $P_\omega$ with the outer walls $a_S(t)$ and $c_S(t)$ of $L_S(t)$ as tangent lines. Imagine $K$ as a block of clay and rotate it around the hallway $L$ while touching the outer walls $a$ and $c$. Then the inner corner of $L$ carves out the set
$$
...
> __Proposition [cap-hallway-intersection].__ Let $S$ be any moving sofa with rotation angle $\omega \in [0, \pi/2]$. By translating $S$, assume $p_S(\omega) = p_S(\pi/2) = 1$ without loss of generality. Define $\mathcal{C}(S)$ as the following set.
$$
\mathcal{C}(S) = P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t)
$$
> Then $\mathcal{C}(S)$ is a cap with the angle set $[0, \omega]$. ^pro-cap-hallway-intersection

With [[07. Tangent Hallway#^pro-cap-hallway-intersection]], we can call $\mathcal{C}(S)$ the _cap_ of $S$. We omit the proof of [[07. Tangent Hallway#^pro-cap-hallway-intersection]] but note that it only depends on the condition $p_S(\omega) = p_S(\pi/2) = 1$ of $S$. The tangent hallways of a moving sofa $S$ are exactly same as the tangent hallways of its cap $K = \mathcal{C}(S)$.
...
> __Theorem [monotonization-cap].__ Moreover, the monotonization $\mathcal{M}(S)$ shares the same cap with $S$; that is, $\mathcal{C}(\mathcal{M}(S)) = \mathcal{C}(S)$. ^thm-monotonization-cap
