...
_Proof._ The support functions of $S$ and $K$ match on the set $J = [0, \omega] \cup [\pi/2, \pi/2 + \omega]$ by [[15. Monotone sofas/10. Monotonization#^lem-cap-same-support-function]]. The tangent hallways $L_X(t)$ of $X = S, K$ depend solely on the values of the support function $p_X$ of $X$ on $J$ by the equation of $L_X(t)$ in [[15. Monotone sofas/07. Tangent Hallway#^def-tangent-hallway]]. The result follows. □
