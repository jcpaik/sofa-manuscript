...
Every monotonization have the following structure of cap and niche, which will be used thoroughout the rest of the paper including the proof of [[10. Monotonization#^thm-monotonization-is-connected]].
...
\end{split}
$$
The first set $K = P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t)$ is a convex set that we will call the _cap of_ $S$ ([[10. Monotonization#^def-cap-sofa]]). We will call the second set $N = F_\omega \cap \bigcup_{0 \leq t \leq \omega} Q^-_S(t)$ that we subtract from $K$ as the _niche of cap_ $K$ ([[10. Monotonization#^def-niche]]). 

Imagine the set $K$ as a block of clay that rotates inside the hallway $L$ in the clockwise angle of $t \in [0, \omega]$ while always touching the outer walls $a$ and $c$ of $L$. The intuition is that as $K$ rotates inside $L$, the inner corner of $L$ carves out the set $N$ from $K$, so that the final clay $K \setminus N$ is a moving sofa $\mathcal{M}(S)$.
...
Define the first set of the subtraction in [[10. Monotonization#^eqn-monotonization]] as $\mathcal{C}(S)$.
