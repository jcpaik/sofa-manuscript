...
> __Proposition [monotonization-is-sofa].__ Let $S$ be any moving sofa with rotation angle $\omega \in (0, \pi/2]$ in standard position. The monotonization $\mathcal{M}(S)$ of $S$ is then a moving sofa with the same rotation angle $\omega$ in standard position containing $S$. ^pro-monotonization-is-sofa
...
$$we have $p_{\mathcal{M}(S)}(\omega) = p_{\mathcal{M}(S)}(\pi/2) = 1$. □
