...
> __Lemma [cap-same-support-function].__ Let $S$ be any moving sofa with rotation angle $\omega \in [0, \pi/2]$ in standard position. Let $K = \mathcal{C}(S)$ be its cap. Then the support functions $p_S$ and $p_K$ of $S$ and $K$ agree on the set $[0, \omega] \cup [\pi/2, \pi/2 + \omega]$. Consequently, the tangent hallways $L_S(t)$ and $L_K(t)$ of $S$ and $K$ are equal for any $t \in [0, \omega]$. ^lem-cap-same-support-function
...
# Proof of [[10. Monotonization#^lem-cap-hallway-intersection]]
