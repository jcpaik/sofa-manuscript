...
Every monotonization have the following structure consisting of a _cap_ and a _niche_. This structure will be used thoroughout the rest of the paper including the proof of [[10. Monotonization#^thm-monotonization-is-connected]]. The full definitions of a cap and a niche are technical and so given separately in [[10. Monotonization#^def-cap-sofa]] and [[10. Monotonization#^def-niche]].

> __Theorem [monotonization-structure].__ Let $S$ be a moving sofa with rotation angle $\omega \in (0, \pi/2]$ in standard position. The monotonization $\mathcal{M}(S)$ of $S$ is then equal to $K \setminus \mathcal{N}(K)$, where $K = \mathcal{C}(S)$ is the cap of $S$ and $\mathcal{N}(K)$ is the niche of the cap $K$. ^thm-monotonization-structure

Instead of proving [[10. Monotonization#^thm-monotonization-structure]] fully, which is done rigorously later, let us understand the structure it gives intuitively at this moment. Fix an arbitrary moving sofa $S$ with rotation angle $\omega \in (0, \pi/2]$ in a standard position. By breaking down each $L_S(t)$ into $Q_S^+(t) \setminus Q_S^-(t)$, the monotonization $\mathcal{M}(S)$ of $S$ can be represented as a subtraction of two sets. See [[05. Notations and conventions#^def-parallelogram]] and [[05. Notations and conventions#^def-fan]] for the definition of $P_\omega$ and $F_\omega$. ^eqn-monotonization
$$
\begin{split}
\mathcal{M}(S) & = P_\omega \cap \bigcap_{0 \leq t \leq \omega} L_S(t) \\
...
\end{split}
$$
After proving [[10. Monotonization#^thm-monotonization-structure]], we will see that the first set $K = P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t)$ is the _cap $\mathcal{C}(S)$ of $S$_, which is the intersection of closed half-planes and thus a convex set. It turns out that the second set $N = F_\omega \cap \bigcup_{0 \leq t \leq \omega} Q^-_S(t)$ that we subtract from $K$ is completely determined by the cap $K$. Accordingly, $N = \mathcal{N}(K)$ will be the _niche of cap_ $K$.
...
We will show that the set $\mathcal{C}(S)$ confines to [[10. Monotonization#^def-cap]], so that $\mathcal{C}(S)$ is a cap with the angle set $[0, \omega]$ associated to a moving sofa $S$ with rotation angle $\omega$. This justifies calling $\mathcal{C}(S)$ _the cap of_ $S$ particularly associated to $S$.
