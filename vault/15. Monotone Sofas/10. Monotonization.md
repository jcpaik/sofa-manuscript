...
> __Lemma [cap-hallway-intersection].__ The set $\mathcal{C}(S)$ in [[10. Monotonization#^def-cap-sofa]] is a cap with angle set $[0, \omega]$ as in [[10. Monotonization#^def-cap]], that $S$ as a subset. With this, call $\mathcal{C}(S)$ as _the cap of the moving sofa_ $S$. ^lem-cap-hallway-intersection
...
With [[10. Monotonization#^lem-cap-same-support-function]], we have $Q^-_S(t) = Q^-_K(t)$ in particular. So the set $N$ in [[10. Monotonization#^eqn-monotonization]] can be written purely in terms of the cap $K = \mathcal{C}(S)$ as the following.
...
We move on to proving [[10. Monotonization#^lem-cap-same-support-function]]. Let $J = [0, \omega] \cup [\pi/2, \pi/2 + \omega]$ and pick any $t \in J$. By [[10. Monotonization#^def-cap-sofa]], we have $\mathcal{C}(S) \subseteq H_S(t)$ so we have $p_K(t) \leq p_S(t)$. It remains to show $p_S(t) \leq p_K(t)$ for all $t \in J$. Let $\Pi = J \cup \left\{ \pi + \omega, 3\pi/2 \right\}$ and set $S' = \bigcap_{t \in \Pi} H_S(t)$. Then by convexity of $S$ we have . Since $S \subseteq F_\omega$ we have 

# Proof of [[10. Monotonization#^thm-monotonization-is-connected]]

We finally show the connectedness of the monotonization $\mathcal{M}(S)$ of a moving sofa $S$ using [[10. Monotonization#^thm-monotonization-structure]] as promised. 
