...
Now the following is a corollary of the condition 3 of [[07. Tangent Hallway#^thm-moving-sofa-iff-hallway-intersection]] and [[10. Monotonization#^thm-monotonization-is-connected]]. In particular, we can always assume that a sofa of maximum area is a monotone sofa.
...
Let us build our intuition on the cap $K$ and the niche $N$ before giving their full definitions. The cap $K$ of $S$ is a convex body bounded from below by the boundary of fan $F_\omega$, and bounded from above by the outer walls $a_S(t)$ and $c_S(t)$ of $L_S(t)$. Imagine the set $K$ as a block of clay that rotates inside the hallway $L$ in the clockwise angle of $t \in [0, \omega]$ while always touching the outer walls $a$ and $c$ of $L$. As $K$ rotates inside $L$, the inner corner of $L$ carves out the niche $N$ which is the regions bounded by inner walls $b_S(t)$ and $d_S(t)$ of $L_S(t)$ from $K$. After the full movement of $K$, the final clay $K \setminus N$ is a moving sofa $\mathcal{M}(S)$.

For the rest of the paper, we will use the following variant [[10. Monotonization#^thm-monotone-sofa-structure]] of [[10. Monotonization#^thm-monotonization-structure]] to represent any monotone sofa $S$ as cap $K = \mathcal{C}(S)$ minus niche $\mathcal{N}(K)$. Note that this variant does not mention anything about 

> __Theorem [monotone-sofa-structure].__ Let $S$ be any monotone sofa with rotation angle $\omega \in (0, \pi/2]$. The monotonization $\mathcal{M}(S)$ of $S$ is then equal to $K \setminus \mathcal{N}(K)$, where $K = \mathcal{C}(S)$ is the _cap of_ $S$ and $\mathcal{N}(K)$ is the _niche of_ the cap $K$. ^thm-monotone-sofa-structure

# Proof of [[10. Monotonization#^thm-monotonization-structure]]
...
Geometrically, the first condition of [[10. Monotonization#^def-cap]] states that $K$ is contained in the parallelogram $P_\omega$ and touches all sides of $P_\omega$. Note that [[10. Monotonization#^def-cap]] is not confined to a moving sofa yet. That is, for any convex body $K$ we can tell if it is a cap with angle set $\Theta$ or not without associating it to any moving sofa.

Now define the set $K$ of the subtraction in [[10. Monotonization#^eqn-monotonization]] as $\mathcal{C}(S)$.
...
We will show that the set $\mathcal{C}(S)$ confines to [[10. Monotonization#^def-cap]], so that $\mathcal{C}(S)$ is a cap with angle set $[0, \omega]$ associated to a moving sofa $S$ with rotation angle $\omega$. This justifies calling $\mathcal{C}(S)$ _the cap of_ $S$ associated to $S$. We delegate the proof in a separate section as it is technical and not so motivating.

> __Theorem [cap-hallway-intersection].__ The set $\mathcal{C}(S)$ in [[10. Monotonization#^def-cap-sofa]] is a cap with angle set $[0, \omega]$ as in [[10. Monotonization#^def-cap]], that $S$ as a subset. With this, call $\mathcal{C}(S)$ as _the cap of the moving sofa_ $S$. ^thm-cap-hallway-intersection

The tangent hallways of a moving sofa $S$ are exactly same as the tangent hallways of its cap $K = \mathcal{C}(S)$. We also delegate the proof.

> __Theorem [cap-same-support-function].__ Let $S$ be any moving sofa with rotation angle $\omega \in [0, \pi/2]$ in standard position. Let $K = \mathcal{C}(S)$ be its cap. Then the support functions $p_S$ and $p_K$ of $S$ and $K$ agree on the set $[0, \omega] \cup [\pi/2, \pi/2 + \omega]$. Consequently, the tangent hallways $L_S(t)$ and $L_K(t)$ of $S$ and $K$ are equal for any $t \in [0, \omega]$. ^thm-cap-same-support-function

With [[10. Monotonization#^thm-cap-same-support-function]], we have $Q^-_S(t) = Q^-_K(t)$ in particular. So the set $N$ in [[10. Monotonization#^eqn-monotonization]] can be written purely in terms of the cap $K = \mathcal{C}(S)$ as the following.
...
Now [[10. Monotonization#^thm-monotonization-structure]] is a corollary of [[10. Monotonization#^eqn-monotonization]]. We now prove the technical theorems [[10. Monotonization#^thm-cap-same-support-function]] and [[10. Monotonization#^thm-cap-same-support-function]] that we have left.

# Proof of [[10. Monotonization#^thm-cap-hallway-intersection]]

Let $S$ be any moving sofa with rotation angle $\omega \in [0, \pi/2]$ in standard position. Let $K = \mathcal{C}(S)$. That $S \subseteq K$ is an immediate consequence of the condition 3 of [[07. Tangent Hallway#^thm-moving-sofa-iff-hallway-intersection]]. We now show that $K$ is a cap with angle set $[0, \omega]$. Define the following subset of $P_\omega$.
...
We finally show the connectedness of the monotonization $\mathcal{M}(S)$ of a moving sofa $S$ as promised. 

_Proof._ (of [[10. Monotonization#^thm-monotonization-is-connected]])
