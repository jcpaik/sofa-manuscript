...
> __Corollary [monotonization-is-sofa].__ Any monotonization $\mathcal{M}(S)$ of a sofa $S$ in standard position is a moving sofa containing $S$. ^cor-monotonization-is-sofa

Every monotonization have the following structure consisting of a _cap_ and a _niche_ (we will give the full definitions of them later). This structure will be used thoroughout the rest of the paper including the proof of [[10. Monotonization#^thm-monotonization-is-connected]].
...
\end{split}
$$
The first set $K = P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t)$ is a convex set that we will soon define as the _cap $K = \mathcal{C}(S)$ of_ $S$ ([[10. Monotonization#^def-cap-sofa]]). It turns out that the second set $N = F_\omega \cap \bigcup_{0 \leq t \leq \omega} Q^-_S(t)$ that we subtract from $K$ is completely determined by the cap $K$. Accordingly, we will define $N = \mathcal{N}(K)$ as the _niche of cap_ $K$ ([[10. Monotonization#^def-niche]]).

Let us build our intuition on the cap $K$ and the niche $N$ before giving their full definitions. The cap $K$ of $S$ is a convex body bounded from below by the boundary of fan $F_\omega$, and bounded from above by the outer walls $a_S(t)$ and $c_S(t)$ of $L_S(t)$. Imagine the set $K$ as a block of clay that rotates inside the hallway $L$ in the clockwise angle of $t \in [0, \omega]$ while always touching the outer walls $a$ and $c$ of $L$. As $K$ rotates inside $L$, the inner corner of $L$ carves out the union $N$ of regions bounded by inner walls $b_S(t)$ and $d_S(t)$ of $L_S(t)$ from $K$, so that the final clay $K \setminus N$ is a moving sofa $\mathcal{M}(S)$.



> __Theorem [monotone-sofa-structure].__ Let $S$ be a monotone sofa with rotation angle $\omega \in (0, \pi/2]$. The monotonization $\mathcal{M}(S)$ of $S$ is then equal to $K \setminus \mathcal{N}(K)$, where $K = \mathcal{C}(S)$ is the _cap of_ $S$ and $\mathcal{N}(K)$ is the _niche of_ the cap $K$. ^thm-monotone-sofa-structure

# Definition of cap and niche

We now give the full, formal definition of a cap and its niche. Define a _cap_ as a convex body satisfying certain properties. 
...
We will show that the set $\mathcal{C}(S)$ confines to [[10. Monotonization#^def-cap]], so that $\mathcal{C}(S)$ is the cap with angle set $[0, \omega]$ associated to a moving sofa $S$ with rotation angle $\omega$. This justifies calling $\mathcal{C}(S)$ the particular _cap of_ $S$ as we did in [[10. Monotonization#^eqn-monotonization]].
...
We delegate the proof of [[10. Monotonization#^thm-cap-hallway-intersection]] in a separate section as it is technical and not so motivating.
...
We also delegate the proof of [[10. Monotonization#^thm-cap-same-support-function]]. With [[07. Tangent Hallway#^pro-cap-same-support-function]], we have $Q^-_S(t) = Q^-_K(t)$ in particular. So we can define the set $N$ in [[10. Monotonization#^eqn-monotonization]] purely in terms of the cap $K = \mathcal{C}(S)$ as the following.
...
Now [[10. Monotonization#^thm-monotonization-structure]] is a corollary of [[10. Monotonization#^eqn-monotonization]]. We now prove the technical theorems [[10. Monotonization#^thm-cap-same-support-function]] and [[10. Monotonization#^thm-cap-same-support-function]] that we left.
