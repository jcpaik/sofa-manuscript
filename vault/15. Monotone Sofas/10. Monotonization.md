...
> __Definition [monotone-sofa].__ A _monotone sofa_ is a moving sofa which is the monotonization of a moving sofa in standard position. ^def-monotone-sofa
...
Gerver claimed in [[@gerverMovingSofaCorner1992]] that the monotonization $\mathcal{M}(S)$ is also a moving sofa, so that a sofa of maximum area can be assumed to be monotone. However, he did not show that the set $\mathcal{M}(S)$ is connected, which is essential for $\mathcal{M}(S)$ to be a moving sofa. We fill the gap in his argument by showing that $\mathcal{M}(S)$ is indeed connected.
...
Now the following is a corollary of [[10. Monotonization#^cor-monotonization-is-larger]] and [[10. Monotonization#^thm-monotonization-is-connected]]. In particular, we can always assume that a sofa of maximum area is a monotone sofa.

> __Corollary [monotonization-is-sofa].__ Any monotonization $\mathcal{M}(S)$ of a sofa $S$ in standard position is a monotone sofa containing $S$. ^cor-monotonization-is-sofa

Every monotonization have the following structure consisting of a _cap_ and a _niche_ (we will give their full definitions of them later). This structure will be used thoroughout the rest of the paper including the proof of [[10. Monotonization#^thm-monotonization-is-connected]].

> __Theorem [monotonization-structure].__ Let $S$ be a moving sofa with rotation angle $\omega \in (0, \pi/2]$ in standard position. The monotonization $\mathcal{M}(S)$ of $S$ is then equal to $K \setminus \mathcal{N}(K)$, where $K = \mathcal{C}(S)$ is the cap of $S$ ([[10. Monotonization#^def-cap-sofa]]) and $\mathcal{N}(K)$ is the niche of the cap $K$ ([[10. Monotonization#^def-niche]]). ^thm-monotonization-structure

To motivate this structure, fix an arbitrary moving sofa $S$ with rotation angle $\omega \in (0, \pi/2]$ in standard position. By breaking down each $L_S(t)$ into $Q_S^+(t) \setminus Q_S^-(t)$, the monotonization $\mathcal{M}(S)$ of $S$ can be represented as a subtraction of two sets. See [[05. Notations and conventions#^def-parallelogram]] and [[05. Notations and conventions#^def-fan]] for the definition of $P_\omega$ and $F_\omega$. ^eqn-monotonization
$$
\begin{split}
\mathcal{M}(S) & = P_\omega \cap \bigcap_{0 \leq t \leq \omega} L_S(t) \\
...
\end{split}
$$
The first set $K = P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t)$ is the intersection of closed half-planes and thus a convex set. We will define $K$ as the _cap $\mathcal{C}(S)$ of_ $S$ ([[10. Monotonization#^def-cap-sofa]]). It turns out that the second set $N = F_\omega \cap \bigcup_{0 \leq t \leq \omega} Q^-_S(t)$ that we subtract from $K$ is completely determined by the cap $K$. Accordingly, we will define $N = \mathcal{N}(K)$ as the _niche of cap_ $K$ ([[10. Monotonization#^def-niche]]).

Let us build our intuition on the cap $K$ and the niche $N$ before giving their full definitions (see [[01. Moving Sofa Problem#^fig-monotone-sofa]]). The cap $K$ of $S$ is a convex body bounded from below by the boundary of fan $F_\omega$, and bounded from above by the outer walls $a_S(t)$ and $c_S(t)$ of $L_S(t)$. Imagine the set $K$ as a block of clay that rotates inside the hallway $L$ in the clockwise angle of $t \in [0, \omega]$ while always touching the outer walls $a$ and $c$ of $L$. As $K$ rotates inside $L$, the inner corner of $L$ carves out the niche $N$ which is the regions bounded by inner walls $b_S(t)$ and $d_S(t)$ of $L_S(t)$ from $K$. After the full movement of $K$, the final clay $K \setminus N$ is a moving sofa $\mathcal{M}(S)$.

For the rest of the paper, we will always use the following variant [[10. Monotonization#^thm-monotone-sofa-structure]] of [[10. Monotonization#^thm-monotonization-structure]] to represent any monotone sofa $S$ as cap $K = \mathcal{C}(S)$ minus niche $\mathcal{N}(K)$. Note that this variant does not mention anything about monotonization.

> __Theorem [monotone-sofa-structure].__ Let $S$ be any monotone sofa with rotation angle $\omega \in (0, \pi/2]$. Then $S$ is in standard position and $S = K \setminus \mathcal{N}(K)$, where $K = \mathcal{C}(S)$ is the cap of $S$ with angle set $[0, \omega]$ ([[10. Monotonization#^def-cap-sofa]]), and $\mathcal{N}(K)$ is the _niche of_ the cap $K$ ([[10. Monotonization#^def-niche]]). ^thm-monotone-sofa-structure

# Proof of [[10. Monotonization#^thm-monotonization-structure]] and [[10. Monotonization#^thm-monotone-sofa-structure]]

We now give the full definition of a cap and its niche. Define a _cap_ as a convex body satisfying certain properties. 
...
We will show that the set $\mathcal{C}(S)$ confines to [[10. Monotonization#^def-cap]], so that $\mathcal{C}(S)$ is a cap with angle set $[0, \omega]$ associated to a moving sofa $S$ with rotation angle $\omega$. This justifies calling $\mathcal{C}(S)$ _the cap of_ $S$ particularly associated to $S$. We delegate the proof in a separate section as it is technical and not so motivating.

> __Lemma [cap-hallway-intersection].__ The set $\mathcal{C}(S)$ in [[10. Monotonization#^def-cap-sofa]] is a cap with angle set $[0, \omega]$ as in [[10. Monotonization#^def-cap]]. With this, call $\mathcal{C}(S)$ as _the cap of the moving sofa_ $S$. ^lem-cap-hallway-intersection

The tangent hallways of a moving sofa $S$ are exactly same as the tangent hallways of its cap $K = \mathcal{C}(S)$. We also delegate its proof.
...
With [[10. Monotonization#^cor-cap-same-tangent-hallway]], we have $Q^-_S(t) = Q^-_K(t)$ in particular. So the set $N$ in [[10. Monotonization#^eqn-monotonization]] can be written purely in terms of the cap $K = \mathcal{C}(S)$ as the following.
...
Now [[10. Monotonization#^thm-monotonization-structure]] is a corollary of [[10. Monotonization#^eqn-monotonization]], [[10. Monotonization#^def-cap-sofa]] and [[10. Monotonization#^def-niche]].
...
## Proof of [[10. Monotonization#^lem-cap-hallway-intersection]] and [[10. Monotonization#^lem-cap-same-support-function]]

We now prove the technical [[10. Monotonization#^lem-cap-hallway-intersection]] and [[10. Monotonization#^lem-cap-same-support-function]] that we have left out. Let $S$ be any moving sofa with rotation angle $\omega \in [0, \pi/2]$ in standard position. Let $K = \mathcal{C}(S)$. That $S \subseteq K$ is an immediate consequence of [[07. Tangent Hallway#^thm-moving-sofa-iff-hallway-intersection]]. We now show that $K$ is a cap with angle set $[0, \omega]$. Define the following subset of $P_\omega$.
...
Assume by contradiction that for every $\theta \in [\omega, \pi/2]$ the line $l_\theta$ is disjoint from $S$. By [[10. Monotonization#^lem-cap-same-support-function]] the support function of $\mathcal{M}(S)$ agrees with that of $S$ on the set $[0, \omega] \cup [\pi/2, \pi/2 + \omega]$. As $p \in \mathcal{M}(S)$, the line $l_{\pi/2}$ is either equal to $l_{\mathcal{M}(S)}(0) = l_S(0)$ or strictly on the left side of $l_{S}(0)$. The line $l_{\pi/2}$ can't be equal to $l_S(0)$ because otherwise, $l_{\pi/2}$ contains some point of $S$ contradicting our assumption. So the line $l_{\pi/2}$ is strictly on the left side of $l_{S}(0)$, and there is a point of $S$ strictly on the right side of $l_{\pi/2}$.
