...
With [[07. Tangent Hallway#^pro-cap-same-support-function]], we have $Q^-_S(t) = Q^-_K(t)$ in particular. So we can define the set $N$ [[#^eqn-niche]] purely in terms of the cap $K = \mathcal{C}(S)$ as the following.
...
We fix the following terminology that will be used in later parts as well.
...
We are now ready to show the connectedness of the monotonization $\mathcal{M}(S)$ of a moving sofa $S$. 

_Proof._ (of [[10. Monotonization#^thm-monotonization-is-sofa]])

In this proof, we fix the angle set $\Theta = [0, \omega]$. We first show that the set $\mathcal{M}(S)$ is connected.
