...
Two corollaries of [[10. Monotonization#^lem-cap-same-support-function]] are:
...
> __Corollary [monotonization-cap].__ For any moving sofa $S$ with rotation angle $\omega \in [0, \pi/2]$ in standard position, we have $\mathcal{C}(\mathcal{M}(S)) = \mathcal{C}(S)$. ^cor-monotonization-cap
