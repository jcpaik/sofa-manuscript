...
Every monotonization have the following structure consisting of a _cap_ and a _niche_. This structure will be used thoroughout the rest of the paper including the proof of [[10. Monotonization#^thm-monotonization-is-connected]].
...
\end{split}
$$
We will call the first set $K = P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t)$ _the cap $\mathcal{C}(S)$ of $S$_, which is the intersection of closed half-planes and thus a convex set. It turns out that the second set $N = F_\omega \cap \bigcup_{0 \leq t \leq \omega} Q^-_S(t)$ that we subtract from $K$ is completely determined by the cap $K$. Accordingly, we will call $N = \mathcal{N}(K)$ the _niche of cap_ $K$. Justifying all this requires us to take care of subtle details, which are fully done in the proof of [[10. Monotonization#^thm-monotonization-structure]].

An intuition on the cap $K$ and the niche $N$ can be described as the following (see [[01. Moving Sofa Problem#^fig-monotone-sofa]]). The cap $K$ of $S$ is a convex body bounded from below by the boundary of fan $F_\omega$, and bounded from above by the outer walls $a_S(t)$ and $c_S(t)$ of $L_S(t)$. Imagine the set $K$ as a block of clay that rotates inside the hallway $L$ in the clockwise angle of $t \in [0, \omega]$ while always touching the outer walls $a$ and $c$ of $L$. As $K$ rotates inside $L$, the inner corner of $L$ carves out the niche $N$ which is the regions bounded by inner walls $b_S(t)$ and $d_S(t)$ of $L_S(t)$ from $K$. After the full movement of $K$, the final clay $K \setminus N$ is a moving sofa $\mathcal{M}(S)$.

For the rest of the paper, we will always use the following variant [[10. Monotonization#^thm-monotone-sofa-structure]] of [[10. Monotonization#^thm-monotonization-structure]] to represent any monotone sofa $S$ as its cap $K = \mathcal{C}(S)$ minus niche $\mathcal{N}(K)$. Note that this variant does not mention anything about monotonization.
...
# Definition of a cap and a niche

We now give the full definitions of a cap and its niche. Define a general _cap_ as a convex body satisfying certain properties. 
...
Now define the set $K$ in [[10. Monotonization#^eqn-monotonization]] as $\mathcal{C}(S)$.
...
We will show that the set $\mathcal{C}(S)$ confines to [[10. Monotonization#^def-cap]], so that $\mathcal{C}(S)$ is a cap with angle set $[0, \omega]$ associated to a moving sofa $S$ with rotation angle $\omega$. This justifies calling $\mathcal{C}(S)$ _the cap of_ $S$ particularly associated to $S$.
...
# Proof of [[10. Monotonization#^thm-monotonization-structure]] and [[10. Monotonization#^thm-monotone-sofa-structure]]
 ...
 We delegate the proof in a separate section as it is technical.
