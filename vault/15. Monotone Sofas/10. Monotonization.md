...
Every monotone sofa have the following structure consisting of a cap and a niche (we will give the full definitions of them later). This structure will be used thoroughout the rest of the paper including the proof of [[10. Monotonization#^thm-monotonization-is-connected]].
...
\end{split}
$$
The first set $K = P_\omega \cap \bigcap_{0 \leq t \leq \omega} Q^+_S(t)$ is a convex set that we will later define as the _cap of_ $S$ ([[10. Monotonization#^def-cap-sofa]]). We will also later define the second set $N = F_\omega \cap \bigcup_{0 \leq t \leq \omega} Q^-_S(t)$ that we subtract from $K$ as the _niche of cap_ $K$ ([[10. Monotonization#^def-niche]]).
