...
As monotonizing a sofa never decreases its area, we can assume that a sofa $S$ of maximum area is monotone. Moreover, this $S$ is equal to the cap $K = \mathcal{C}(S)$ subtracted by its niche $\mathcal{N}(K)$ by [[01. Monotone Sofas#^cor-monotonization-eq]] and [[01. Monotone Sofas#^thm-monotonization-is-sofa]].

We will also show that for any monotone sofa $S$, its cap $K = \mathcal{C}(S)$ contains the niche $\mathcal{N}(K)$. More generally, for any cap $K$ with arbitrary angle set $\Theta$, we will show that $\mathcal{N}(K)$ is a subset of $K$ if and only if the set $K \setminus \mathcal{N}(K)$ is connected. To do so, we need to prepare a set of definitions and lemmas regarding the cap $K$.
