...
Monotization $S \mapsto \mathcal{M}(S)$ is a process that takes any moving sofa $S$ and enlarges it to a monotone sofa by [[10. Monotonization#^cor-monotonization-is-sofa]]. Moreover, if $S$ is already monotone (so that $S = \mathcal{M}(S')$ for some $S'$), then the monotonization fixes $S$.
...
Thus, monotonization $S \mapsto \mathcal{M}(S)$ can be said as a 'projection' from moving sofas to monotone sofas that enlarges the area, in the sense that $\mathcal{M}$ is a surjective map that fixes monotone sofas.
