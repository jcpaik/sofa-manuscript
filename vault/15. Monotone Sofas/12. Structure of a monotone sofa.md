...
We will always use the intrinsic variant [[#^thm-monotone-sofa-structure]] of [[#^thm-monotonization-structure]] to represent any monotone sofa $S$ as its cap minus niche. Note that this variant does not mention anything about monotonization.
...
Thus, the monotonization $S \mapsto \mathcal{M}(S)$ can be said as a 'projection' to monotone sofas, in the sense that $\mathcal{M}$ is a surjective map that fixes monotone sofas.
