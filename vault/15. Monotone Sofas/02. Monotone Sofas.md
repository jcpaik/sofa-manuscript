We have just shown that a sofa $S$ with rotation angle $\omega \in [0, \pi/2]$ is essentially a hallway intersection with angle set $[0, \omega]$ up to rigid transformation, and that it admits a monotonized movement $f_S$ with tangent hallways ([[01. Moving Hallway Problem#^thm-sofa-iff-hallway-intersection]]). So $S \subseteq P_\omega$ and $S \subseteq L_S(t)$ for any angle $t \in [0, \omega]$. The idea of this section is to take the intersection $\mathcal{M}(S) = P_\omega \cap \bigcap_{t \in [0, \omega]} L_S(t)$ of the parallelogram and all the tangent hallways to make a monotonically larger sofa. This intersection $\mathcal{M}(S)$ 

Let $S$ be an arbitrary hallway intersection in standard position with rotation angle $\omega \in [0, \pi/2]$ and angle set $\Theta$. We defined the tangent hallway $L_S(t)$ for any angle $t \in \Theta$, so that $L_S(t)$ contains $S$ ([[01. Moving Hallway Problem#^thm-in-tangent-hallway]]) and touches $S$ in the outer walls $a_S(t)$ and $c_S(t)$ ([[01. Moving Hallway Problem#^cor-tangent-hallway]]). Here, we define the _monotonization_ of $S$ as the intersection of the parallelogram $P_\omega$ and the tangent hallways pushed towards $S$.
