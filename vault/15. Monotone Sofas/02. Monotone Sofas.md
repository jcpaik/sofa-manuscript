...
We inspect the exact condition where $\mathcal{N}(K) \subseteq K$ holds for a general cap $K$. Note that the niche $\mathcal{N}(K)$ is the union of the wedges $T_K(t)$ for all $t \in \Theta$. 

> __Definition [cap-wedge-gap].__ Fix any cap $K$ with rotation angle $\omega \in [0, \pi/2]$. If $t \in [0, \omega]$ and $t < \pi/2$, define $W_K(t)$ as the intersection of the $x$-axis and the inner wall $b_K(t)$. If $\omega = \pi/2$, define $W_K(\pi/2) = A^-_K(\pi/2)$. For any $t \in [0, \omega]$, define $w_K(t) = (A^-_K(0) - W_K(t)) \cdot u_0$ as the signed distance measured from the intersection $W_K(t)$ to the the rightmost vertex $A_K^-(0)$ of the cap.
> 
> Similarly, define $Z_K(t)$ as the intersection of the line $l_\omega$ and the inner wall $d_K(t)$. For any $t \in [0, \omega]$, define $z_K(t) = (C^+_K(\omega) - Z_K(t)) \cdot v_\omega$ as the signed distance measured from the intersection $Z_K(t)$ to the leftmost vertex $C^+_K(\omega)$ of the cap. ^def-cap-wedge-gap

If $t \in (0, \omega)$ and the corner $\mathbf{x}_K(t)$ happens to contain the rightmost (resp. leftmost) point of the wedge $T_K(t)$ is . 

> __Lemma [cap-wedge-gap].__ Fix any cap $K$, rotation angle $\omega \in [0, \pi/2]$ and an angle $t \in (0, \omega)$. If the corner $\mathbf{x}_K(t)$ is in $K$ and the gaps $z_K(t)$ and $w_K(t)$ are nonnegative, then the wedge $T_K(t)$ is contained in $K \setminus \delta K$. ^lem-cap-wedge-gap
