...
The following is an immediate corollary of [[01. Moving Hallway Problem#^def-hallway-intersection]] and [[01. Moving Hallway Problem#^cor-sofa-is-hallway-intercsection]].
...
> __Theorem [monotone-sofa-cap].__ Let $S = \mathcal{M}(S')$ be any monotonization of a hallway intersection $S'$ with any angle set $\Theta$. Then the cap $\mathcal{C}(S)$ of $S$ is the cap $K = \mathcal{C}(S')$ of $S'$. Consequently, $S = K \setminus \mathcal{N}(K)$ where $K$ is the cap of $S$, and $\mathcal{N}(K) \subseteq K \setminus \delta K$. ^thm-monotone-sofa-cap

_Proof._  □

Specializing [[02. Monotone Sofas#^thm-monotone-sofa-cap]] to $\Theta = [0, \omega]$, any monotone sofa $S$ is equal to $K \setminus \mathcal{N}(K)$ where $K$ is the cap of $S$.