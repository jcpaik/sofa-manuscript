...
We show that for any monotone sofa with cap $K$, the niche $\mathcal{N}(K)$ is contained in $K$. Proving this requires some preparations. We first define geometric primitives derived from the cap $K$. By [[01. Moving Hallway Problem#^def-tangent-hallway]] and [[01. Moving Hallway Problem#^def-rotating-hallway-parts]], the tangent hallway $L_K(t)$ has the walls $a_K(t), b_K(t), c_K(t), d_K(t)$ where the outer walls $a_K(t) = l_K(t)$ and $c_K(t) = l_K(t+\pi/2)$ are the upper tangent lines of $K$. We also define the vertices of the cap $K$.
