We have just shown that any sofa $S$ with rotation angle $\omega \in [0, \pi/2]$ is essentially a hallway intersection in standard position with angle set $[0, \omega]$ up to rigid transformation ([[01. Moving Hallway Problem#^cor-sofa-iff-hallway-intersection]]), and that it admits a monotonized sofa movement $f_S$ with tangent hallways $L_S(t)$ containing $S$ ([[01. Moving Hallway Problem#^thm-hallway-intersection-is-sofa]]). In this subsection, we define the _monotonization_ of $S$ as the intersection $\mathcal{M}(S) = P_\omega \cap \bigcap_{t \in [0, \omega]} L_S(t)$ of the parallelogram and all the tangent hallways ([[02. Monotone Sofas#^def-monotonization]]). It turns out that $\mathcal{M}(S)$ is itself a sofa containing $S$ ([[02. Monotone Sofas#^thm-monotonization-is-hallway-intersection]]), so we call $\mathcal{M}(S)$ a _monotone sofa_ ([[02. Monotone Sofas#^def-monotone-sofa]]) and safely assume that a sofa of maximum area is monotone.

Moreover, a monotone sofa $\mathcal{M}(S)$ is equal to a _cap_ $K$ subtracted by its _niche_ $\mathcal{N}(K)$ ([[02. Monotone Sofas#^thm-monotonization-eq]]). The cap $K$ ([[02. Monotone Sofas#^def-cap]]) is a convex body bounded from above by the outer walls $a_S(t), c_S(t)$ of $L_S(t)$ as tangent lines and from below by the bottom edges of $P_\omega$. The niche $\mathcal{N}(K)$ ([[02. Monotone Sofas#^def-niche]]) is the union of triangular regions carved out by the inner corner $Q^-_S(t)$ of the hallway $L_S(t)$. The niche $\mathcal{N}(K)$ turns out to be a subset of the cap $K$ ([[02. Monotone Sofas#^thm-niche-in-cap]]), so the area $\mathcal{A}(K) = |K| - |\mathcal{N}(K)|$ can be analyzed separately in terms of the cap $K$ and its niche $\mathcal{N}(K)$. In general, for any cap $K$ (which may not be the cap of a sofa) we show that the niche $\mathcal{N}(K)$ is contained in $K$ if and only if the set $K \setminus \mathcal{N}(K)$ is connected ([[02. Monotone Sofas#^thm-monotonization-connected-iff]]).
...
The following is an immediate corollary of [[01. Moving Hallway Problem#^def-hallway-intersection]] and [[01. Moving Hallway Problem#^cor-sofa-is-hallway-intersection]].
...
We describe the geometry of an arbitrary cap $K$ with angle set $\Theta$ and rotation angle $\omega$. We define geometric primitives derived from the cap $K$. By [[01. Moving Hallway Problem#^def-tangent-hallway]] and [[01. Moving Hallway Problem#^def-rotating-hallway-parts]], the tangent hallway $L_K(t)$ has the walls $a_K(t), b_K(t), c_K(t), d_K(t)$ where the outer walls $a_K(t) = l_K(t)$ and $c_K(t) = l_K(t+\pi/2)$ are the upper tangent lines of $K$. We also define the vertices of the cap $K$.
...
By [[03. Geometric Results#^thm-convex-set-support-disjoint]], we have the followings.
...
_Proof._ As $Q^+_S(t)$ is the intersection of two half-planes of normal angle $t$ and $t + \pi/2$, the set $K = \mathcal{C}(S)$ is a convex set supported by the set $\Theta \cup (\Theta + \pi/2) \cup \{\pi + \omega, 3\pi/2\}$ ([[03. Geometric Results#^thm-convex-set-support]]). Moreover, $p_K(\omega) = p_K(\pi/2) = 1$ because $S$ is in standard position so $p_{S}(\omega)=p_S(\pi/2)=1$. It remains to verify $p_K(\pi + \omega) = p_K(3\pi/2) = 0$.
...
With [[02. Monotone Sofas#^lem-cap-same-support-function]] we will understand $S$ mostly in terms of $K$. We show the first main theorem that the monotonization $\mathcal{M}(S)$ is also a hallway intersection. The hard part is to show that $\mathcal{M}(S)$ is connected.
...
> __Corollary [upper-boundary-arc].__ If $K$ has nonempty interior, by [[03. Geometric Results#^thm-boundary-parametrization-segment]] the function $\mathbf{x}_{K, A^-_K(0), C^+_K(\omega)}$ is a Jordan arc parametrization of $\delta K$ from $A_K^-(0)$ to $C^+_K(\omega)$. In particular, $\delta K$ is connected. ^cor-upper-boundary-arc
...
We now apply [[02. Monotone Sofas#^lem-niche-in-cap]] to the wedge $T = T_K(t)$.

> __Definition [cap-wedge-gap].__ Fix any cap $K$ with rotation angle $\omega \in [0, \pi/2]$ and an angle $t \in (0, \omega)$. Define the values $W_K(t), w_K(t), Z_K(t), z_K(t)$ as the values $W_T, w_T, Z_T, z_T$ in [[02. Monotone Sofas#^lem-niche-in-cap]] when $T = T_K(t)$. So for example, $W_K(t) = b_K(t) \cap l_{\pi/2}$. ^def-cap-wedge-gap

The left and right endpoints $W_K(t)$ and $Z_K(t)$ are already controlled.

> __Lemma [wedge-ends-in-cap].__ Let $K$ be any cap with angle set $\Theta$ and rotation angle $\omega$. For any angle $t \in (0, \omega)$, we have $w_K(t), z_K(t) \geq 0$. ^lem-wedge-ends-in-cap

_Proof._ We will show that $w_K(t) \geq 0$. We need to show that the point $A_K^-(0)$ is further than $W_K(t) = b_K(t) \cap l_{\pi/2}$ in the positive direction $u_0$ of $x$-axis. The point $q = a_K(t) \cap l(\pi/2, 1)$ is further than $W_K(t) = b_K(t) \cap l(\pi/2, 0)$ in the direction of $u_0$, because $a_K(t)$ and $b_K(t)$ form the boundary of a unit-width vertical strip rotated counterclockwise by $t$. The point $A^-_K(t)$ is further than $q = a_K(t) \cap a_K(\pi/2)$ in the direction of $u_0$ because $K$ is a convex body. Finally, the point $A^-_K(0)$ is further than $A_K^-(t)$ in the direction of $u_0$ again because $K$ is a convex body. Summing up, the points $W_K(t), q, A_K^-(t), A_K^-(0)$ are aligned in the direction of $u_0$, completing the proof. A symmetric argument can prove $z_K(t) \geq 0$. □
...
(3 $\Rightarrow$ 1) If $\mathbf{x}(t) \not \in F_\omega^\circ$, then $T_K(t)$ is an empty set. If $\mathbf{x}(t) \in K$, then by [[02. Monotone Sofas#^lem-niche-in-cap]] and [[02. Monotone Sofas#^lem-wedge-ends-in-cap]] we have $T_K(t) \subseteq K$.

(2 $\Rightarrow$ 4) As $\delta K$ is disjoint from $\mathcal{N}(K)$, we have $\delta K \subseteq \mathcal{M}(K)$. We show that $\mathcal{M}(K)$ is connected. First, $\delta K$ is connected ([[02. Monotone Sofas#^cor-upper-boundary-arc]]). Next, take any point $p \in \mathcal{M}(K)$. Take the ray $r = R_p(\pi/2)$ in the upward direction $u_{\pi/2}$ starting from $p$. Then $r$ touches a point in $\delta K$ as $p \in K$. Moreover, $r$ is disjoint from $\mathcal{N}(K)$ as the set $\mathcal{N}(K) \cup (\mathbb{R}^2 \setminus F_\omega)$ is closed in the downwards direction. Now $r \cap K$ is a line segment inside $\mathcal{M}(K)$ that connects the arbitrarily chosen point $p \in \mathcal{M}(K)$ to a point in $\delta K$. So $\mathcal{M}(K)$ is connected.
...
_Proof._ By [[02. Monotone Sofas#^thm-monotonization-is-hallway-intersection]], monotonization $\mathcal{M}(S)$ is connected. By [[02. Monotone Sofas#^thm-monotonization-connected-iff]], the niche $\mathcal{N}(K)$ is contained $K \setminus \delta K$. □
...
_Proof._ By [[02. Monotone Sofas#^thm-niche-in-cap]] on $S'$ we have $\delta K \subseteq \mathcal{M}(S') = S$. On the other hand we have $S = \mathcal{M}(S') \subseteq K$. So $\delta K \subseteq S \subseteq K$ and this implies that $\mathcal{C}(S) = K$. □

> __Corollary [monotone-sofa-cap].__ Specializing [[02. Monotone Sofas#^thm-monotone-sofa-cap]] to $\Theta = [0, \omega]$, any monotone sofa $S$ is equal to $K \setminus \mathcal{N}(K)$ where $K$ is the cap of $S$. ^cor-monotone-sofa-cap