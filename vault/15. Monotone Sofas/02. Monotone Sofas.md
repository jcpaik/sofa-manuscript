...
Moreover, a monotone sofa $\mathcal{M}(S)$ is equal to a _cap_ $K$ subtracted by its _niche_ $\mathcal{N}(K)$. The cap $K$ is a convex body bounded from above by the outer walls of $L_S(t)$ as tangent lines and from below by the bottom edges of $P_\omega$. The niche $\mathcal{N}(K)$ is the union of triangular regions carved out by the inner corner of $L_S(t)$. The niche $\mathcal{N}(K)$ turns out to be a subset of the cap $K$, so the area $\mathcal{A}(K) = |K| - |\mathcal{N}(K)|$ can be analyzed separately in terms of the cap $K$ and its niche $\mathcal{N}(K)$.
