We have just shown that any sofa $S$ with rotation angle $\omega \in [0, \pi/2]$ is essentially a hallway intersection in standard position with angle set $[0, \omega]$ up to rigid transformation ([[01. Moving Hallway Problem#^cor-sofa-iff-hallway-intersection]]), and that it admits a monotonized sofa movement $f_S$ with tangent hallways $L_S(t)$ containing $S$ ([[01. Moving Hallway Problem#^thm-hallway-intersection-is-sofa]]). In this subsection, we define the _monotonization_ of $S$ as the intersection $\mathcal{M}(S) = P_\omega \cap \bigcap_{t \in [0, \omega]} L_S(t)$ of the parallelogram and all the tangent hallways. It turns out that $\mathcal{M}(S)$ is itself a sofa containing $S$, so we call $\mathcal{M}(S)$ a _monotone sofa_ and safely assume that a sofa of maximum area is monotone.

Moreover, a monotone sofa $\mathcal{M}(S)$ is equal to a _cap_ $K$ subtracted by its _niche_ $\mathcal{N}(K)$. The cap $K$ is a convex body bounded from above by the outer walls $a_S(t), c_S(t)$ of $L_S(t)$ as tangent lines and from below by the bottom edges of $P_\omega$. The niche $\mathcal{N}(K)$ is the union of triangular regions carved out by the inner corner $Q^-_S(t)$ of $L_S(t)$. The niche $\mathcal{N}(K)$ turns out to be a subset of the cap $K$, so the area $\mathcal{A}(K) = |K| - |\mathcal{N}(K)|$ can be analyzed separately in terms of the cap $K$ and its niche $\mathcal{N}(K)$. In general, for any cap $K$ (which may not be the cap of a sofa) we show that the niche $\mathcal{N}(K)$ is contained in $K$ if and only if the set $K \setminus \mathcal{N}(K)$ is connected.
