...
> __Theorem [main-cap].__ For any cap $K \in \mathcal{K}_\omega$ with rotation angle $\omega \in (0, \pi/2]$, if the rotation path $\mathbf{x} : [0^\circ, 90^\circ] \to \mathbb{R}^2$ of $S$ is injective, and it is always on or above the line $y = -1$, then the area of $S$ is at most $1 + \pi^2/8 = 2.2337\dots$. ^thm-main-cap

Note that the rotation path $\mathbf{x}$ of a monotone sofa in [[01. Moving sofa problem#^con-injectivity]] is equal to the path $\mathbf{x}$

[[20. Sofa area functional#^thm-main-cap]] is 
