...
> __Theorem [main-cap].__ For any cap $K \in \mathcal{K}_\omega$ with rotation angle $\omega \in (0, \pi/2]$, if the rotation path $\mathbf{x}_K : [0^\circ, 90^\circ] \to \mathbb{R}^2$ of $S$ is injective, and it is always on the fan $F_\omega$, then the area of $S$ is at most $1 + \omega^2/2$. ^thm-main-cap
