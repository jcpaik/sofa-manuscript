...
> __Proposition [upper-boundary-jordan-arc].__ Unless $\omega = \pi/2$ and $K$ is the vertical line segment $\left\{ 0 \right\}\times [0, 1]$, the cap $K$ has nonempty interior and its upper boundary $\delta K$ is the Jordan arc $\mathbf{b}_K^{0-, \pi/2 + \omega}$ ([[15. Parametrization of boundary#^thm-param-segment]]) connecting the endpoints $A_K^-(0)$ and $C_K^+(\omega)$. ^pro-upper-boundary-jordan-arc
...
(2 $\Rightarrow$ 4) As $\delta K$ is disjoint from $\mathcal{N}(K)$, we have $\delta K \subseteq S$. We show that $S$ is connected. First, note that $\delta K$ is connected by [[15. Cap contains niche#^pro-upper-boundary-jordan-arc]]. Next, take any point $p \in S$. Take the half-line $r$ starting from $p$ in the upward direction $v_0$. Then $r$ touches a point in $\delta K$ as $p \in K$. Moreover, $r$ is disjoint from $\mathcal{N}(K)$ as the set $\mathcal{N}(K) \cup (\mathbb{R}^2 \setminus F_\omega)$ is closed in the direction $-v_0$. Now $r \cap K$ is a line segment inside $S$ that connects the arbitrary point $p \in S$ to a point in $\delta K$. So $S$ is connected.
