...
> __Proposition [upper-boundary-interior].__ The set $K \setminus \delta K$ is the interior of cap $K$ with rotation angle $\omega$ in the subset topology of $F_\omega$. ^pro-upper-boundary-interior

> __Proposition [upper-boundary-param].__ The upper boundary $\delta K$ has an arc-length parametrization $\mathbf{b}_K^{0-, \pi/2 + \omega}$ ([[15. Parametrization of boundary#^def-closed-param]]) from $A_K^-(0)$ to $C_K^+(\omega)$ in the counterclockwise direction along $\partial K$. ^pro-upper-boundary-param
...
_Proof._ The conditions 1 and 2 are equivalent because the niche $\mathcal{N}(K)$ is open in the subset topology of $F_\omega$, and the set $K \setminus \delta K$ is the interior of $K$ in the subset topology of $F_\omega$ ([[15. Cap contains niche#^pro-upper-boundary-interior]]).
...
(2 $\Rightarrow$ 4) As $\delta K$ is disjoint from $\mathcal{N}(K)$, we have $\delta K \subseteq S$. We show that $S$ is connected. First, note that $\delta K$ is connected by [[15. Cap contains niche#^pro-upper-boundary-param]]. Next, take any point $p \in S$. Take the half-line $r$ starting from $p$ in the upward direction $v_0$. Then $r$ touches a point in $\delta K$ as $p \in K$. Moreover, $r$ is disjoint from $\mathcal{N}(K)$ as the set $\mathcal{N}(K) \cup (\mathbb{R}^2 \setminus F_\omega)$ is closed in the direction $-v_0$. Now $r \cap K$ is a line segment inside $S$ that connects the arbitrary point $p \in S$ to a point in $\delta K$. So $S$ is connected.
