In this subsection, we will show the following theorem. 
...
With [[15. Cap contains niche#^thm-niche-in-cap]], the area $|S| = |K| - |\mathcal{N}(K)|$ of a monotone sofa can be understood separately in terms of its cap and niche. The proof is elementary but needs careful analysis on the geometry of monotone sofas.
...
Now we prove [[15. Cap contains niche#^thm-niche-in-cap]]. In fact, we identify the exact condition where $\mathcal{N}(K) \subseteq K$ for a general cap $K$ ([[10. Monotone Sofa#^def-cap]]) not necessary associated to a moving sofa.
