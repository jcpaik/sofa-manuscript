...
> __Proposition [upper-boundary-of-cap].__ For any cap $K$ with angle set $\Theta$, the upper boundary $\delta K$ is exactly the points of $K$ making contact with the outer walls $a_K(t)$ and $c_K(t)$ of tangent hallways $L_K(t)$ touch $K$ for every $t \in \Theta$. ^pro-upper-boundary-of-cap
