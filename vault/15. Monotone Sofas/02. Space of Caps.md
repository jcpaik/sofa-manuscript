...
we change the maximization of the area of a moving sofa to the maximization of a _area functional_ on the space of caps.
...
Note that the space of caps $\mathcal{K}_\Theta$ is a _convex space_: for every caps $K_1, K_2 \in \mathcal{K}_\Theta$ and any $\lambda \in [0, 1]$, their convex combination $K = (1-\lambda) K_1 + \lambda K_2$ using Minkowski sum is also a cap in $\mathcal{K}_\Theta$.
