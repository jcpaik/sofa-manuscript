In this section, we will change the sofa area optimization problem to optimizing the _area functional_ on the space of caps.
...
Note that the space of caps $\mathcal{K}_\Theta$ is a _convex space_: for every caps $K_1, K_2 \in \mathcal{K}_\Theta$ and any $\lambda \in [0, 1]$, we can take their convex combination $K = (1-\lambda) K_1 + \lambda K_2$ using Minkowski sum, then $K \in \mathcal{K}_\Theta$ is also a cap.

We could assume that a sofa $S$ of maximum area is actually monotone, so that $S = K \setminus \mathcal{N}(K)$ for a cap $K \in \mathcal{K}_\omega$. In [[01. Monotone Sofa#^thm-monotonization-connected-iff]] we showed that $\mathcal{N}(K) \subseteq K$. So the area of $S$ is equal to 
