In [[@gerverMovingSofaCorner1992]], Gerver constructed a special subset $\mathcal{T}$ of all possible sofas and showed that a sofa of maximum area exists in $\mathcal{T}$. In this section, we rephrase his ideas and define our version of the compact subset of  provide a we will call the space of _monotone sofas_.
...
> __Definition [hallway-intersection].__ Let $\omega \in [0, \pi/2]$ and $\Theta$ be a closed set such that $\left\{ 0, \omega \right\} \subseteq \Theta \subseteq [0, \omega]$. A connected shape $S$ is a _hallway intersection_ with _rotation angle_ $\omega$ and _angle set_ $\Theta$ if the followings hold.
...
We generalized [[01. Monotone Sofa#^def-hallway-intersection]] to an arbitrary set $\Theta$ of angles for future use. Gerver essentially showed that [[01. Monotone Sofa#^def-hallway-intersection]] with angle set $\Theta = [0, \omega]$ is equivalent to $S$ being a moving sofa with rotation angle $\omega$. That is, any hallway intersection with angle set $[0, \omega]$ admits a continuous sofa movement. so. He did by taking the rotated hallways containing $S$, and pushing it toward $S$ so that its outer walls touch $S$.
...
> __Proposition [tangent-hallway].__ For any hallway intersection $S$ with angle set $\Theta$, and for any $t \in \Theta$, the tangent hallway contains $S$. ^pro-tangent-hallway

Note that the proposition above holds because the hallway is rotated exactly $90^\circ$. If the angle of hallway was less than $90^\circ$, then 
...
_Proof._ We observe that the tangent hallway $L_S(t)$ with respect to $t \in [0, \omega]$ moves continuously. This is because the position of $L_S(t)$ depends on the values $p_S(t)$ and $p_S(t + \pi / 2)$ of the support function $p_S$ which is continuous. □

We name the intersection of tangent hallways of $S$ Gerver takes in [[@gerverMovingSofaCorner1992]] as the _monotonization_ of $S$. Without loss of generality, we translate any hallway intersection $S$ with rotation angle $\omega$ so that it satisfies $p_S(\omega) = p_S(\pi/2) = 1$, before taking the monotonization.
...
We observe that the monotonization $\mathcal{M}(S)$ is equal to a convex set called the _cap_ $K$ of $S$, subtracted by a union of convex sets called the _niche_ $\mathcal{N}(K)$ of $K$. 

We will soon show that $\mathcal{M}(S)$ is also connected, so is itself a hallway intersection. Note that in [[@gerverMovingSofaCorner1992]], he missed showing that $\mathcal{M}(S)$ is connected for the rest of his arguments to work.

To do so, 

> __Definition [parallelogram].__ For any angle $\omega \in [0, \pi/2]$, define the _parallelogram_ $P_\omega = H \cap R_\omega(V)$ with rotation angle $\omega$. If $\omega = \pi / 2$, then $P_\omega$ is the horizontal strip $H$. For any $\omega < \pi / 2$, let $p_{\omega} = l(\omega, 1) \cap l(\pi/2, 1)$ be the upper-right corner of $P_\omega$. ^def-parallelogram
...
In [[@gerverMovingSofaCorner1992]], Gerver did not show that $\mathcal{M}(S)$ is connected, which is required for properly enlarging a sofa $S$ to $\mathcal{M}(S)$. We show the connectedness of $\mathcal{M}(S)$ by understanding its geometric structure. 

> __Remark [connectivity-gerver].__ In the proof of in Gerver, he defines $\mathcal{T}$ as the collection of sets $S$, where each $S$ is the horizontal translation of the intersection of $H$, $R_\omega(S)$ and $L_S(t)$ for any $t \in [0, \omega]$. He factually shows that any sofa is contained in an element of $\mathcal{T}$. But he fails to show that that maximum area shape is connected.  ^rem-connectivity-gerver
