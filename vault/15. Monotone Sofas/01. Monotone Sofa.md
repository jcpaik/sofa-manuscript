In Theorem 1 of [[@gerverMovingSofaCorner1992]], Gerver defined a specific subset $\mathcal{T}$ of moving sofas and showed that a moving sofa of maximum area exists in $\mathcal{T}$. In this section, we follow his idea of constructing $\mathcal{T}$ and call the elements of $\mathcal{T}$ as _monotone sofas_. So we can assume that there is a moving sofa $S_{\text{max}}$ of maximum area which is also a monotone sofa. On top of Gerver's work, we also show that any monotone sofa $S$ consists of a convex set $K$ (called the _cap_ of $S$) subtracted by a set $N$ (called the _niche_ of $K$) determined by $K$.
...
To prove [[01. Monotone Sofa#^thm-moving-sofa-iff-hallway-intersection]], we introduce another idea of Gerver. Assume that a shape $S$ is contained in a translation $L'$ of $R_t(L)$, so that $L'$ is a hallway rotated counterclockwise by an angle of $t$. Then we can push $L'$ towards $S$ until the outer walls of $L'$ touch $S$.
...
We omit the proof of [[01. Monotone Sofa#^pro-tangent-hallway]], but note that it depends on the fact that the hallway is rotated exactly by $90^\circ$. We are now ready to prove [[01. Monotone Sofa#^thm-moving-sofa-iff-hallway-intersection]].

_Proof._ (of [[01. Monotone Sofa#^thm-moving-sofa-iff-hallway-intersection]])
...
($\Leftarrow$) The position of the tangent hallway $L_S(t)$ depends only on the values $p_S(t)$ and $p_S(t + \pi / 2)$ of the support function $p_S$ of $S$. As $p_S$ is continuous, the tangent hallway $L_S(t)$ moves continuously with respect to $t \in [0, \omega]$. Let $f_t$ be the rigid transformation that sends $L_S(t)$ to $L$. Then the transformation $f_t(S)$ of $S$ moves continuously with respect to $t$, and is inside $L$ by [[01. Monotone Sofa#^pro-tangent-hallway]]. Because $S$ is contained in a translation of $H$, the width of $S$ measured in the direction of $v_0$ is at most 1. So $S$ is contained in the horizontal side of $L_S(0)$, and by mapping it under $f_0$ we have $f_0(S) \subseteq L_H$. Likewise, $S$ is contained in a translation of $R_\omega(V)$, so the width of $S$ measured in the direction of $v_\omega$ is at most 1. Thus $S$ is contained in the vertical side of $L_S(\omega)$, and by mapping it under $f_\omega$ we have $f_\omega(S) \subseteq L_V$. □
...
The immediate consequence of [[01. Monotone Sofa#^pro-tangent-hallway]] is the following.
...
Our [[01. Monotone Sofa#^def-cap]] of cap is generalized to an arbitrary set $\Theta$ of angles for future use. In particular, we will later approximate a cap of full angle set $[0, \omega]$ by a polygonal cap of a finite angle set $\Theta$. We continue by definiting the cap of a moving sofa $S$.
...
$$
> Then $\mathcal{C}(S)$ is a cap with the angle set $[0, \omega]$ according to [[01. Monotone Sofa#^def-cap]]. ^pro-cap-hallway-intersection

With [[01. Monotone Sofa#^pro-cap-hallway-intersection]], we can call $\mathcal{C}(S)$ the _cap_ of $S$. We omit the proof but note that it only depends on the condition $p_S(\omega) = p_S(\pi/2) = 1$. The tangent hallways of a moving sofa $S$ are completely determined by its cap $K = \mathcal{C}(S)$ as the following.
...
With [[01. Monotone Sofa#^pro-cap-same-support-function]], we have $Q^-_S(t) = Q^-_K(t)$ in particular, and so we can define the set $N$ purely in terms of the cap $K$ as the following.
...
> __Theorem [monotonization-is-sofa].__ Let $S$ be a moving sofa with rotation angle $\omega$ satisfying $p_S(\omega) = p_S(\pi/2) = 1$. Then the monotonization $\mathcal{M}(S)$ is a moving sofa with rotation angle $\omega$ containing $S$. Moreover, $\mathcal{M}(S)$ shares the same cap with $S$; that is, $\mathcal{C}(\mathcal{M}(S)) = \mathcal{C}(S)$. ^thm-monotonization-is-sofa
...
As the set $\mathcal{M}(S)$ is connected, and satisfies the second condition of [[01. Monotone Sofa#^thm-moving-sofa-iff-hallway-intersection]] by definition, $\mathcal{M}(S)$ is a moving sofa with rotation angle $\omega$. By [[01. Monotone Sofa#^cor-monotonization-contains]], the moving sofa $\mathcal{M}(S)$ contains $S$. □

With [[01. Monotone Sofa#^thm-monotonization-is-sofa]], call any monotonization of any sofa a _monotone sofa_. As monotonizing a sofa never decreases its area, we can assume that a sofa of maximum area is monotone.

By [[01. Monotone Sofa#^cor-monotonization-eq]], any monotone moving sofa $S$ with rotation angle $\omega$ (resp. monotone hallway intersection $S$ with angle set $\Theta$) is its cap $K = \mathcal{C}(S)$ with angle set $[0, \omega]$ (resp. $\Theta$) subtracted by its niche $\mathcal{N}(K)$. With this, we will assume that a moving sofa $S$ is a monotone sofa without loss of generality, and understand $S$ mainly in terms of its cap $K$.
