...
We generalized [[01. Monotone Sofa#^def-hallway-intersection]] to an arbitrary set $\Theta$ of angles for future use. Gerver essentially showed that any hallway intersection with angle set $[0, \omega]$ admits a continuous sofa movement. He did by taking the rotated hallways containing $S$, and pushing it toward $S$ so that its outer walls touch $S$.
