...
_Proof._ (Sketch) There is a unique extension of $\beta$ on the set $\Pi = \Theta \cup (\Theta + \pi/2) \cup \{\pi + \omega, 3\pi/2\}$ such that $\int_{t \in \Pi} u_t \, \beta(dt) = 0$. 

Use [[03. Geometric Results#^thm-supported-gauss-measure]] to construct such a cap $K$ and show the uniqueness. □
