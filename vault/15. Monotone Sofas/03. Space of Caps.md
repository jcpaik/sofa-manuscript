...
With [[02. Monotone Sofas#^thm-niche-in-cap]], the maximization of sofa area with rotation angle $\omega$ is nearly equivalent to maximizing the following _area functional_ on the space of caps $\mathcal{K}_\omega$.
...
Note however that not all cap $K \in \mathcal{K}_\Theta$ gives rise to a hallway intersection $K \setminus \mathcal{N}(K)$, as $K$ might be too wide making $K \setminus \mathcal{N}(K)$ disconnected. Indeed, [[02. Monotone Sofas#^thm-monotonization-connected-iff]] specifies exactly when $K$ gives rise to a sofa, and the domain of the caps of monotone sofas is narrower than the space of all caps. Still, we expect that the maximizer of $\mathcal{A}$ over all $\mathcal{K}_\omega$ to be the cap of Gerver's sofa, and this is sufficient to show that Gerver's sofa attains the maximum area.
