...
In the introduction, we defined a moving sofa $S$ as a connected, nonempty and compact subset of $L_H$ that can be moved inside $L$ by a continuous rigid motion to a subset of $L_V$. However, it turns out that $S$ has to be a subset $L_H$ is too restrictive for our future use. Thus, we call any translation of such $S$ as also a moving sofa as well without loss of generality from now on.
