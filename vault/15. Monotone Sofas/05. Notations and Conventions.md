...
We set up basic notions and conventions that will be throughly used in the rest of the document. It is important to read this first before attempting to read the rest. 

# Details

> __Definition [shape].__ In this paper, a _shape_ is a nonempty compact subset of $\mathbb{R}^2$. ^def-shape

Denote the area (Borel measure) of a shape $X$ as $|X|$. For any subset $X$ of $\mathbb{R}^2$, denote the topological closure and interior as $\overline{X}$ and $X^\circ$ respectively. For a subset $X$ of $\mathbb{R}^2$ and a vector $v$ in $\mathbb{R}^2$, define the set $X + v = \left\{ x + v : x \in X \right\}$. For any two subsets $X, Y$ of $\mathbb{R}^2$, the set $X + Y = \left\{ x + y : x \in X, y \in Y \right\}$ is the Minkowski sum of $X$ and $Y$.
...
> __Definition [hallway].__ The planar _hallway_ $L = L_H \cup L_V$ is the union of sets $L_H = (-\infty, 0] \times [-1, 0]$ and $L_V = [-1, 0] \times (-\infty, 0]$, each representing the horizontal and vertical side of $L$ respectively. ^def-hallway

> __Definition [sofa].__ A _moving sofa_ $S$ is a connected, nonempty and compact subset of $\mathbb{R}^2$, such that a translation of $S$ is a subset of $L_H$ that can be moved inside $L$ by a continuous rigid motion to a subset of $L_V$. ^def-sofa
...
For every convex body $K$ with nonempty interior, there is a minimum closed subset $\Pi_K$ of $S^1$ such that $K$ is the intersection of half-planes $H_K(t)$ for $t \in \Pi_K$ (see ). Call this set $\Pi_K$ the _angular support_ of $K$ (see for the full definition). 