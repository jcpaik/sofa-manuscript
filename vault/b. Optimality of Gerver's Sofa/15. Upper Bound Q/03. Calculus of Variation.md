We prepare a minimum amount of language needed to execute the calculus of variation to a concave quadratic functional $f$ on a general convex space $\mathcal{K}$. We will later execute this to a convex quadratic functional $\mathcal{Q}$ defined on extended space $\mathcal{K}^{\varphi_R, \varphi_L}$.
...
To prove that $K$ maximizes a concave quadratic functional $f(K)$ on $\mathcal{K}$, we only need to prove that $Df(K; -)$ is a non-positive linear functional on $\mathcal{K}$. 
