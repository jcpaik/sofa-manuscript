...
We will define a functional $\mathcal{A}_u^{\varphi_R, \varphi_L}$ on the space of caps $\mathcal{K}$, so that it is an upper bound of the sofa area functional $\mathcal{A}$ on the set of x-monotone sofas. Recall that the niche of Gerver's sofa $S_G$ with cap $K_G$ consists of two tails and one core $\mathbf{x}_{K_G}$ restricted to $[\varphi_R, \varphi_L]$. We generalize this notion to arbitrary x-monotone cap $K$.
...
> __Remark [cap-tail-extension].__ The map is NOT convex-linear. In particular, taking the curves $\mathbf{b}_K$ can be seen as taking the Wulff shape $K_b$ of the function $1 - p_K(t)$. In particular, it is not guaranteed that the support function of the convex body $K_b$ matches with $1 - p_K(t)$. Moreover, it is not guaranteed that $K_b$ is smooth even if boundary of $K$ is smooth. This is why we work with full generality in [[b. Optimality of Gerver's Sofa/05. Convex bodies/00. Preface]].  ^rem-cap-tail-extension
...
Define $\mathcal{A}_2^{\varphi_R, \varphi_L}$ as $|K| - \mathcal{I}(\mathbf{z}_K^{\varphi_R, \varphi_L})$.
...
_Proof._ Conditions (1) to (4) in [[b. Optimality of Gerver's Sofa/15. Upper Bound Q/05. Definition of Q#^def-extended-cap-space]] are linear equalities and inequalities on $K$, $\mathbf{b}$ and $\mathbf{d}$. □
