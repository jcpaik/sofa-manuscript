...
_Proof._ $\Theta$ is the set of dyadic angles in $(0, \pi/2)$. For any $a, b \in \Theta \cup \left\{ \pi/2 \right\}$, take the interval $(a, b]$. $na, n b$ are integers. $\sigma_K$ on that interval can be divided into smaller chunks. Supress that with [[c. Optimality of Gerver's Sofa/10. x-Monotone/10. x-Monotoneness#^lem-balanced-discrete-ineq]].
...
We bootstrap [[c. Optimality of Gerver's Sofa/10. x-Monotone/10. x-Monotoneness#^thm-leg-length-bounds]] multiple times.
...
1. By (1) and (2) of [[c. Optimality of Gerver's Sofa/10. x-Monotone/10. x-Monotoneness#^pro-lower-bound-constants]] we have values from $1+T$ to 2.
