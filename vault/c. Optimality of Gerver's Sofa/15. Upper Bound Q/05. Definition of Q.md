...
> __Lemma [corner-always-outside].__ Fix an arbitrary $K \in \mathcal{K}$. Omit the subscript and denote $B_K$ as $B$. The followings are true in succession. ^lem-corner-always-outside
> 
...
> __Lemma [cap-left-right-tail].__ Fix an arbitrary $K \in \mathcal{K}^\mathrm{i}$ and omit the subscript.  $l_{\mathbf{b}_K}(\pi + t) = b_K(t)$ for $t \in \left\{ \varphi_R, \pi/2 \right\}$, and $\mathcal{M}_{\mathbf{b}_K} \subseteq B_K \cap \mathcal{N}_K = \mathcal{N}_K \cap H_K^b(\varphi_R)$. ^lem-cap-left-right-tail
