...
> __Lemma [corner-always-outside].__ Fix an arbitrary $K \in \mathcal{K}$. Omit the subscript and denote $B_K$ as $B$. The followings are true. ^lem-corner-always-outside
> 
> 1. The supporting line $l_B(\varphi^\mathrm{R} + \pi)$ is the line $l_K^\mathrm{R}$ and the corresponding edge $e_B(\varphi^\mathrm{R} + \pi)$ connects two points $W_K(\varphi^\mathrm{R})$ and $l_K^\mathrm{R} \cap \delta K$.
> 2. The line $l_K^\mathrm{R}$ meets the boundary of $K$ 

> __Lemma [cap-left-right-tail].__ $l_{\mathbf{b}_K}(\pi + t) = b_K(t)$ for $t \in \left\{ \varphi_R, \pi/2 \right\}$, and $\mathcal{M}_{\mathbf{b}_K} \subseteq B_K \cap \mathcal{N}_K = \mathcal{N}_K \cap H_K^b(\varphi_R)$. ^lem-cap-left-right-tail
