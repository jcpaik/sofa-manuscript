...
Geometrically, the upper boundary $\delta K$ is an arc from $A_K^-(0)$ to $C_K^+(\omega)$ in the counterclockwise direction along the boundary $\partial K$ of $K$. This is rigorously justified by the following corollary of [[A1. Convex bodies/15. Parametrization of boundary#^cor-closed-param-segment]]. For full details, read the introduction of [[A1. Convex bodies/15. Parametrization of boundary]].
...
If $\omega = \pi/2$, then by $\mathbf{x}_K(t) \in K$, the wedge $T_K(t)$ is the triangle with vertices $W_K(t)$, $\mathbf{x}_K(t)$, and $Z_K(t)$ in counterclockwise order. Note also that $W_K(t)$ is further in the direction of $u_0$ than $Z_K(t)$ ([[15. Monotone sofas/12. Structure of a monotone sofa#^def-further-in-direction]]). As $w_K(t), z_K(t) \geq 0$, this implies that all three vertices of $T_K(t)$ are in $K$.
