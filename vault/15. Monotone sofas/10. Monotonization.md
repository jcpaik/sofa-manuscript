Here, we rigorously define the notion of monotone sofas and establish [[01. Introduction/02. Moving hallway problem#^thm-monotone-sofa]]. First, define the _monotonization_ of any moving sofa $S$ in standard position as the following set.
...
Compare the equation in [[15. Monotone sofas/10. Monotonization#^def-monotonization]] to [[01. Introduction/02. Moving hallway problem#^eqn-monotone]]. The paralleogram $P_\omega$ is the intersection of $H$ and $V_\omega$ ([[10. Notations and conventions/02. Moving sofa#^def-parallelogram]]), and the tangent hallways $L_S(t)$ are the rotating hallways $L_t$ making contact with $S$ as described in [[01. Introduction/02. Moving hallway problem]]. Condition 3 of [[15. Monotone sofas/07. Tangent Hallway#^thm-moving-sofa-iff-hallway-intersection]] implies that the set $\mathcal{M}(S)$ contains $S$.
...
The existence of a monotone sofa attaining the maximum area ([[01. Introduction/02. Moving hallway problem#^thm-monotone-sofa]]) is an immediate consequence of [[15. Monotone sofas/10. Monotonization#^thm-monotonization-is-sofa]]. The rest of this subsection is dedicated to proving [[15. Monotone sofas/10. Monotonization#^thm-monotonization-is-connected]].
...
_Proof._ We have $S \subseteq \mathcal{M}(S) \subseteq \mathcal{C}(S)$ by [[15. Monotone sofas/10. Monotonization#^cor-monotonization-is-larger]] and the definitions of $\mathcal{M}(S)$ and $\mathcal{C}(S)$. So it remains to show that $p_{\mathcal{C}(S)}(t) \leq p_S(t)$ on every $t$ in $J_\omega$. Define $S' = \bigcap_{t \in J_\omega} H_S(t)$, then by convexity of $S$ we have $p_{S'}(t) = p_S(t)$ on every $t \in J_\omega$. By the definition of $\mathcal{C}(S)$ we have $\mathcal{C}(S) \subseteq S'$, and so $p_{\mathcal{C}(S)}(t) \leq p_{S'}(t) = p_S(t)$ for all $t \in J_\omega$. □
