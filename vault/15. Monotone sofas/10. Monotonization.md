...
_Proof._ We have $S \subseteq \mathcal{M}(S) \subseteq \mathcal{C}(S)$ by [[15. Monotone sofas/10. Monotonization#^cor-monotonization-is-larger]] and $L_S(t) \subset Q_S^+(t)$. So it remains to show that $p_{\mathcal{C}(S)}(t) \leq p_S(t)$ on every $t$ in $J_\omega$. Define $S' = \bigcap_{t \in J_\omega} H_S(t)$, then by convexity of $S$ we have $p_{S'}(t) = p_S(t)$ on every $t \in J_\omega$. By the definition of $\mathcal{C}(S)$ we have $\mathcal{C}(S) \subseteq S'$, and so $p_{\mathcal{C}(S)}(t) \leq p_{S'}(t) = p_S(t)$ for all $t \in J_\omega$. □
