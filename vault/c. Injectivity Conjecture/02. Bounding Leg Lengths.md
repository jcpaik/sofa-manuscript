...
> __Theorem [lower-bound-leg-h].__ Fix an interval $[t_{i}, t_{i+1}]$. For any $t \in [t_i, t_{i+1}]$, we have $h_K^{\pm}(t)$ bounded from below by the following value. ^thm-lower-bound-leg-g
$$
(A_{i+1} - C_{i-1}) \cdot u_{i+1}
$$
...
We need at least this to proceed
