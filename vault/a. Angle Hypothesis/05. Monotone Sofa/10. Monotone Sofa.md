...
That $S'$ have rotation angle $\omega$ comes from that $S_t'$ for $t \in [0, \omega]$ is rotated clockwise by $t$. That $S'$ contains $S$ comes from [[a. Angle Hypothesis/05. Monotone Sofa/10. Monotone Sofa#^pro-monotonization-contains-sofa]]. That $S'$ is in standard position comes from $S \subseteq S' \subseteq H \cap V_\omega$. □