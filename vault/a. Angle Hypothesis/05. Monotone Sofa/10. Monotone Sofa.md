...
_Proof._ (of [[a. Angle Hypothesis/05. Monotone Sofa/10. Monotone Sofa#^thm-monotonization]]) We first show that $S' := \mathcal{I}(S)$ is a moving sofa. By [[a. Angle Hypothesis/05. Monotone Sofa/10. Monotone Sofa#^thm-monotonization-is-connected]], it suffices to show that $S'$ can move continuously inside $L$. The rigid transformation $f_{S, t}$ maps $L$ to $L_S(t)$ by [[a. Angle Hypothesis/05. Monotone Sofa/07. Tangent Hallway#^def-tangent-hallway]]. Let $S'_t := f_{S, t}^{-1}(S')$ for $t \in [0, \omega]$, then $S' \subset L_S(t)$ implies $S_t' \subseteq L$. We claim that $S'_t$ is a continuous rigid motion of $S'$ inside $L$ from $L_H$ to $L_V$.
