...
> __Remark [mirror-symmetry].__ We use the mirror reflection of a cap in [[a. Angle Hypothesis/05. Monotone Sofa/15. Cap Contains Niche#^def-mirror-reflection]] and [[a. Angle Hypothesis/05. Monotone Sofa/15. Cap Contains Niche#^pro-mirror-reflection]] extensively to exploit the symmetry without loss of generality. For an example, say we want to show both $w_K(t) > 0$ and $z_K(t) > 0$ for arbitrary $K \in \mathcal{K}_\omega$ and $t \in (0, \omega)$. Then it suffices to show the case $w_K(t) > 0$, as the symmetric case $z_K(t) = w_{K^{\mathrm{m}}}(\omega - t) > 0$ follows by [[a. Angle Hypothesis/05. Monotone Sofa/15. Cap Contains Niche#^pro-mirror-reflection]]. ^rem-mirror-symmetry
