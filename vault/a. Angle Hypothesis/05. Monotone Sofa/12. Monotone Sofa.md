...
We show that any intersection $\mathcal{I}(S)$ of a moving sofa $S$ is equal to the _cap_ $K$ of $S$ minus the _niche_ $\mathcal{N}(K)$ of $K$ ([[a. Angle Hypothesis/05. Monotone Sofa/12. Monotone Sofa#^thm-monotonization-structure]]). We first define the notion of _cap_ as a kind of convex body.
...
_Proof._ The second condition of [[a. Angle Hypothesis/05. Monotone Sofa/12. Monotone Sofa#^def-cap]] on $\mathcal{C}(S)$ is satisfied by [[a. Angle Hypothesis/05. Monotone Sofa/10. Intersection Sofa#^def-cap-sofa]]. Since $S$ is in standard position, it suffices to check $p_{\mathcal{C}(S)}(\omega + \pi) = p_{\mathcal{C}(S)}(3\pi/2) = 0$ in the first condition of [[a. Angle Hypothesis/05. Monotone Sofa/12. Monotone Sofa#^def-cap]].

We first prove the case $\omega = \pi/2$. Since $p_S(\pi/2) = 1$ and $S \subseteq \mathcal{C}(S)$ by [[a. Angle Hypothesis/05. Monotone Sofa/10. Intersection Sofa#^pro-cap-contains-sofa]], we can take a point $q \in S \cap l(\pi/2, 1) \subseteq \mathcal{C}(S)$. For any $t \in [0, \pi/2]$, each $Q_S^+(t)$ is closed in the direction of $-v_0$ ([[a. Angle Hypothesis/05. Monotone Sofa/10. Intersection Sofa#^def-closed-in-direction]]). This with $q \in \mathcal{C}(S)$ implies that $q - v_0 \in \mathcal{C}(S)$. Note that $q - v_0 \in l(\pi/2, 0)$, so $q - v_0 \in \mathcal{C}(S) \subseteq P_{\pi/2}$ implies $p_{\mathcal{C}(S)}(3\pi/2) = 0$ as we desired.
...
> __Theorem [monotonization-structure].__ Let $S$ be a moving sofa with rotation angle $\omega \in (0, \pi/2]$ in standard position. The intersection $\mathcal{I}(S)$ from $S$ is equal to $K \setminus \mathcal{N}(K)$ where $K := \mathcal{C}(S)$ is the cap of $S$. ^thm-monotonization-structure
...
_Proof._ Consequence of [[a. Angle Hypothesis/05. Monotone Sofa/10. Intersection Sofa#^lem-cap-same-support-function]]. □

We now show that taking the intersection twice does not help in enlarging a sofa.
...
_Proof._ Let $S' := \mathcal{I}(S)$ and $K := \mathcal{C}(S)$. By [[a. Angle Hypothesis/05. Monotone Sofa/12. Monotone Sofa#^thm-monotonization-structure]] we have $S' = K \setminus \mathcal{N}(K)$. By [[a. Angle Hypothesis/05. Monotone Sofa/12. Monotone Sofa#^pro-monotonization-cap]] the cap $K$ is also the cap $\mathcal{C}(S')$ of $S'$. So by [[a. Angle Hypothesis/05. Monotone Sofa/12. Monotone Sofa#^thm-monotonization-structure]] again we have $\mathcal{I}(S') = K \setminus \mathcal{N}(K)$, which is equal to $S'$. □

Now define _monotone sofas_ as the image of $S \mapsto \mathcal{I}(S)$.

> __Definition [monotone-sofa].__ Define a _monotone sofa_ as the intersection $\mathcal{I}(S)$ of a moving sofa in standard position. ^def-monotone-sofa
...
_Proof._ Consequence of [[a. Angle Hypothesis/05. Monotone Sofa/10. Intersection Sofa#^thm-monotonization]], [[a. Angle Hypothesis/05. Monotone Sofa/12. Monotone Sofa#^thm-monotonization-idempotent]], and [[a. Angle Hypothesis/05. Monotone Sofa/12. Monotone Sofa#^def-monotone-sofa]]. □