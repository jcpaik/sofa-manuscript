We will always assume without loss of generality that a moving sofa $S$ is in _standard position_ by translating it.
...
_Proof._ If $\omega < \pi/2$, there is a unique translation of $S$ below the supporting lines $l(\omega, 1)$ and $l(\pi/2, 1)$. If $\omega = \pi/2$, there is a unique translation of $S$ below the supporting line $l(\pi/2, 1)$ up to horizontal translations. □

Define the intersection $\mathcal{I}$ in [[a. Angle Hypothesis/03. Overview/03. Monotone Sofa#^eqn-sofa-intersection]] with the tangent hallways $L_t = L_S(t)$.
...
$$
> of strips $H, V_\omega$ and tangent hallways $L_S(t)$.
