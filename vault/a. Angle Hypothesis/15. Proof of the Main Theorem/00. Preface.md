...
> __Definition [right-triangle].__ Say that a right triangle $T$ have _base_ $b$ and _angle_ $\theta$, if an edge $e$ of $T$ connecting right-angled vertex $O$ with other vertex $B$ have length $b$, and the angle at $B$ is equal to $\theta$. We call the length of the edge orthogonal to $e$ the _height_ of $T$. ^def-right-triangle
