...
_Proof._ (of [[a. Angle Hypothesis/01. Introduction/00. Preface#^thm-angle]]) Take the monotone sofa $S_\omega$ with cap $K_\omega$, attaining the maximum area among monotone sofas of rotation angle $\omega$. Then by [[a. Angle Hypothesis/15. Proof of the Main Theorem/00. Preface#^thm-balanced-consumed]], either $|S_{\omega}| < 2.2$ or $S_\omega$ admits another movement of rotation angle $\pi/2$. In either way we have $|S_\omega| \leq |S_{\pi/2}|$, as $2.2 \leq \mu_G \leq |S_{\pi/2}|$ and $S_{\pi/2}$ attains the maximum area among sofas of rotation angle $\pi/2$. □

The moving sofa problem is now the maximization of the sofa area functional $\mathcal{A}_{\pi/2}$ on the space of caps $\mathcal{K}_{\pi/2}$; this summarizes all contributions of this paper.

> __Theorem [sofa-area-functional-summary].__ The maximum area $\mu_{\max}$ of a moving sofa exists, and it is equal to the maximum value of the sofa area functional $\mathcal{A}_{\pi/2} : \mathcal{K}_{\pi/2} \to \mathbb{R}$ on the space of caps with rotation angle $\pi/2$. ^thm-sofa-area-functional-summary
