...
We will show that the polygon cap $K \in \mathcal{K}_\Theta$ that maximizes the polygon sofa area $\mathcal{A}_\Theta(K)$ is balanced ([[a. Angle Hypothesis/10. Balanced Sofa/15. Maximum Polygon Cap#^thm-balanced-polygon-sofa]]). As described in [[a. Angle Hypothesis/03. Outline/05. Balanced Sofa]], we circumvent the issue that $S_\Theta := K \setminus \mathcal{N}_\Theta(K)$ might be disconnected after balancing, by separately balancing the cap $K \in \mathcal{K}_\Theta$ and polygon niche $\mathcal{N}_\Theta(K)$.
...
$$
> For any cap translate $K' \in \mathcal{K}'_\Theta$, define $P_{K'}$, $\mathcal{C}(K')$, $F_{K'}$, $\mathcal{N}_\Theta(K')$, $\mathcal{A}_\Theta(K')$ as simply that of $h := p_{K'}$, that is $P_h$, $\mathcal{C}(h)$, $F_h$, $\mathcal{N}_\Theta(h)$, $\mathcal{A}_\Theta(h)$.
...
> 2. For any $t \in \left\{ \omega, \pi/2 \right\}$ we have $\nu_K(t) + \lambda^1\left( \mathcal{N}(K) \cap l(t, 0) \right) = \sigma_K(t + \pi)$.
