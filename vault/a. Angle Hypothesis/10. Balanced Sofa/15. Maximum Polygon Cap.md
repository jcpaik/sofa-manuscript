...
_Proof._ Consequence of [[a. Angle Hypothesis/05. Monotone Sofa/07. Tangent Hallway#^pro-rotating-hallway-parts]]. □

Now inherit [[a. Angle Hypothesis/10. Balanced Sofa/15. Maximum Polygon Cap#^def-height-extensions]] on $\mathcal{H}_\Theta$ to $\mathcal{K}_\Theta'$. By [[a. Angle Hypothesis/10. Balanced Sofa/15. Maximum Polygon Cap#^pro-niche-extension-compatible]] 
...
_Proof._ By [[a. Angle Hypothesis/10. Balanced Sofa/15. Maximum Polygon Cap#^pro-cap-niche-nef-polygons]] and [[a. Angle Hypothesis/10. Balanced Sofa/15. Maximum Polygon Cap#^def-cap-translate-extensions]], the polygon $\mathcal{N}_{\Theta}(K')$ is a simple Nef polygon with defining hyperplanes $H_{K'}(t)$ and $H_{K'}(t) - u_t$ for $t \in \Theta^{\diamond}$. For any convex body $K$ and fixed $t \in S^1$, the supporting hyperplane $H_K(t)$ is equivariant under translation of $K$ so that $H_{K + \mathbf{v}}(t) = H_K(t) + \mathbf{v}$. So we have the equivariance $\mathcal{N}_\Theta(K') = \mathcal{N}_\Theta(K) + \mathbf{v}$. We then have
$$
\begin{align*}
