...
> __Theorem [polygon-upper-bound].__ For any polygon cap $K \in \mathcal{K}_\Theta$ we have $\mathcal{A}_\Theta(K) = |K| - |\mathcal{N}_\Theta(K)|$. For every cap $K \in \mathcal{K}_\omega$, we have $\mathcal{A}_{\omega}(K) \leq \mathcal{A}_\Theta(K)$.
