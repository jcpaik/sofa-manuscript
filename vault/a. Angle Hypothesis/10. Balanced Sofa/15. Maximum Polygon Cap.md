...
We first choose the right angle $t \in \Theta^\diamond$ to balance $K \in \mathcal{K}_\Theta$.
...
_Proof._ Assume $t \not\in \left\{ \omega, \pi/2 \right\}$. Then $K = \mathcal{C}_\Theta(h) \subseteq \mathcal{C}_\Theta(h^+) = K^+$ by the definition of $\mathcal{C}_\Theta$ on $\mathcal{H}_\Theta$. So $K \subseteq K^+ \subseteq P_\omega$ and $K^+ \in \mathcal{K}_\Theta$ as we want. 

We first show that $K^+$ is a cap translate. □
