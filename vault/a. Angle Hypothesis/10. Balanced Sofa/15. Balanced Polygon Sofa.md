...
> __Definition [angled-cap-space].__ Assume $\omega \in (0, \pi/2]$ and a nonempty finite subset $\Theta$ of $(0, \omega)$. Define $\mathcal{K}_\Theta$ as the subset of $\mathcal{K}_\omega$ where each $K \in \mathcal{K}_\Theta$ is the intersection of supporting planes $H_K(t)$ with normal angles $t \in \Theta \cup (\Theta + \pi/2) \cup \left\{ \pi + \omega, 3\pi/2 \right\}$. ^def-angled-cap-space
...
> __Definition [wedge-gap-polygon].__ Let $K \in \mathcal{K}_\Theta$ be an arbitrary polygon cap. Define its _right wedge gap_ $w_K^\Theta := \min_{t \in \Theta} w_K(t)$ and _left wedge gap_ $z_K^\Theta := \min_{t \in \Theta } z_K(t)$. ^def-wedge-gap-polygon
...
Finally, we show that for any maximizer of 

# Extensions of Polygon Cap

We extend the space $\mathcal{K}_\Theta$ of polygon caps to the space $\mathcal{K}_\Theta'$ of all polygon cap _translates_, and subsequently the space $\mathcal{H}_\Theta$ of functions $h : \Theta \cup (\Theta + \pi/2) \to \mathbb{R}$.
...
> 
> 1. The widths $p_K(\omega) + p_K(\omega + \pi)$ and $p_K(\pi/2) + p_K(3\pi/2)$ of $K$ measured along the angles $\omega$ and $\pi/2$ are exactly one.
...
> __Proposition [height-space-embedding].__ The map $\mathcal{K}_\Theta' \to \mathcal{H}_\Theta$ mapping $K' \in \mathcal{K}'_\Theta$ to its support function $p_{K'}$ on the domain $\Theta \cup (\Theta + \pi/2)$ is a well-defined injection. ^pro-height-space-embedding

_Proof._ Let $K' \in \mathcal{K}'_\Theta$ be arbitrary. By (2) of [[a. Angle Hypothesis/10. Balanced Sofa/15. Balanced Polygon Sofa#^pro-cap-trans-space]], the values of $p_K$ on $\Theta \cup (\Theta + \pi/2) \cup \left\{ \pi + \omega, 3\pi/2 \right\}$ determine $K$ uniquely. By (1) of [[a. Angle Hypothesis/10. Balanced Sofa/15. Balanced Polygon Sofa#^pro-cap-trans-space]], the value of $p_K$ on  □

We will extend the polygon sofa area functional $\mathcal{A}_\Theta$ on $\mathcal{K}_\Theta$ to to a larger space $\mathcal{H}_\Theta$. The cap and niche of a cap generalizes naturally to an arbitrary function $h \in \mathcal{H}_\Theta$. We only need to follow [[a. Angle Hypothesis/10. Balanced Sofa/15. Balanced Polygon Sofa#^eqn-cap-support-ftn]] and [[a. Angle Hypothesis/10. Balanced Sofa/15. Balanced Polygon Sofa#^eqn-niche-support-ftn]].
