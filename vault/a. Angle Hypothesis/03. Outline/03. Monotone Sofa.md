...
$$
All known derivations of Gerver's sofa $S_G$ [@gerverMovingSofaCorner1992; @romikDifferentialEquationsExact2018] assumes the equality $S = \mathcal{I}$ for a maximum-area moving sofa $S$ and (locally) maximizes $S$ by maximizing $\mathcal{I}$ with respect to the locations of $L_t$. However, the equality $S = \mathcal{I}$ may not hold if $\mathcal{I}$ is disconnected, and the connectedness of $\mathcal{I}$ is assumed without proof in [@gerverMovingSofaCorner1992; @romikDifferentialEquationsExact2018].[^connectedness]

In [[a. Angle Hypothesis/05. Monotone Sofa/00. Preface]], we explicitly state and prove that any moving sofa $S$ maximal under set inclusion $\subset$ satisfies the following properties, establishing that $\mathcal{I}$ is connected and $S = \mathcal{I}$ in particular.
...
2. Taking $L_t$ as the tangent hallways of $S$, the intersection $\mathcal{I}$ is connected and the equality $S = \mathcal{I}$ holds in [[a. Angle Hypothesis/03. Outline/03. Monotone Sofa#^eqn-sofa-intersection]].

We will call any moving sofa $S$, not necessarily of maximum area, satisfying the properties (1) and (2) above a _monotone sofa_. Thus it suffices to consider only monotone sofa $S$ for the moving sofa problem.

As a consequence of (1), the outer walls of tangent hallways $L_t$ of a monotone sofa $S$ form the supporting lines of a convex body $K$ that we call the _cap_ of $S$ (see [[a. Angle Hypothesis/03. Outline/03. Monotone Sofa#^fig-monotone-sofa]]). We will identify a monotone sofa $S$ with its cap $K$. In particular, the monotone sofa $S = K \setminus \mathcal{N}(K)$ is completely determined by its cap $K$, where $\mathcal{N}(K)$ is the _niche_ of $K$ which is the union of all triangular regions bounded by the inner walls of $L_t$. We prove $\mathcal{N}(K) \subset K$, so the area $|K| - |\mathcal{N}(K)|$ of $S$ can be understood in terms of its cap and niche separately. With this, the moving sofa problem becomes the maximization of the _sofa area functional_ $\mathcal{A}_{\omega}(K) := |K| - |\mathcal{N}(K)|$ on a convex body $K$.
...
[^connectedness]: In [@gerverMovingSofaCorner1992], Gerver defines the collection $\mathcal{T}$ of all such intersection $\mathcal{I}$ and find a an intersection $\mathcal{I} \in \mathcal{T}$ ($T$ in [@gerverMovingSofaCorner1992]) attaining the maximum area, but it is not shown that $\mathcal{I}$ should be connected. In [@romikDifferentialEquationsExact2018], Romik differentiates $S$ and $\mathcal{I}$ and explicitly assumes $S = \mathcal{I}$ without proof for the derivation of $S_G$. [@kallusImprovedUpperBounds2018] rigorously treats this issue by letting $S$ be the _connected component_ of $\mathcal{I}$ of the largest area instead.