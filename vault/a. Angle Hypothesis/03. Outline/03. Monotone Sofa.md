...
$$
All known derivations of Gerver's sofa $S_G$ [@gerverMovingSofaCorner1992; @romikDifferentialEquationsExact2018] identify a moving sofa $S$ with the intersection $\mathcal{I}$, and (locally) maximizes $S$ by maximizing $\mathcal{I}$ with respect to the locations of $L_t$. However, the equality $S = \mathcal{I}$ for a maximum-area moving sofa $S$ may not hold if $\mathcal{I}$ is disconnected, and in .[^connectedness]

In [[a. Angle Hypothesis/05. Monotone Sofa/00. Preface]], we explicitly state and prove that any moving sofa $S$ maximal under set inclusion $\subset$ satisfies the following properties, establishing $S = \mathcal{I}$ in particular.
...
By identifying a monotone sofa $S$ with its cap $K$, the moving sofa problem becomes the maximization of the _sofa area functional_ $\mathcal{A}_{\omega}(K) := |K| - |\mathcal{N}(K)|$ on a convex body $K$.
