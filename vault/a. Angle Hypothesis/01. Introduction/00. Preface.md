...
In particular, the first assumption that the maximum-area $S_{\max}$ have rotation angle $\omega = \pi/2$ seems indispensable in establishing (or reasonably convincing) $S_{\max} = S_G$. Gerver derived his sofa $S_G$ for the rotation angle $\omega = \pi/2$ only (Theorem 2 of [@gerverMovingSofaCorner1992]), although it seems possible that the same calculation could be done for $\omega < \pi/2$. Romik [@romikDifferentialEquationsExact2018] writes that $\omega = \pi/2$ "seems extremely plausible". But note that a moving sofa may rotate less than $\pi/2$ in its full movement (see XX).
