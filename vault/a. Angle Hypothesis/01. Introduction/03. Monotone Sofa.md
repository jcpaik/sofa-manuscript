A fundamental idea of Gerver [@gerverMovingSofaCorner1992] is to see a moving sofa as the intersection of rotating hallways. Fix a moving sofa $S$. As $S$ moves inside the hallway $L$, look at the hallway in the perspective of $S$ so that $S$ is fixed and the hallway $L$ rotates and translates around $S$ while containing $S$ (see XX). The sofa $S$ is then a common subset of the rotating hallways, and if $S$ achieves the maximum area then we can assume that $S$ is a connected component of the intersection of rotating hallways.

We can further parametrize the rotating hallways $L_t$ by its angle of rotation $t$.

> __Proposition [moving-sofa-common-subset].__ Fix an arbitrary moving sofa $S$ with rotation angle $\omega \in (0, \pi/2]$. Then $S$ is a common subset of the following sets. ^pro-moving-sofa-common-subset
> 
> 1. For every angle $t \in [0, \omega]$, the rotating hallway $L_t$ which is the hallway $L$ rotated counterclockwise by $t$ and translated.
...
> 
> Furthermore, without loss of generality, we can translate the moving sofa $S$ horizontally and assume that $S$ is contained in $V_\omega$, not its arbitrary translation.[^translation] 

_Proof._ To observe (1) that $S \subseteq L_t$ for every angle $t \in [0, \omega]$, note that the sofa $S$ is rotated initially by a clockwise angle of $0$ and finally by $\omega$ during its movement. By the intermediate value theorem, there is a moment where a copy of $S$ is rotated clockwise by $t$ inside $L$. Push the rotated copy of $S$ in $L$ towards the positive $x$ and $y$ directions, until it makes contact with the outer walls $x=1$ and $y=1$ of $L$. See this configuration in the perspective of $S$ to conclude $S \subseteq L_t$. To observe (2) and (3), note that the sofa $S$ is initialy in the horizontal side $L_H$ and finally in the vertical side $L_H$ rotated clockwise by $\omega$; see the configurations in perspective of $S$ to conclude that $S$ is in $H$ and a translation of $V_\omega$. □

Let $\mathbf{x}(t)$ be the location of the inner corner of the rotated hallway $L_t$. Let $R_t : \mathbb{R}^2 \to \mathbb{R}^2$ be the rotation of $\mathbb{R}^2$ along the origin. Then $L_t = R_t(L) + \mathbf{x}(t)$ and we have
$$
S \subseteq H \cap V_\omega \cap \bigcap_{t \in [0, \omega]} \left( R_t(L) + \mathbf{x}(t) \right).
$$
and the problem becomes the maximization of the area of $S$ with respect to the coordinates $\mathbf{x}:[0, \omega] \to \mathbb{R}^2$ of inner corners $L_t$. Note that a monotone sofa is when each hallway is the tangent hallway. 
...
Then from (1), (2) and (3) above, we can always assume that $S$ is contained in the intersection ^eqn-monotone
$$
...
[^translation]: Technically, translating the moving sofa $S$ may invalidate the initial condition $S \subseteq L_H$. We will relax the [[c. Upper Bound A1/10. Notations and conventions/00. Preface#^def-sofa]] of a moving sofa $S$ so that only some translation of $S$ is required to be movable from $L_H$ to $L_V$ inside $L$.
