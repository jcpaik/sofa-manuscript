A fundamental idea of Gerver [@gerverMovingSofaCorner1992] is to see a moving sofa as the intersection of rotating (and translating) hallways. Fix a moving sofa $S$. As $S$ moves inside the hallway $L$, look at the hallway in the perspective of $S$, so that $S$ is fixed and the hallway $L$ rotates and translates around $S$ while containing $S$. This perspective suggests that if $S$ attains the maximum area, we can assume that $S$ is the intersection of rotating hallways. 

used in establishing both the lower and upper bound of $\mu_{\max}$ in [[a. Angle Hypothesis/01. Introduction/01. Main Result#^eqn-area-bounds]], is to change perspective and solve the moving _hallway_ problem instead. This suggests the idea that the moving sofa $S$ is essentially the intersection of rotating hallways $L_t$ of angles $t$ from $0$ to $\omega$ (see ^fig-monotone-sofa).  which is the intersection of 
