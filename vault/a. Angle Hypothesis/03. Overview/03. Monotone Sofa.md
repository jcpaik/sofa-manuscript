...
1. Each rotating hallway $L_t$ can be taken to be the _tangent hallways_ of $S$ making contact with $S$ in its outer walls. As a consequence, the outer walls of $L_t$ form the supporting lines of a convex body $K$ that we call the _cap_ of $S$ (see [[a. Angle Hypothesis/03. Overview/03. Monotone Sofa#^fig-monotone-sofa]]).
2. The intersection $\mathcal{I}$ of $H$, $V_\omega$ and the tangent hallways $L_t$ is indeed connected. As a consequence, the equality $S = \mathcal{I}$ holds for maximum-area $S$.

We will call any moving sofa $S$ (not necessarily of maximum area) satisfying the two properties above a _monotone sofa_. Thus, it suffices to only consider monotone sofas for .  A consequence of (2) is that, if $S$ attains the maximum area, then the equality $S = \mathcal{I}$ indeed holds.
