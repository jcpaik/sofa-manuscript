Here, we prove properties of the support function $p_K$ ([[10. Notations and Conventions/00. Preface#^def-support-function]]) and the vertices $v_K^{\pm}(t)$ ([[10. Notations and Conventions/00. Preface#^def-convex-body-vertex]]) of a convex body $K$.
...
_Proof._ Take two arbitrary values $t_1 < t_2$ in the interval $[t_0, t_0 +  \pi]$. The points $v_K^+(t_1), l_K(t_1) \cap l_K(t_2), v_K^-(t_2), v_K^+(t_2)$ goes further in the direction of $-v_{t_0}$ ([[15. Monotone sofas/12. Structure of a Monotone Sofa#^def-further-in-direction]]) in the increasing order. This shows that the value $v_K^+(t) \cdot v_{t_0}$ is monotonically decreasing on the interval $t \in [t_0, t_0 + \pi]$. A symmetric argument proves the other claim. □
