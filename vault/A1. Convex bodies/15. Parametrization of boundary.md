...
We define the closed-interval variant $\mathbf{b}_K^{t_0-, t_1}$ of curve $\mathbf{b}_K^{t_0, t_1}$. With following theorems, $\mathbf{b}_K^{t_0-, t_1}$ is essentially the arc-length parametrization of the curve connecting $v_K^-(t_0)$ to $v_K^+(t_1)$ along the boundary $\partial K$ counterclockwise.
