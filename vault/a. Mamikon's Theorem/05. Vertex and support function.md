...
Note that the sum used in [[a. Mamikon's Theorem/05. Vertex and support function#^def-convex-body-linear]] is the Minkowski sum of convex bodies. That is, $aK = \left\{ ap : p \in K \right\}$ and $K_1 + K_2 = \left\{ p_1 + p_2 : p_1 \in K_1, p_2 : K_2 \right\}$.
...
_Proof._ First condition of [[a. Mamikon's Theorem/05. Vertex and support function#^def-convex-body-linear]] on $p_K$ follows from a direct argument.
$$
...
\end{align*}
$$
so the second condition of [[a. Mamikon's Theorem/05. Vertex and support function#^def-convex-body-linear]] is true. □

We will soon show that the vertex $v_K^{+}(t)$ (resp. $v_K^-(t)$) is right-continuous (resp. left-continuous) respect to $t$ and is linear with respect to $K$. To do so, we compute the limit of vertices via [[a. Mamikon's Theorem/05. Vertex and support function#^thm-limits-converging-to-vertex]].
...
_Proof._ The point $p = v_K(t_1, t_2)$ is the unique point such that $p \cdot u_{t_1} = p_K(t_1)$ and $p \cdot u_{t_2} = p_K(t_2)$ holds. By solving the linear equations, observe that the coordinates of $p$ are linear combinations of $p_K(t_1)$ and $p_K(t_2)$. By [[a. Mamikon's Theorem/05. Vertex and support function#^pro-support-function-linear]] the result follows. □
...
The vertex $v_K^+(t)$ is right-continuous by [[a. Mamikon's Theorem/05. Vertex and support function#^thm-limits-converging-to-vertex]].
...
From [[a. Mamikon's Theorem/05. Vertex and support function#^lem-tangent-lines-intersection-linear]] and [[a. Mamikon's Theorem/05. Vertex and support function#^thm-limits-converging-to-vertex]], we have the linearlity of vertices $v_K^{\pm}(t)$.
...
Using [[a. Mamikon's Theorem/05. Vertex and support function#^lem-intersection-tangent-lines]], we can show that $v_K(t, t')$ parametrizes the half-lines in $l_K(t)$ emanating from $v_K^{\pm}(t)$.
...
_Proof._ The function $\mathbf{p}$ is continuous at $t$ because of [[a. Mamikon's Theorem/05. Vertex and support function#^thm-limits-converging-to-vertex]]. The function $\mathbf{p}$ is absolutely continuous on any closed subinterval of $(t, t+ \pi)$ by [[a. Mamikon's Theorem/05. Vertex and support function#^lem-intersection-tangent-lines]] and [[a. Mamikon's Theorem/05. Vertex and support function#^thm-support-function-lipschitz]]. So the derivative $\mathbf{p}'(u)$ of $\mathbf{p}$ on $u \in (t, t + \pi)$ exists almost everywhere and $\mathbf{p}(u_2) - \mathbf{p}(u_1) = \int_{u_1}^{u_2} \mathbf{p}'(u) \, du$ for every $t < u_1 < u_2 < t + \pi$. Take the limit $u_1 \to t^+$ to obtain $\mathbf{p}(u_2) - \mathbf{p}(t) = \int_{t}^{u_2} \mathbf{p}'(u)\,du$. So $\mathbf{p}$ is absolutely continuous on any closed subinterval of $[t, t + \pi)$ including the endpoint $t$. This verifies (1).
...
By taking the limit $u \to t + \pi^-$ in [[a. Mamikon's Theorem/05. Vertex and support function#^lem-intersection-tangent-lines]], we have $\alpha(u) \to \infty$ (note that we use the fact that the width $p_K(t + \pi) + p_K(t)$ is positive). Now (3) follows from (1), (2), and $\alpha(u) \to \infty$. □
