...
> 
> - $(t_1, t_2)$ is disjoint from the angular support of $K$
> - $v_K^+(t_1) = v_K^-(t_2)$
> - Both $v_K^+(t_1)$ and $v_K^-(t_2)$ are equal to a point $p$. Also, $p = l_K(a) \cap l_K(b)$ for every $a, b \in [t_1, t_2]$ where $a \neq b$. Al

> __Theorem [convex-set-support].__ Let $\Pi$ be any closed subset of $S^1$ such that $S^1 \setminus \Pi$ is the disjoint union of open intervals of length $< \pi$. Then for any convex body $K$, $K = \bigcap_{t \in \Theta} H_K(t)$ holds if and only if the angular support of $K$ is contained in $\Pi$. ^thm-convex-set-support
