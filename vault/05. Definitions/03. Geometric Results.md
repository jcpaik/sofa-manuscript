...
> __Theorem [boundary-parametrization-segment].__ For any $p, q \in \partial K$, define $\mathbf{x}_{K, p, q}$ as the initial part of $\mathbf{x}_{K, p}$ that connects $p$ and $q$ counterclockwise in $\partial K$ (if $q=p$ then choose $\mathbf{x}_{K, p, p} = \mathbf{x}_p$). Then for any $t_1, t_2 \in \mathbb{R}$ with $t_2 \in (t_1, t_1 + 2\pi]$, the function $\mathbf{x}_{K, v^+_K(t_1), v^+_K(t_2)}$ parametrizes the set $\left\{ v_K^+(t_1) \right\} \bigcup \cup_{t \in (t_1, t_2]} e_K(t)$. Similarly, if $t_2 < t_1 + 2\pi$ and $v_K^-(t_1) \neq v_K^+(t_2)$ then the function $\mathbf{x}_{K, v_K^-(t_1), v_K^+(t_2)}$ parametrizes the set $\bigcup_{t \in [t_1, t_2]} e_K(t)$. ^thm-boundary-parametrization-segment
...
> __Theorem [mamikon-tangent-line].__ Let $K$ be an arbitrary convex body. Let $p, q \in \partial K$ be two different points on the boundary of $K$. Let $t_0, t_1 \in \mathbb{R}$ be the angles such that $p \in e_K(t_0)$ and $q \in e_K(t_1)$ and $t_0 < t_1 < t_0 + \pi$. For every $t \in [t_0, t_1)$, define $f(t)$ to be the value such that $l_K(t) \cap l_K(t_1) = v_K^+(t) + f(t) v_t$. Let $r = l_K(t_0) \cap l_K(t_1)$. Then $f$ is integrable and the following holds. ^thm-mamikon-tangent-line
$$
I(r, q) - I(\mathbf{x}_{K, p, q}) - I\left(r, p \right) =  \frac{1}{2}\int_{t_0}^{t_1} f(t) ^2 \, dt
$$
