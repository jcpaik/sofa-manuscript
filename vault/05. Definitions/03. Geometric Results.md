...
> __Theorem [supported-iff].__ A convex body $K$ is supported by the closed set $\Theta \subseteq S^1$ if and only if for any $t_1, t_2 \in \mathbb{R}$ such that $t_1 < t_2 < t_1 + \pi$, if the open interval $(t_1, t_2)$ of $S^1$ is disjoint from $K$ then $v_K^+(t) = v_K^-(t) = l_K(t_1) \cap l_K(t_2)$ for all $t \in (t_1, t_2)$. ^thm-supported-iff
