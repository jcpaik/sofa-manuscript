...
The fundamental idea of Gerver [@gerverMovingSofaCorner1992], used in establishing both the lower and upper bound of $\mu_{\max}$ in [[b. Angle Hypothesis/01. Introduction/01. Main Result#^eqn-area-bounds]], is to change perspective and solve the moving _hallway_ problem instead. Fix the rotation angle $\omega$ of a moving sofa $S$. As $S$ moves inside the hallway $L$, look at the hallway in perspective of $S$ so that $S$ is fixed and the hallway rotates and translates around $S$. This suggests the idea that the moving sofa $S$ is essentially the intersection of rotating hallways $L_t$ of angles $t$ from $0$ to $\omega$ (see ^fig-monotone-sofa). We establish this rigorously by showing that a maximum-area moving sofa should be a _monotone sofa_ which is the intersection of 
...
To prove [[b. Angle Hypothesis/01. Introduction/03. Monotone Sofas#^thm-extend-rotation-angle]], we will find two 
...
[^translation]: Technically, translating the moving sofa $S$ may invalidate the initial condition $S \subseteq L_H$. To resolve this, we relax the [[a. Upper Bound A1/10. Notations and conventions/00. Preface#^def-sofa]] of a moving sofa $S$ so that only some translation of $S$ is required to be movable from $L_H$ to $L_V$ inside $L$.
