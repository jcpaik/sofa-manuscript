...
> __Theorem [main-balanced].__ Let $\omega \in (0, \pi/2]$ be arbitrary. Then there exists a maximizer $K \in \mathcal{K}_\omega$ of the sofa area functional $\mathcal{A}_{\omega}(K)$ such that ^thm-main-balanced
> 
> 1. if $\omega < \pi/2$, then $w_K^\circ \leq \beta_K\left( \pi/2 \right)$ and $z_K^\circ \leq \beta_K\left( \omega \right)$ and
