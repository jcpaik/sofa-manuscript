...
Our goal is to show the existence of a maximizer satisfying additional properties.

> __Theorem [max-monotone-balanced].__ There exists a cap $K \in \mathcal{K}_\omega$ with rotation angle $\omega \in (0, \pi/2]$. It attains the maximum value of $\mathcal{A}(K)$. We have $p_K(\omega + \pi/2) \geq 1.1$. We also have $\beta_K(\left\{ \omega \right\}) \geq 0.1$.
> ^thm-max-monotone-balanced
