...
Note that if $K \in \mathcal{K}_\Theta$, then we have $F_K = F_\omega$ so [[15. Limiting theorem#^def-cap-translate-niche]] agrees with [[10. Monotone Sofa#^def-niche]], justifying the same notion $\mathcal{N}(K)$ used in [[15. Limiting theorem#^def-cap-translate-niche]].
