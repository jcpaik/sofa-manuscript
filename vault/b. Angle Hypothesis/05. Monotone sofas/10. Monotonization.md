...
Compare the equation in [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^def-monotonization]] to [[a. Upper Bound A1/01. Introduction/02. Moving hallway problem#^eqn-monotone]] in [[a. Upper Bound A1/01. Introduction/02. Moving hallway problem]]. The paralleogram $P_\omega$ is the intersection of $H$ and $V_\omega$ ([[a. Upper Bound A1/10. Notations and conventions/02. Moving sofa#^def-parallelogram]]), and the tangent hallways $L_S(t)$ are the rotating hallways $L_t$ making contact with $S$ in the outer walls as described in [[a. Upper Bound A1/01. Introduction/02. Moving hallway problem]]. Condition 3 of [[a. Upper Bound A1/15. Monotone sofas/07. Tangent Hallway#^thm-moving-sofa-iff-hallway-intersection]] implies that the set $\mathcal{M}(S)$ contains $S$.
...
_Proof._ By [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^thm-monotonization-is-connected]], the shape $\mathcal{M}(S)$ is connected. By [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^def-monotonization]], the set $\mathcal{M}(S)$ is contained $P_\omega$ and $L_S(t)$ for all $t \in [0, \omega]$, so it satisfies the second condition of [[a. Upper Bound A1/15. Monotone sofas/07. Tangent Hallway#^thm-moving-sofa-iff-hallway-intersection]]. So the set $\mathcal{M}(S)$ is a moving sofa with rotation angle $\omega$. $\mathcal{M}(S)$ contains $S$ by [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^cor-monotonization-is-larger]]. From $S \subseteq \mathcal{M}(S) \subseteq P_\omega$ and
$$
...
The moving sofa problem asks for the largest-area moving sofas. So [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^thm-monotonization-is-sofa]] tells us that we only need to consider monotone sofas for the problem. The rest of this [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization]] proves [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^thm-monotonization-is-connected]] as promised.

# Proof of [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^thm-monotonization-is-connected]]
...
The set $\mathcal{C}(S)$ will later be called as the _cap_ of $S$ ([[a. Upper Bound A1/15. Monotone sofas/12. Structure of a monotone sofa#^thm-cap-hallway-intersection]]) after defining the notion of cap in [[a. Upper Bound A1/15. Monotone sofas/12. Structure of a monotone sofa#^def-cap]]. We don't need this notion of cap as of now.
...
_Proof._ We have $S \subseteq \mathcal{M}(S) \subseteq \mathcal{C}(S)$ by [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^cor-monotonization-is-larger]] and $L_S(t) \subset Q_S^+(t)$. So it remains to show $p_{\mathcal{C}(S)}(t) \leq p_S(t)$ for every $t$ in $J_\omega$. By the definition of $\mathcal{C}(S)$ we have $S \subseteq \mathcal{C}(S) \subseteq H_S(t)$. So we have $p_{\mathcal{C}(S)}(t) \leq p_S(t)$ indeed. □
