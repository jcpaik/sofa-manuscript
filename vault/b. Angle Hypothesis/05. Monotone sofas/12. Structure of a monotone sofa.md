Here, we show that any monotone sofa $S$ is always equal to a _cap_ $K$ minus its _niche_ $\mathcal{N}(K)$ ([[a. Upper Bound A1/15. Monotone sofas/12. Structure of a monotone sofa#^thm-monotone-sofa-structure]]; see [[a. Upper Bound A1/01. Introduction/02. Moving hallway problem#^fig-monotone-sofa]] in [[a. Upper Bound A1/01. Introduction/02. Moving hallway problem]]).
...
> 2. $K$ is an intersection of closed half-planes with normal angles ([[a. Upper Bound A1/10. Notations and conventions/00. Preface#^def-half-plane]]) in $J_\omega \cup \{\pi + \omega, 3\pi/2\}$.

Geometrically, the first condition of [[#^def-cap]] states that $K$ is contained in the parallelogram $P_\omega$ making contact with all sides of $P_\omega$. By [[a. Upper Bound A1/A1. Convex bodies/20. Normal angles#^thm-convex-set-support]], the second condition of [[a. Upper Bound A1/15. Monotone sofas/12. Structure of a monotone sofa#^def-cap]] is equivalent to saying that the _normal angles_ $\mathbf{n}(K)$ of $K$ ([[a. Upper Bound A1/A1. Convex bodies/20. Normal angles#^def-convex-set-support]]) is contained in the set $J_\omega \cup \{\pi + \omega, 3\pi/2\}$. See [[a. Upper Bound A1/A1. Convex bodies/20. Normal angles]] for a quick overview of $\mathbf{n}(K)$.

We will show that the set $\mathcal{C}(S)$ in [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^def-cap-sofa]] is a cap with rotation angle $\omega$. This justifies calling $\mathcal{C}(S)$ _the cap of_ $S$ associated to $S$.

> __Theorem [cap-hallway-intersection].__ The set $\mathcal{C}(S)$ in [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^def-cap-sofa]] is a cap with rotation angle $\omega$ as in [[#^def-cap]]. With this, call $\mathcal{C}(S)$ the _cap of the moving sofa_ $S$. ^thm-cap-hallway-intersection

We postpone the proof of [[a. Upper Bound A1/15. Monotone sofas/12. Structure of a monotone sofa#^thm-cap-hallway-intersection]] at the end of this [[a. Upper Bound A1/15. Monotone sofas/12. Structure of a monotone sofa]]. Define the _niche_ $\mathcal{N}(K)$ associated to any cap $K$.
...
_Proof._ By [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^def-cap-sofa]] and [[a. Upper Bound A1/15. Monotone sofas/07. Tangent Hallway#^pro-rotating-hallway-parts]], the set $\mathcal{C}(X)$ of $X = S$ or $\mathcal{M}(S)$ depend only on the values of the support function $p_X$ on $J_\omega$. The support functions of $S$ and $\mathcal{M}(S)$ match on $J_\omega$ by [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^lem-cap-same-support-function]], completing the proof. □
...
Note that this variant does not mention anything about monotonization. In particular, by [[a. Upper Bound A1/15. Monotone sofas/12. Structure of a monotone sofa#^thm-monotone-sofa-structure]] any monotone sofa $S$ can be recovered from its cap $K = \mathcal{C}(S)$.

Monotization $S \mapsto \mathcal{M}(S)$ is a process that enlarges any moving sofa $S$ by [[a. Upper Bound A1/15. Monotone sofas/10. Monotonization#^thm-monotonization-is-sofa]]. Moreover, if $S$ is already monotone (so that $S = \mathcal{M}(S')$ for some $S'$), then the monotonization fixes $S$.
...
$$
which holds from [[a. Upper Bound A1/15. Monotone sofas/12. Structure of a monotone sofa#^thm-monotonization-structure]] and [[a. Upper Bound A1/15. Monotone sofas/12. Structure of a monotone sofa#^pro-monotonization-cap]]. □
...
# Proof of [[a. Upper Bound A1/15. Monotone sofas/12. Structure of a monotone sofa#^thm-cap-hallway-intersection]]
