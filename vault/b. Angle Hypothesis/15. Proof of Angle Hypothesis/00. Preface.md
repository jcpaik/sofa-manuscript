...
We want to show that the outer $a_X(t)$ of $L_X(t)$ passes $q_0$ and $c_X(t)$ of $L_X(t)$ passes $q_0$. Then by Proposition on representation of $Q_X^-(t)$, it turns out that $Q_X^-(t) = H_-^{\circ}(t, q_0 \cdot u_t - 1) \cap H_-^{\circ}(t + \pi/2, q_1 \cdot v_t - 1)$. To show $q_0 \in a_X(t)$, it suffices to show that $q_0$ is further than $q_1$ in the direction of $u_t$. That is, the angle the line connecting $q_0$ to $q_1$ makes with the $x$-axis is at least $t + \pi/2 = \pi - \omega < 3\pi/4$. Note that the height of $q_1 - q_0$ is one, but the width of $q_0 - q_1$ is at least $1.1$ by definiton. So the vector $q_1 - q_0$ makes an angle of at least $3\pi/4$ from the positive $x$-axis. So the proof is complete.
