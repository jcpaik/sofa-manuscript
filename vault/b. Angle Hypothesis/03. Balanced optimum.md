...
> __Lemma [extended-cap-space-optimum].__ Let $\Theta$ be any finite angle set with rotation angle $\omega \in (0, \pi/2]$ and size $\geq 3$. Let $K \in \mathcal{K}_\Theta'$ be any maximizer of the sofa area functional $\mathcal{A}_{\omega}' : \mathcal{K}_\Theta' \to \mathbb{R}$. Then $K$ must be a translation of a cap in $\mathcal{K}_\Theta$.
> ^lem-extended-cap-space-optimum

_Proof._ Take any maximizer $K \in \mathcal{K}_\Theta'$ of $\mathcal{A}'_\Theta(K)$. We will show that the width of $K$ measured in the direction of $v_0$ is equal to 1. Then by a symmetric argument, we can also show that the width of $K$ measured in the direction of $u_\omega$ is also equal to 1.

Fix $K$. As $\Theta \to [0, \omega]$, we have $\mathcal{A}_\Theta(K)$ converging to $\mathcal{A}(K)$. WHY?
- A point $p \in \mathcal{N}(K)$ is contained in a wedge $T_K(t)$
- Tweak $t$ a little bit. $T_K(t)$ is open w.r.t. $F_\omega$ so $p$ is still contained in $T_K(t)$
- 
...
$$
with new locations $\mathbf{x} : \Theta \to \mathbb{R}^2$. The new locations $\mathbf{x}$ are defined as $\mathbf{x}(t) = \mathbf{x}_K(t)$ for all $t \in \Theta \setminus \left\{ 0 \right\}$, thus fixing the translates of $R_t(L)$ and $R_\omega(H)$, and $\mathbf{x}(0) = \mathbf{x}_K(0) - \epsilon u_0$ for sufficiently small $\epsilon > 0$, thus moving the translate of $H$ slightly downwards.
