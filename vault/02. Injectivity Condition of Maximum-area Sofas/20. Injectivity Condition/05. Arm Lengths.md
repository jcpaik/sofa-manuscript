...
\end{align*}
$$
> Likewise, the left derivatives $\partial^- \mathbf{y}_K(t)$ and $\partial^- \mathbf{x}_K(t)$ of $\mathbf{y}_K(t)$ and $\mathbf{x}_K(t)$ exists for all $0 < t \leq \pi/2$ and is equal to the following.
$$
\begin{align*}
	\partial^- \mathbf{y}_K(t) = -f_K^-(t) u_t + g_K^-(t) v_t \qquad \partial^+ \mathbf{x}_K(t) = -(f_K^-(t) - 1) u_t + (g_K^-(t) - 1) v_t
\end{align*}
