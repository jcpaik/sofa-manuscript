...
> 2. The inner corner $\mathbf{x}_K : [0, \pi/2] \to \mathbb{R}^2$ is continously differentiable, and we have $\mathbf{x}_K'(t) = -(f_K(t) - 1) u_t + (g_K(t) - 1) v_t$
...
_Proof._ By [[#^lem-balanced-ineq-limit]] and the Radon-Nikodym theorem, we have $\sigma_K = s_K(t) \mathrm{d} t$ on the domain $t \in [0, \pi/2)$ for some measurable function $s_K : [0, \pi/2) \to \mathbb{R}_{\geq 0}$ that is unique up to null set. In particular, we can assume $s_K \leq g_K^+$ on $[0, \pi/2)$. □