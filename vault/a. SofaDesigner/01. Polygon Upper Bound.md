We rigorously define the polygon upper bound $\mathcal{A}_\Theta$ as a functional on a subset $\mathcal{K}_\Theta$ of the cap space $\mathcal{K}_\omega$.
...
Any affine functional $f$ on $\mathcal{K}_\Theta$ lifts naturally to 

> __Theorem [sofa-designer-api-inequality-proof].__ Let $\Theta$ be a finite partition of $[0, \pi/2]$. Let $I$ be an inequality depending solely on the angles of $\Theta \setminus \left\{ \pi/2 \right\}$. If the procedure $SofaProve(\Theta, T, I)$ returns true, then the inequality $I$ is true for all $\mathcal{K}_\omega$ and 
> ^thm-sofa-designer-api-inequality-proof

Exactly what properties of $\mathcal{K}_\Theta$ with rotation angle $\pi/2$ transfers to $\mathcal{K}_\omega$?

It's that we need $\omega \in [\theta_{n-1}, \theta_n]$. 
