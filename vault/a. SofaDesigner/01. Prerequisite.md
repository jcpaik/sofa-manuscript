We rigorously set up the area $\mathcal{A}$ of a monotone sofa and its polygon upper bound $\mathcal{A}_\Theta$. The general idea is that any monotone sofa $S$ is always equal to its _cap_ $K = \mathcal{C}(S)$ (essentially, the convex set enclosing $S$ from above) minus the _niche_ of cap $K$ (essentially, the region ). 
...
> __Proposition [angled-cap].__ The set $\mathcal{C}_\Theta(S)$ in [[a. SofaDesigner/01. Prerequisite#^def-angled-cap]] is a member of $\mathcal{K}_\Theta$. So in particular, $\mathcal{C}_\Theta : \mathcal{K}_\omega \to \mathcal{K}_\Theta$ is a map from $\mathcal{K}_\omega$ to $\mathcal{K}_\Theta$.
> ^pro-angled-cap
