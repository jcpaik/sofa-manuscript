...
$$
The lower bound $2.2195\dots \leq \mu_{\max}$ comes from Gerver's sofa $S_G$ of area $\mu_G := 2.2195\dots$ constructed in 1994 [[@gerverMovingSofaCorner1992]]. Gerver conjectured $\mu_G = \mu_{\max}$, that his sofa attains the maximum possible area, but this remains unproven to this date. Approximate solutions found by computer experiments are consistent with his [[01. Introduction/00. Preface#^con-gerver]].[^experiments] On the other hand, the upper bound $\mu_{\max} \leq 2.37$ was proved by Kallus and Romik using computer assistance [[@kallusImprovedUpperBounds2018]].

In this work, we present SofaDesigner, a framework for calculating the polygonal upper bound $\mathcal{A}_\Theta$ exactly. It consists of the following procedures.

- Procedure SofaBranch calculates the upper bound $\mathcal{A}_\Theta$ precisely, and returns it in a form of complete binary tree $T_\Theta$.
- Procedure SofaProve takes a valid sofa tree $T_\Theta$ and attempts to prove the linear inequality $I$. If successful, the procedure produces a 

To verify the mathematical correctness of SofaDesigner, it is not required to trust the procedures 

- Procedure SofaVerify can take any complete binary tree $T_\Theta$ and verify if $T_\Theta$ is indeed a valid branch-and-bound tree representing an uper bound $\mathcal{A}_{T_\Theta}$.
- 
